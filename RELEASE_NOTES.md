...
- ✅ **마스터 라이센스 감지 강화**: 마스터 에디션 빌드 시 모든 기능(삭제 등)이 즉시 해제되도록 감지 로직 고도화.
- ✅ **선택 업데이트/롤백 추가**: 벌크 에디터 툴바에 '선택 업데이트' 및 '선택 롤백' 버튼 신설 및 백엔드 로직 연동.
- ✅ **인톨러 UI 복구**: ZIP 파일 설치 시 '즉시 활성화' 체크박스를 다시 노출하여 사용자 편의성 증대.
- ✅ **번역 명칭 표시 최적화**: 플러그인 목록에서 다국어 명칭이 더 돋보이도록 UI 스타일 및 구조 개선.
