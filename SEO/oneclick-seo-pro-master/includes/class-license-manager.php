...
class OneClick_SEO_License_Manager {
    ...
    public function generate_license($data) {
        ...
        $result = $wpdb->insert(
            $wpdb->prefix . 'oneclick_master_licenses',
            [
    ...
    public function validate_license($license_key, $site_url = null) {
        ...
        $license = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_licenses WHERE license_key = %s",
            $license_key
        ));
        ...
        if ($license->max_sites > 0 && $license->active_sites >= $license->max_sites) {
            ...
            if ($site_url) {
                $existing = $wpdb->get_var($wpdb->prepare(
                    "SELECT COUNT(*) FROM {$wpdb->prefix}oneclick_master_activations 
    ...
    public function activate_site($license_key, $site_url, $site_data = []) {
        ...
        if ($existing) {
            // Reactivate
            $wpdb->update(
                $wpdb->prefix . 'oneclick_master_activations',
                [
            ...
            $wpdb->query($wpdb->prepare(
                "UPDATE {$wpdb->prefix}oneclick_master_licenses SET active_sites = active_sites + 1 WHERE id = %d",
                $license->id
            ));
        }
    ...
    public function deactivate_site($activation_token) {
        ...
        $activation = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_activations WHERE activation_token = %s AND status = 'active'",
            $activation_token
        ));
        ...
        $wpdb->query($wpdb->prepare(
            "UPDATE {$wpdb->prefix}oneclick_master_licenses SET active_sites = GREATEST(0, active_sites - 1) WHERE id = %d",
    ...
    public function get_licenses($args = []) {
        ...
        $count_sql = "SELECT COUNT(*) FROM {$wpdb->prefix}oneclick_master_licenses WHERE $where_sql";
        $total = $wpdb->get_var($params ? $wpdb->prepare($count_sql, $params) : $count_sql);

        $params[] = $args['per_page'];
        $params[] = $offset;

        $sql = "SELECT * FROM {$wpdb->prefix}oneclick_master_licenses WHERE $where_sql ORDER BY created_at DESC LIMIT %d OFFSET %d";
    ...
    public function get_activations($license_id) {
        ...
        return $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_activations WHERE license_id = %d ORDER BY activated_at DESC",
            $license_id
        ));
    }
    ...
    public function ajax_generate_license() {
        check_ajax_referer('oneclick_master_nonce', 'nonce');
