...
class OneClick_SEO_Rank_Tracker {
    ...
    private function __construct() {
        add_action('oneclick_seo_rank_check', [$this, 'run_scheduled_check']);
    ...
    public function check_keyword_rank($keyword_id) {
        ...
        $keyword = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_keywords WHERE id = %d",
            $keyword_id
        ));
        ...
        $previous = $wpdb->get_var($wpdb->prepare(
            "SELECT position FROM {$wpdb->prefix}oneclick_master_rank_history 
    ...
    public function run_scheduled_check() {
        ...
        $keywords = $wpdb->get_results(
            "SELECT id FROM {$wpdb->prefix}oneclick_master_keywords WHERE status = 'active'"
        );
    ...
    public function get_history($keyword_id, $days = 30) {
        ...
        return $wpdb->get_results($wpdb->prepare(
            "SELECT position, previous_position, url, serp_features, checked_at 
             FROM {$wpdb->prefix}oneclick_master_rank_history 
    ...
    public function get_keywords($args = []) {
        ...
        $sql = "SELECT k.*, 
                ...
                FROM {$wpdb->prefix}oneclick_master_keywords k
                LEFT JOIN (
    ...
    public function get_statistics($site_id = null) {
        ...
        // Total keywords
        $total = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}oneclick_master_keywords k $where"
        );
