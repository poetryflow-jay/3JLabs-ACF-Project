...
class OneClick_SEO_Remote_Site_Manager {
    ...
    public function add_site($data) {
        ...
        $existing = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}oneclick_master_remote_sites WHERE site_url = %s",
            $site_url
        ));
        ...
        $result = $wpdb->insert(
            $wpdb->prefix . 'oneclick_master_remote_sites',
            [
                'site_url' => $site_url,
...
(function(){
    ...
    s.src='{$master_url}/wp-content/plugins/oneclick-seo-master/assets/js/seo-snippet.min.js';
...
HTML;
    ...
    public function crawl_site($site_id) {
        ...
        $site = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_remote_sites WHERE id = %d",
            $site_id
        ));
        ...
        // Save audit
        $wpdb->insert(
            $wpdb->prefix . 'oneclick_master_audits',
            [
                'site_id' => $site_id,
                'audit_type' => 'crawl',
                'overall_score' => $score,
    ...
    public function push_changes($site_id, $changes) {
        ...
        foreach ($changes as $change) {
            $wpdb->insert(
                $wpdb->prefix . 'oneclick_master_bulk_queue',
                [
                    'site_id' => $site_id,
    ...
    public function get_pending_changes($snippet_key) {
        ...
        $site = $wpdb->get_row($wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}oneclick_master_remote_sites WHERE snippet_key = %s",
            $snippet_key
        ));
        ...
        return $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_bulk_queue 
    ...
    public function mark_changes_applied($change_ids) {
        ...
        $wpdb->query(
            "UPDATE {$wpdb->prefix}oneclick_master_bulk_queue 
    ...
    public function get_sites($args = []) {
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}oneclick_master_remote_sites WHERE $where_sql ORDER BY created_at DESC LIMIT %d OFFSET %d";
    ...
    public function ajax_remove_site() {
        ...
        $wpdb->delete($wpdb->prefix . 'oneclick_master_remote_sites', ['id' => $site_id]);
        $wpdb->delete($wpdb->prefix . 'oneclick_master_bulk_queue', ['site_id' => $site_id]);
