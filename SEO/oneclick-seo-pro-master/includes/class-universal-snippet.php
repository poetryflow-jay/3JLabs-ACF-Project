...
class OneClick_SEO_Universal_Snippet {
    ...
    public function get_snippet_config($request) {
        ...
        $site = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}oneclick_master_remote_sites WHERE snippet_key = %s AND status = 'active'",
            $snippet_key
        ));
        ...
        $pending_changes = $wpdb->get_results($wpdb->prepare(
            "SELECT id, change_type, target_selector, new_value 
             FROM {$wpdb->prefix}oneclick_master_bulk_queue 
