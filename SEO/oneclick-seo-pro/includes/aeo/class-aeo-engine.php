...
class OneClick_SEO_Pro_AEO_Engine {
    ...
    private function load_settings() {
        $this->settings = get_option('oneclick_seo_pro_aeo', [
            'enabled' => true,
    ...
    private function init_dependencies() {
        ...
        if (class_exists('OneClick_SEO_Pro_FAQ_Generator')) {
            $this->faq_generator = new OneClick_SEO_Pro_FAQ_Generator();
        }

        if (class_exists('OneClick_SEO_Pro_Featured_Snippet_Optimizer')) {
            $this->snippet_optimizer = new OneClick_SEO_Pro_Featured_Snippet_Optimizer();
        }

        if (class_exists('OneClick_SEO_Pro_AI_Engine')) {
            $this->ai_engine = new OneClick_SEO_Pro_AI_Engine();
        }
    }
    ...
    public function enhance_content_for_aeo($content) {
        ...
        $aeo_enabled = get_post_meta($post->ID, '_oneclick_seo_pro_aeo_enabled', true);
        if ($aeo_enabled === '0') {
            return $content;
        }
    ...
    public function add_aeo_meta_box() {
        ...
        foreach ($post_types as $post_type) {
            add_meta_box(
                ...
                __('AEO Analysis', 'oneclick-seo-pro'),
    ...
    public function render_aeo_meta_box($post) {
        wp_nonce_field('oneclick_seo_pro_aeo_nonce', 'oneclick_seo_pro_aeo_nonce_field');
        ...
        $analysis = $this->analyze($post);
        ?>
        <div class="oneclick-seo-pro-aeo-meta">
            <p>
                <label>
                    <input type="checkbox" name="oneclick_seo_pro_aeo_enabled" value="1" <?php checked($aeo_enabled, '1'); ?>>
                    <?php esc_html_e('Enable AEO optimization for this content', 'oneclick-seo-pro'); ?>
                </label>
            </p>

            <div class="aeo-score-display">
                <h4><?php esc_html_e('AEO Score', 'oneclick-seo-pro'); ?>:
            ...
            <div class="aeo-recommendations">
                <h4><?php esc_html_e('Top Recommendations', 'oneclick-seo-pro'); ?></h4>
                <ul>
        ...
        <style>
            .oneclick-seo-pro-aeo-meta { padding: 10px 0; }
    ...
    public function save_aeo_meta($post_id) {
        if (!isset($_POST['oneclick_seo_pro_aeo_nonce_field'])) {
            return;
        }

        if (!wp_verify_nonce($_POST['oneclick_seo_pro_aeo_nonce_field'], 'oneclick_seo_pro_aeo_nonce')) {
            return;
        }
        ...
        $aeo_enabled = isset($_POST['oneclick_seo_pro_aeo_enabled']) ? '1' : '0';
        update_post_meta($post_id, '_oneclick_seo_pro_aeo_enabled', $aeo_enabled);
    }
    ...
    public function register_rest_routes() {
        register_rest_route('oneclick-seo-pro/v1', '/aeo/analyze/(?P<id>\d+)', [
            'methods' => 'GET',
        ...
        register_rest_route('oneclick-seo-pro/v1', '/aeo/suggestions/(?P<id>\d+)', [
            'methods' => 'GET',
