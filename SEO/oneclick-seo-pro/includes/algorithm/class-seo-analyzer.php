...
class OneClick_SEO_Pro_Analyzer {
    ...
    public function __construct() {
        $this->pagespeed_api_key = get_option('oneclick_seo_pro_pagespeed_api_key', '');
        $this->search_console_credentials = get_option('oneclick_seo_pro_search_console_credentials', []);
    }
    ...
    public function analyze_url($url, $options = []) {
        ...
        // Check cache first
        $cache_key = 'oneclick_seo_pro_analysis_' . md5($url);
    ...
    public function fetch_pagespeed_data($url, $strategy = 'mobile') {
        ...
        $cache_key = 'oneclick_seo_pro_psi_' . md5($url . $strategy);
        $cached = get_transient($cache_key);
    ...
    public function fetch_search_console_data($url) {
        ...
        $cache_key = 'oneclick_seo_pro_gsc_' . md5($url);
        $cached = get_transient($cache_key);
