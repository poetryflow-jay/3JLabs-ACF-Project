 ...
 * AI Provider Class
 */
class OneClick_SEO_Pro_AI_Provider {
    ...
    private function __construct() {
        ...
        $this->api_keys = [
            'openai' => get_option('oneclick_seo_pro_openai_api_key', ''),
            'anthropic' => get_option('oneclick_seo_pro_anthropic_api_key', ''),
        ];
    }
    ...
    public function generate_title_suggestions($post, $count = 3) {
        ...
        $focus_keyword = get_post_meta($post->ID, '_oneclick_seo_pro_focus_keyword', true);
    ...
    private function call_openai($prompt, $max_tokens = 1000) {
        ...
        if (empty($api_key)) {
            return new WP_Error('no_api_key', __('OpenAI API key not configured', 'oneclick-seo-pro'));
        }
        ...
        if (isset($body['error'])) {
            return new WP_Error('api_error', $body['error']['message'] ?? __('OpenAI API error', 'oneclick-seo-pro'));
        }
    ...
    private function call_anthropic($prompt, $max_tokens = 1000) {
        ...
        if (empty($api_key)) {
            return new WP_Error('no_api_key', __('Anthropic API key not configured', 'oneclick-seo-pro'));
        }
        ...
        if (isset($body['error'])) {
            return new WP_Error('api_error', $body['error']['message'] ?? __('Anthropic API error', 'oneclick-seo-pro'));
        }
