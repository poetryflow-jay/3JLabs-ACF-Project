...
class OneClick_SEO_Pro_Sitemap_Manager {
    ...
    private function load_settings() {
        $this->settings = get_option('oneclick_seo_pro_sitemap', [
            'enabled' => true,
            'include_images' => true,
    ...
    private function init_hooks() {
        ...
        add_action('oneclick_seo_pro_activated', [$this, 'flush_rewrite_rules']);
    ...
    public function register_rewrite_rules() {
        ...
        add_rewrite_rule(
            '^sitemap\.xml$',
            'index.php?oneclick_seo_pro_sitemap=index',
            'top'
        );
        ...
        add_rewrite_rule(
            ...
            'index.php?oneclick_seo_pro_sitemap=$matches[1]&oneclick_seo_pro_sitemap_page=$matches[2]',
            'top'
        );
        ...
        add_rewrite_rule(
            '^sitemap\.xsl$',
            'index.php?oneclick_seo_pro_sitemap=xsl',
            'top'
        );
    }
    ...
    public function add_query_vars($vars) {
        $vars[] = 'oneclick_seo_pro_sitemap';
        $vars[] = 'oneclick_seo_pro_sitemap_page';
        return $vars;
    }
    ...
    public function handle_sitemap_request() {
        $sitemap_type = get_query_var('oneclick_seo_pro_sitemap');

        if (empty($sitemap_type)) {
            return;
        }

        $page = (int) get_query_var('oneclick_seo_pro_sitemap_page', 1);
        if ($page < 1) $page = 1;
    ...
    public function generate_sitemap_index() {
        $cache_key = 'oneclick_seo_pro_sitemap_index';
    ...
    public function generate_post_type_sitemap($post_type, $page = 1) {
        $cache_key = "oneclick_seo_pro_sitemap_{$post_type}_{$page}";
        $cached = get_transient($cache_key);
    ...
    public function generate_taxonomy_sitemap($taxonomy, $page = 1) {
        $cache_key = "oneclick_seo_pro_sitemap_{$taxonomy}_{$page}";
        $cached = get_transient($cache_key);
...
<body>
    ...
    public function clear_sitemap_cache() {
        ...
        $wpdb->query(
            "DELETE FROM {$wpdb->options}
             WHERE option_name LIKE '_transient_oneclick_seo_pro_sitemap%'
             OR option_name LIKE '_transient_timeout_oneclick_seo_pro_sitemap%'"
        );
    ...
    public function schedule_ping($post_id, $post) {
        ...
        if (!wp_next_scheduled('oneclick_seo_pro_ping_search_engines')) {
            wp_schedule_single_event(time() + 60, 'oneclick_seo_pro_ping_search_engines');
        }
    }
    ...
    private function ping_indexnow($url) {
        $api_key = get_option('oneclick_seo_pro_indexnow_key', '');
    ...
    public function exclude_url($post_id) {
        update_post_meta($post_id, '_oneclick_seo_pro_sitemap_exclude', '1');
    ...
    public function include_url($post_id) {
        delete_post_meta($post_id, '_oneclick_seo_pro_sitemap_exclude');
        $this->clear_sitemap_cache();
    }
    ...
    public function is_excluded($post_id) {
        return get_post_meta($post_id, '_oneclick_seo_pro_sitemap_exclude', true) === '1';
    }
