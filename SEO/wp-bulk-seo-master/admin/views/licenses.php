...
$license_manager = WP_Bulk_SEO_License_Manager::get_instance();
...
<div class="wrap seo-master-wrap">
    <h1>
        <?php esc_html_e('License Management', 'wp-bulk-seo-master'); ?>
        <button type="button" class="page-title-action" id="generate-license-btn">
            <?php esc_html_e('Generate New License', 'wp-bulk-seo-master'); ?>
        </button>
    </h1>
    ...
    <form method="get" class="search-form">
        ...
        <p class="search-box">
            <input type="search" name="s" value="<?php echo esc_attr($_GET['s'] ?? ''); ?>" placeholder="<?php esc_attr_e('Search licenses...', 'wp-bulk-seo-master'); ?>">
            <button type="submit" class="button"><?php esc_html_e('Search', 'wp-bulk-seo-master'); ?></button>
        </p>
    </form>
    ...
    <table class="wp-list-table widefat striped">
        <thead>
            <tr>
                <th><?php esc_html_e('License Key', 'wp-bulk-seo-master'); ?></th>
                <th><?php esc_html_e('Type', 'wp-bulk-seo-master'); ?></th>
                <th><?php esc_html_e('Customer', 'wp-bulk-seo-master'); ?></th>
                <th><?php esc_html_e('Sites', 'wp-bulk-seo-master'); ?></th>
                <th><?php esc_html_e('Status', 'wp-bulk-seo-master'); ?></th>
                <th><?php esc_html_e('Expires', 'wp-bulk-seo-master'); ?></th>
                <th><?php esc_html_e('Actions', 'wp-bulk-seo-master'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php if (empty($licenses_data['licenses'])): ?>
            <tr><td colspan="7"><?php esc_html_e('No licenses found', 'wp-bulk-seo-master'); ?></td></tr>
            <?php else: ?>
            ...
            <tr data-id="<?php echo esc_attr($license->id); ?>">
                ...
                <td>
                    <button type="button" class="button button-small view-activations" data-id="<?php echo esc_attr($license->id); ?>"><?php esc_html_e('View Sites', 'wp-bulk-seo-master'); ?></button>
                    <?php if ($license->status === 'active'): ?>
                    <button type="button" class="button button-small revoke-license" data-id="<?php echo esc_attr($license->id); ?>"><?php esc_html_e('Revoke', 'wp-bulk-seo-master'); ?></button>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; ?>
...
<div id="generate-license-modal" class="seo-master-modal" style="display: none;">
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h2><?php esc_html_e('Generate New License', 'wp-bulk-seo-master'); ?></h2>
            <button type="button" class="modal-close">&times;</button>
        </div>
        <div class="modal-body">
            <form id="generate-license-form">
                <p>
                    <label><?php esc_html_e('Customer Email', 'wp-bulk-seo-master'); ?> *</label>
                    <input type="email" name="email" required class="regular-text">
                </p>
                <p>
                    <label><?php esc_html_e('Customer Name', 'wp-bulk-seo-master'); ?></label>
                    <input type="text" name="name" class="regular-text">
                </p>
                <p>
                    <label><?php esc_html_e('License Type', 'wp-bulk-seo-master'); ?></label>
                    <select name="type">
                ...
                <p>
                    <label><?php esc_html_e('Expires At', 'wp-bulk-seo-master'); ?></label>
                    <input type="date" name="expires_at">
                    <small><?php esc_html_e('Leave empty for lifetime license', 'wp-bulk-seo-master'); ?></small>
                </p>
            </form>
        </div>
        <div class="modal-footer">
            <button type="button" class="button modal-cancel"><?php esc_html_e('Cancel', 'wp-bulk-seo-master'); ?></button>
            <button type="button" class="button button-primary" id="submit-license"><?php esc_html_e('Generate License', 'wp-bulk-seo-master'); ?></button>
        </div>
    </div>
</div>
...
<div id="activations-modal" class="seo-master-modal" style="display: none;">
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h2><?php esc_html_e('License Activations', 'wp-bulk-seo-master'); ?></h2>
            <button type="button" class="modal-close">&times;</button>
        </div>
        <div class="modal-body">
            <table class="wp-list-table widefat">
                <thead>
                    <tr>
                        ...
                        <th><?php esc_html_e('Type', 'wp-bulk-seo-master'); ?></th>
                        <th><?php esc_html_e('Status', 'wp-bulk-seo-master'); ?></th>
                        <th><?php esc_html_e('Activated', 'wp-bulk-seo-master'); ?></th>
                    </tr>
                </thead>
...
jQuery(document).ready(function($) {
    ...
    $('#submit-license').on('click', function() {
        ...
        $btn.prop('disabled', true).text('<?php esc_html_e('Generating...', 'wp-bulk-seo-master'); ?>');
        ...
        $.post(seoMaster.ajaxUrl, {
            ...
            if (response.success) {
                alert('<?php esc_html_e('License generated:', 'wp-bulk-seo-master'); ?> ' + response.data.license_key);
                location.reload();
            } else {
                alert(response.data.message || '<?php esc_html_e('Error', 'wp-bulk-seo-master'); ?>');
            }
        }).always(function() {
            $btn.prop('disabled', false).text('<?php esc_html_e('Generate License', 'wp-bulk-seo-master'); ?>');
        });
    });
    ...
    $('.view-activations').on('click', function() {
        ...
        $.post(seoMaster.ajaxUrl, {
            ...
            if (response.success) {
                ...
                $('#activations-list').html(html || '<tr><td colspan="4"><?php esc_html_e('No activations', 'wp-bulk-seo-master'); ?></td></tr>');
    ...
    $('.revoke-license').on('click', function() {
        if (!confirm('<?php esc_html_e('Are you sure you want to revoke this license?', 'wp-bulk-seo-master'); ?>')) return;
