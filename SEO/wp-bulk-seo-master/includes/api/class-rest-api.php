...
class WP_Bulk_SEO_Master_REST_API {
    ...
    public function validate_license($request) {
        ...
        $manager = WP_Bulk_SEO_License_Manager::get_instance();
    ...
    public function get_sites($request) {
        $manager = WP_Bulk_SEO_Remote_Site_Manager::get_instance();
    ...
    public function get_site_audit($request) {
        ...
        $audit = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}seo_master_audits 
        ...
        if (!empty($data['rankings'])) {
            foreach ($data['rankings'] as $ranking) {
                ...
                $keyword_id = $wpdb->get_var($wpdb->prepare(
                    "SELECT id FROM {$wpdb->prefix}seo_master_keywords 
    ...
    public function get_stats($request) {
        ...
        $total_licenses = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}seo_master_licenses"
        );
        $active_licenses = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}seo_master_licenses WHERE status = 'active'"
        );

        // Site stats
        $total_sites = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}seo_master_remote_sites"
        );
        $active_activations = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}seo_master_activations WHERE status = 'active'"
        );
