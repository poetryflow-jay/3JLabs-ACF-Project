...
?>
<div class="wrap wp-bulk-seo-analyzer">
    <h1><?php esc_html_e('SEO Analyzer', 'wp-bulk-seo'); ?></h1>

    <div class="analyzer-container">
        <!-- Single URL Analyzer -->
        <div class="analyzer-card">
            <h2><?php esc_html_e('Analyze URL', 'wp-bulk-seo'); ?></h2>
            <form id="url-analyzer-form" class="analyzer-form">
                <div class="form-group">
                    <label for="analyze-url"><?php esc_html_e('Enter URL to analyze', 'wp-bulk-seo'); ?></label>
                    <div class="input-group">
                        <input type="url" id="analyze-url" name="url" placeholder="https://example.com/page" required>
                        <button type="submit" class="button button-primary">
                            <span class="dashicons dashicons-search"></span>
                            <?php esc_html_e('Analyze', 'wp-bulk-seo'); ?>
                        </button>
                    </div>
                </div>
                <div class="options-row">
                    <label>
                        ...
                        <?php esc_html_e('Include PageSpeed API data', 'wp-bulk-seo'); ?>
                    </label>
                </div>
            </form>
        </div>
        ...
        <!-- Post Selector -->
        <div class="analyzer-card">
            <h2><?php esc_html_e('Analyze Content', 'wp-bulk-seo'); ?></h2>
            <form id="post-analyzer-form" class="analyzer-form">
                <div class="form-group">
                    <label for="post-type-select"><?php esc_html_e('Select Post Type', 'wp-bulk-seo'); ?></label>
                ...
                <div class="form-group">
                    <label for="post-select"><?php esc_html_e('Select Content', 'wp-bulk-seo'); ?></label>
                    <select id="post-select" name="post_id">
                        <option value=""><?php esc_html_e('Select...', 'wp-bulk-seo'); ?></option>
                    </select>
                </div>

                <button type="submit" class="button button-primary" disabled>
                    <span class="dashicons dashicons-chart-bar"></span>
                    <?php esc_html_e('Analyze Selected', 'wp-bulk-seo'); ?>
                </button>
            </form>
        </div>
    </div>
    ...
    <div id="analysis-results" class="analysis-results" style="display: none;">
        <h2><?php esc_html_e('Analysis Results', 'wp-bulk-seo'); ?></h2>
        ...
        <div class="module-scores">
            <h3><?php esc_html_e('Score Breakdown', 'wp-bulk-seo'); ?></h3>
            <div id="module-scores-grid" class="scores-grid"></div>
        </div>

        <!-- Recommendations -->
        <div class="recommendations-section">
            <h3><?php esc_html_e('Recommendations', 'wp-bulk-seo'); ?></h3>
        ...
        <div class="priority-actions-section" id="priority-actions-section" style="display: none;">
            <h3><?php esc_html_e('Priority Actions', 'wp-bulk-seo'); ?></h3>
        ...
        <div class="result-actions">
            <button type="button" id="optimize-btn" class="button button-primary">
                ...
                <?php esc_html_e('Optimize with AI', 'wp-bulk-seo'); ?>
            </button>
            <button type="button" id="export-btn" class="button">
                <span class="dashicons dashicons-download"></span>
                <?php esc_html_e('Export Report', 'wp-bulk-seo'); ?>
            </button>
        </div>
    </div>
    ...
    <div id="loading-overlay" class="loading-overlay" style="display: none;">
        <div class="loading-content">
            ...
            <p><?php esc_html_e('Analyzing...', 'wp-bulk-seo'); ?></p>
        </div>
    </div>
</div>
...
jQuery(document).ready(function($) {
    ...
    $('#post-type-select').on('change', function() {
        ...
        $postSelect.html('<option value=""><?php esc_html_e('Loading...', 'wp-bulk-seo'); ?></option>');

        $.get(wpBulkSeo.ajaxUrl, {
            action: 'wp_bulk_seo_get_posts',
            nonce: wpBulkSeo.nonce,
            post_type: postType
        }).done(function(response) {
            if (response.success) {
                var options = '<option value=""><?php esc_html_e('Select...', 'wp-bulk-seo'); ?></option>';
    ...
    function analyzeUrl(url, usePageSpeed) {
        ...
        $.post(wpBulkSeo.ajaxUrl, {
            action: 'wp_bulk_seo_analyze_url',
            nonce: wpBulkSeo.nonce,
            url: url,
    ...
    function displayResults(data) {
        ...
        if (data.recommendations && data.recommendations.length) {
            ...
            recsHtml = '<li class="no-issues"><?php esc_html_e('No recommendations - great job!', 'wp-bulk-seo'); ?></li>';
        }
    ...
    $('#optimize-btn').on('click', function() {
        var postId = $('#post-select').val();
        if (!postId) {
            alert('<?php esc_html_e('Please select a post to optimize', 'wp-bulk-seo'); ?>');
            return;
        }
    ...
    function showOptimizationModal(optimizations) {
        ...
        modal += '<h3><?php esc_html_e('AI Optimization Suggestions', 'wp-bulk-seo'); ?></h3>';

        if (optimizations.title) {
            modal += '<div class="optimization-item">';
            modal += '<h4><?php esc_html_e('Title', 'wp-bulk-seo'); ?></h4>';
            modal += '<p class="original"><strong><?php esc_html_e('Original:', 'wp-bulk-seo'); ?></strong> ' + optimizations.title.original + '</p>';
            modal += '<p class="optimized"><strong><?php esc_html_e('Optimized:', 'wp-bulk-seo'); ?></strong> ' + optimizations.title.optimized + '</p>';
            modal += '<button class="button apply-optimization" data-type="title"><?php esc_html_e('Apply', 'wp-bulk-seo'); ?></button>';
            modal += '</div>';
        }

        if (optimizations.meta_description) {
            modal += '<div class="optimization-item">';
            modal += '<h4><?php esc_html_e('Meta Description', 'wp-bulk-seo'); ?></h4>';
            modal += '<p class="original"><strong><?php esc_html_e('Original:', 'wp-bulk-seo'); ?></strong> ' + optimizations.meta_description.original + '</p>';
            modal += '<p class="optimized"><strong><?php esc_html_e('Optimized:', 'wp-bulk-seo'); ?></strong> ' + optimizations.meta_description.optimized + '</p>';
            modal += '<button class="button apply-optimization" data-type="meta"><?php esc_html_e('Apply', 'wp-bulk-seo'); ?></button>';
            modal += '</div>';
        }

        modal += '<button class="button modal-close"><?php esc_html_e('Close', 'wp-bulk-seo'); ?></button>';
        modal += '</div></div>';
