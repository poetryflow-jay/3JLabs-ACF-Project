 ...
 * REST API Controller
 */
class WP_Bulk_SEO_REST_Controller {

    /**
     * Namespace
     */
    const NAMESPACE = 'wp-bulk-seo/v1';
    ...
    public function analyze_url($request) {
        ...
        $analyzer = WP_Bulk_SEO_Analyzer::get_instance();
    ...
    public function analyze_post($request) {
        ...
        if (!$post) {
            return new WP_REST_Response([
                'success' => false,
                'message' => __('Post not found', 'wp-bulk-seo'),
            ], 404);
        }
    ...
    public function get_post_seo($request) {
        ...
        $data = [
            ...
            'meta_description' => get_post_meta($post_id, '_wp_bulk_seo_description', true),
            'focus_keyword' => get_post_meta($post_id, '_wp_bulk_seo_focus_keyword', true),
            'canonical_url' => get_post_meta($post_id, '_wp_bulk_seo_canonical', true),
            'robots' => get_post_meta($post_id, '_wp_bulk_seo_robots', true),
            'score' => get_post_meta($post_id, '_wp_bulk_seo_score', true),
            'grade' => get_post_meta($post_id, '_wp_bulk_seo_grade', true),
            'last_analyzed' => get_post_meta($post_id, '_wp_bulk_seo_analyzed', true),
        ];
    ...
    public function update_post_seo($request) {
        ...
        foreach ($fields as $field) {
            ...
            if ($value !== null) {
                $meta_key = '_wp_bulk_seo_' . str_replace('seo_', '', $field);
        ...
        return new WP_REST_Response([
            'success' => true,
            'message' => __('SEO data updated', 'wp-bulk-seo'),
        ], 200);
    }
    ...
    public function get_scores($request) {
        ...
        $table = $wpdb->prefix . 'bulk_seo_scores';
    ...
    public function get_stats($request) {
        ...
        // Issues count
        $issues_table = $wpdb->prefix . 'bulk_seo_issues';
    ...
    public function analyze_aeo($request) {
        ...
        $aeo_engine = WP_Bulk_SEO_AEO_Engine::get_instance();
    ...
    public function generate_faq($request) {
        ...
        $faq_generator = WP_Bulk_SEO_FAQ_Generator::get_instance();
    ...
    public function save_faq($request) {
        ...
        // Save to post meta
        update_post_meta($post_id, '_wp_bulk_seo_faq', $sanitized);

        return new WP_REST_Response([
            'success' => true,
            'message' => __('FAQ saved', 'wp-bulk-seo'),
        ], 200);
    }
    ...
    public function get_schema($request) {
        ...
        $schema_generator = WP_Bulk_SEO_Schema_Generator::get_instance();
    ...
    public function regenerate_sitemap($request) {
        $sitemap_manager = WP_Bulk_SEO_Sitemap_Manager::get_instance();
        $sitemap_manager->regenerate();

        return new WP_REST_Response([
            'success' => true,
            'message' => __('Sitemap regenerated', 'wp-bulk-seo'),
    ...
    public function get_ai_suggestions($request) {
        ...
        $ai = WP_Bulk_SEO_AI_Provider::get_instance();
    ...
    public function get_settings($request) {
        $settings = [
            'general' => get_option('wp_bulk_seo_general', []),
            'sitemap' => get_option('wp_bulk_seo_sitemap', []),
            'schema' => get_option('wp_bulk_seo_organization', []),
            'aeo' => get_option('wp_bulk_seo_aeo', []),
        ];
    ...
    public function update_settings($request) {
        ...
        if (isset($settings['general'])) {
            update_option('wp_bulk_seo_general', $settings['general']);
        }
        if (isset($settings['sitemap'])) {
            update_option('wp_bulk_seo_sitemap', $settings['sitemap']);
        }
        if (isset($settings['schema'])) {
            update_option('wp_bulk_seo_organization', $settings['schema']);
        }
        if (isset($settings['aeo'])) {
            update_option('wp_bulk_seo_aeo', $settings['aeo']);
        }

        return new WP_REST_Response([
            'success' => true,
            'message' => __('Settings updated', 'wp-bulk-seo'),
        ], 200);
    }
    ...
    private function save_score($post_id, $result) {
        ...
        // Update post meta
        update_post_meta($post_id, '_wp_bulk_seo_score', $result['score']['overall'] ?? 0);
        update_post_meta($post_id, '_wp_bulk_seo_grade', $result['score']['grade'] ?? 'F');
        update_post_meta($post_id, '_wp_bulk_seo_analyzed', current_time('mysql'));
    ...
    private function save_issues($post_id, $issues) {
        ...
        $table = $wpdb->prefix . 'bulk_seo_issues';
