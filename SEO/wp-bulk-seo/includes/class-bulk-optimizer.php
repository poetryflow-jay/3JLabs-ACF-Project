...
class WP_Bulk_SEO_Bulk_Optimizer {
    ...
    public function __construct() {
        $this->batch_size = get_option('wp_bulk_seo_bulk_batch_size', 10);
    }
    ...
    public function analyze_all($args = []) {
        $defaults = [
            'post_type' => get_option('wp_bulk_seo_post_types', ['post', 'page']),
            'post_status' => 'publish',
            'posts_per_page' => -1,
            'fields' => 'ids',
        ];
    ...
    public function analyze_post($post_id) {
        $analyzer = wp_bulk_seo()->analyzer;
        $scorer = wp_bulk_seo()->scorer;
    ...
    private function add_schema_markup($post_id) {
        $schema_generator = wp_bulk_seo()->schema_generator;
    ...
    private function log_operation_start($operation, $total) {
        ...
        $wpdb->insert($wpdb->prefix . 'bulk_seo_log', [
    ...
    public function run_scheduled_analysis() {
        if (!get_option('wp_bulk_seo_auto_analyze', true)) {
            return;
        }
