<?php
/**
 * Plugin Name:       3J Neural Link
 * Plugin URI:        https://3j-labs.com/
 * Description:       3J Labs 플러그인 패밀리의 라이센스 인증, 자동 업데이트, 원격 제어를 담당하는 중앙 관리 시스템입니다.
 * Version:           8.0.1
...
define( 'JJ_NEURAL_LINK_VERSION', '8.0.1' ); // [v8.0.0] 다중 플러그인 배포 관리 시스템 완성
...
if ( is_admin() ) {
    $version_manager_file = JJ_NEURAL_LINK_PATH . 'includes/admin/class-jj-neural-link-version-manager.php';
    if ( file_exists( $version_manager_file ) ) {
        require_once $version_manager_file;
        if ( class_exists( 'JJ_Neural_Link_Version_Manager' ) ) {
...
// Cloud API 로드
$cloud_api_file = JJ_NEURAL_LINK_PATH . 'includes/api/class-jj-neural-link-cloud-api.php';
if ( file_exists( $cloud_api_file ) ) {
    require_once $cloud_api_file;
    if ( class_exists( 'JJ_Neural_Link_Cloud_API' ) ) {
