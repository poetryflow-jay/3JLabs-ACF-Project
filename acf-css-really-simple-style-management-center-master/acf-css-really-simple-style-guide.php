...
if ( ! defined( 'JJ_STYLE_GUIDE_VERSION' ) ) {
    define( 'JJ_STYLE_GUIDE_VERSION', '26.2.1' ); // [v26.2.1] UI 버그 수정: 탭 전환, 프리셋 적용, 저장 버튼 문제 해결
}
...
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-css-optimizer-ai.php', true );
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-team-sync.php', true );
...
// [v23.0.4] 넛지 메시지 관리자 로드
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-nudge-messages.php', true );

// [v25.2.0] CSS Variable Extractor 로드 (CSS 변수 자동 추출/생성/내보내기/가져오기)
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-css-variable-extractor.php', true );

// [v25.0.0] 보안 강화 모듈 로드
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-file-integrity-monitor-v25.php', true );
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-update-security-v25.php', true );
$jj_safe_require( JJ_STYLE_GUIDE_PATH . 'includes/class-jj-license-security-v25.php', true );

// [v20.2.2] 모든 클래스 초기화 시점을 init(10)으로 늦춤 (번역 로딩 후 실행되도록)
add_action( 'init', function() {
    global $jj_style_guide, $admin_center, $live_page;
    
    // [v23.0.9] 필수 플러그인 관리자 임시 비활성화
    // Neural Link 서버에 최신 버전 업로드 후 재활성화 예정
    // TGMPA가 구버전(23.0.2)을 계속 설치하는 문제 해결 전까지 비활성화
    /*
    ...
    */
    ...
    // [v25.0.0] 파일 무결성 모니터 v25 초기화
    if ( class_exists( 'JJ_File_Integrity_Monitor_V25' ) ) {
        try {
            JJ_File_Integrity_Monitor_V25::instance();
        } catch ( Exception $e ) {
            if ( function_exists( 'error_log' ) ) error_log( 'JJ File Integrity Monitor V25 Init Failed: ' . $e->getMessage() );
        }
    }

    // [v25.0.0] 업데이트 보안 v25 초기화
    if ( class_exists( 'JJ_Update_Security_V25' ) ) {
        try {
            JJ_Update_Security_V25::instance();
        } catch ( Exception $e ) {
            if ( function_exists( 'error_log' ) ) error_log( 'JJ Update Security V25 Init Failed: ' . $e->getMessage() );
        }
    }

    // [v25.0.0] 라이센스 보안 v25 초기화
    if ( class_exists( 'JJ_License_Security_V25' ) ) {
        try {
            JJ_License_Security_V25::instance();
        } catch ( Exception $e ) {
            if ( function_exists( 'error_log' ) ) error_log( 'JJ License Security V25 Init Failed: ' . $e->getMessage() );
        }
    }
    ...
    // 5. CSS Optimizer AI 초기화
    if ( class_exists( 'JJ_CSS_Optimizer_AI' ) ) {
        try {
            JJ_CSS_Optimizer_AI::instance();
        } catch ( Exception $e ) {
            if ( function_exists( 'error_log' ) ) error_log( 'JJ CSS Optimizer AI Init Failed: ' . $e->getMessage() );
        }
    }
    ...
    if ( class_exists( 'JJ_Team_Sync' ) ) {
        try {
            JJ_Team_Sync::instance();
        } catch ( Exception $e ) {
            if ( function_exists( 'error_log' ) ) error_log( 'JJ Team Sync Init Failed: ' . $e->getMessage() );
        }
    }

    // 7. [v25.2.0] CSS Variable Extractor 초기화
    if ( class_exists( 'JJ_CSS_Variable_Extractor' ) ) {
        try {
            JJ_CSS_Variable_Extractor::instance();
        } catch ( Exception $e ) {
            if ( function_exists( 'error_log' ) ) error_log( 'JJ CSS Variable Extractor Init Failed: ' . $e->getMessage() );
        }
    }
}, 10 );
