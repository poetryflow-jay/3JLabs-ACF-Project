...
class EditionConfig:
    ...
    # 빌드 매트릭스 (생성할 조합)
    # [Phase 19.1] ACF CSS Manager 전용 (다른 패밀리 플러그인은 플러그인별 build_matrix 사용)
    # 주의: 파트너는 Premium, Unlimited만 존재
    BUILD_MATRIX = [
        # (에디션, 사용자타입)
        ('free', 'standard'),
        ('basic', 'standard'),
        ('premium', 'standard'),
        ('unlimited', 'standard'),
        ('premium', 'partner'),  # 파트너는 Premium, Unlimited만
        ('unlimited', 'partner'),
    ...
    # [Phase 19.1] 마스터 전용 에디션 정의 (에디션명이 'master'인 경우)
    MASTER_EDITION = {
        'name': 'Master',
        'display_name': 'Master (올인원)',
        'license_type': 'MASTER',
        'features': ['all_features', 'experimental', 'dev_tools', 'all_in_one'],
...
class EditionBuilder:
    ...
    # [Phase 19.1] 에디션 구조 단순화:
    # - ACF CSS Manager: 모든 에디션 지원 (free, basic, premium, unlimited × standard, partner, master)
    # - 다른 패밀리 플러그인: free, premium만 (standard), partner (premium/unlimited), master만
    EDITION_PLUGINS = {
        'acf-css-manager': {
            ...
            'build_matrix': None,  # None이면 전체 BUILD_MATRIX 사용
        },
        'admin-menu-editor-pro': {
            ...
            'build_matrix': [('free', 'standard'), ('premium', 'standard'), ('premium', 'partner'), ('unlimited', 'partner'), ('master', 'master')],
        },
        ...
        'acf-css-neural-link': {
            ...
            'build_matrix': [('master', 'master')],  # 마스터 전용
        },
        'acf-nudge-flow': {
        ...
        'acf-css-woo-license': {
            ...
            'license_constant': 'ACF_CSS_WOO_LICENSE_LICENSE_TYPE',
            'edition_constant': 'ACF_CSS_WOO_LICENSE_EDITION',
            'user_type_constant': 'ACF_CSS_WOO_LICENSE_USER_TYPE',
            'display_name': 'ACF CSS WooCommerce License Bridge',
            'description': 'WooCommerce License & Neural Link Integration',
            'build_matrix': [('master', 'master')],  # 마스터 전용
        },
    }
    ...
    def build_all_editions(self, version: str) -> List[Path]:
        ...
        try:
            ...
            self.log("🧹 dist 폴더 자동 정리 중...")
            cleanup_dist_folder.cleanup_dist_folder()
            self.log("✅ 자동 정리 완료")
        except Exception as e:
            self.log(f"⚠️ 자동 정리 실패 (무시 가능): {e}")
    ...
    def build_plugin_edition(self, plugin_key: str, edition: str, user_type: str, 
        ...
        # 마스터 에디션 처리
        if edition == 'master':
            edition_config = EditionConfig.MASTER_EDITION
        else:
            edition_config = EditionConfig.EDITIONS.get(edition)
    ...
    def _inject_edition_constants(self, main_file: Path, plugin_config: dict,
        ...
        [Phase 19.1] 패밀리 플러그인 라이센스 타입 단순화:
        - ACF CSS Manager: 기존대로 edition_config['license_type'] 사용
        - 다른 패밀리 플러그인:
          * user_type == 'master' → 'MASTER'
          * edition in ['premium', 'unlimited', 'partner'] → 'PREMIUM'
          * 그 외 (free, basic) → 'FREE'
        """
        with open(main_file, 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        # [Phase 19.1] ACF CSS Manager가 아닌 다른 패밀리 플러그인에 대한 라이센스 타입 단순화
        if plugin_config['source_dir'] != 'acf-css-really-simple-style-management-center-master':
            if user_type == 'master':
                actual_license_type = 'MASTER'
            elif edition in ['premium', 'unlimited'] or (edition == 'premium' and user_type == 'partner'):
                # 파트너는 프리미엄/언리미티드에만 존재하므로 PREMIUM으로 처리
                actual_license_type = 'PREMIUM'
            else:  # free, basic
                actual_license_type = 'FREE'
        else:
            # ACF CSS Manager는 기존대로 edition_config의 license_type 사용
            actual_license_type = edition_config['license_type']
        
        if license_const in content:
            content = re.sub(
                rf"define\(\s*'{license_const}',\s*'[^']+'\s*\);",
                f"define( '{license_const}', '{actual_license_type}' );",
                content
            )
        else:
            # 상수가 없으면 버전 상수 다음에 추가
            insert_text = f"\ndefine( '{license_const}', '{actual_license_type}' );"
    ...
    def build_all_plugins_all_editions(self, versions: Dict[str, str]) -> List[Path]:
        ...
        [Phase 19.1] 에디션 구조 단순화:
        - ACF CSS Manager: 모든 에디션 지원
        - 다른 패밀리 플러그인: free/premium (standard), partner (premium/unlimited), master만
        - Neural Link, WooCommerce License Bridge: master만
        """
        results = []
        
        # 마스터 전용 플러그인 (에디션별 빌드 불필요)
        master_only_plugins = {'acf-css-neural-link', 'acf-css-woo-license'}
        
        for plugin_key, plugin_config in self.EDITION_PLUGINS.items():
            # 마스터 전용 플러그인은 별도 처리
            if plugin_key in master_only_plugins:
                self.log(f"⏭️ [{plugin_key}] 마스터 전용 플러그인 - 에디션별 빌드 건너뜀")
                continue
            
            # 플러그인별 빌드 매트릭스 가져오기
            build_matrix = plugin_config.get('build_matrix')
            if build_matrix is None:
                # ACF CSS Manager: 전체 BUILD_MATRIX 사용
                build_matrix = EditionConfig.BUILD_MATRIX
            else:
                # 다른 패밀리 플러그인: 단순화된 매트릭스 사용
                # build_matrix는 ('edition', 'user_type') 튜플 리스트
                pass
            
            for edition, user_type in build_matrix:
                try:
