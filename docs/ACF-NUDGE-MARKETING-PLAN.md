...
*작성일: 2026-01-03 (v22.1.0 업데이트)*
*작성자: Jason (CTO) @ 3J Labs*

---

## 🎁 전략적 프리셋 (Strategic Presets) - v22.1.0 구현 완료

개인화 마케팅 도입 타당성 검토 보고서를 기반으로 한 6종의 고성능 마케팅 시나리오가 프리셋으로 주입되었습니다.

1. **첫 방문자 환영 & 큐레이션**
   - 트리거: `first_visit`
   - 액션: `popup_center` (브랜드 베스트셀러 안내)
2. **회원 가입 유도 혜택 알림**
   - 트리거: `visit_count >= 2`
   - 액션: `popup_slide_in` (10% 할인 쿠폰 제안)
3. **장바구니 이탈 방지 & 리뷰 넛지**
   - 트리거: `exit_intent` (장바구니 페이지 등)
   - 액션: `toast` (품절 임박 알림 및 높은 평점 강조)
4. **무료 배송 임계치 달성 유도**
   - 트리거: `cart_value < 50,000`
   - 액션: `free_shipping_bar` (남은 금액 실시간 표시)
5. **관련 상품 스마트 교차 판매**
   - 트리거: `cart_value >= 10,000` (상품 관심도 기반)
   - 액션: `crosssell_popup` (함께 구매하면 좋은 상품 추천)
6. **VIP 고객 자동 리텐션 팩**
   - 트리거: `total_spent >= 500,000`
   - 액션: `discount_reveal` (VIP 전용 20% 비밀 쿠폰)
