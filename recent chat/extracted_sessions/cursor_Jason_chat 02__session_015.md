...
좋아. 나의 플러그인 테스트와 리뷰에는 시간이 다소 걸리니, 그 동안 제이슨은 우선 다음 계획을 A부터 D까지 실행에 옮겨서 바로 진행하는 걸로 하지. 자네는 나에게 설치와 검증에 필요한 체크리스트를 준 다음, 바로 작업에 착수를 부탁하네.
...
좋습니다. 먼저 대표님 테스트용 **설치/검증 체크리스트**를 드리고, 바로 이어서 제가 **A~D(운영/안정성/자가진단/UX)**를 실제 코드로 확장 구현한 뒤 빌드/번들 폴더까지 다시 만들어 두겠습니다.
...
### 설치/검증 체크리스트 (대표님 테스트용, 10~20분 코스)

## 0) 테스트 번들 폴더(최신)
- **`C:\Users\computer\Desktop\JJ_TestBundle_v8.0.0_20251231-202356`**
  - `Staging / Beta / Stable → Master / Partner / Pro / Free` 구조
  - 각 폴더 안에 **Core ZIP + Add-ons/** 포함

## 1) 설치 순서(권장)
- **1) Core 먼저 설치/활성화**
  - 예: `Stable\Master\ACF-CSS-Master-v8.0.0.zip` (또는 Staging/Beta로 테스트)
- **2) Add-ons 설치/활성화(필요한 것만)**
  - Master/Partner 폴더의 `Add-ons\`는 **Master Add-ons**라서 업그레이드/제한 UI가 나오면 안 됩니다.
- **3) Woo License 테스트 시**
  - **WooCommerce를 먼저 설치/활성화**한 뒤 Woo License Add-on 활성화

## 2) 핵심 검증 포인트(운영/UX)
- **Admin Center → Updates 탭**
  - 상단 **통합 테이블(코어+애드온)** 표시 확인
  - **검색/필터(미설치 숨기기/업데이트만)** 동작 확인
  - **“전체 업데이트 다시 체크”** 버튼 → 동작 후 화면 리로드/업데이트 반영 확인
  - **“보이는 항목 Auto-Update ON/OFF”** 일괄 적용 확인(토글 배지 변화 + WP 플러그인 목록에서도 반영)
  - 각 플러그인 행:
    - **활성화/비활성화 버튼** 동작(WordPress 기본 동작)
    - **업데이트 가능 배지 + ‘지금 업데이트’ 링크** 노출(업데이트가 있을 때만)
    - Woo License 행의 **WooCommerce 상태 배지(MISSING/INACTIVE/ACTIVE)** 확인

- **Admin Center → System Status 탭**
  - **자가 진단 실행** → PASS/WARN/FAIL/SKIP 출력
  - FAIL이 있으면 **누락 파일/로드 오류가 “파일 단위로” 표시**되는지 확인
  - **결과 복사 / JSON 다운로드** 버튼 동작 확인(QA 공유용)

- **Style Center → 추천 팔레트**
  - 프리셋 적용 후 **“되돌리기(Undo)”** 버튼으로 적용 전 상태 복구 확인

- **AI Extension(WebGPU)**
  - 스트리밍/재시도/JSON 재파싱, 팔레트 저장/프리셋 반영 흐름(기존 체크)
