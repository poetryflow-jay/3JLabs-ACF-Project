...
if ( ! acf_get_setting( 'pro' ) ) {
	$acf_more_items[] = array(
		...
		'text'   => __( 'Options Pages', 'advanced-members' ) . '<span class="acf-requires-pro">' . __( 'PRO', 'advanced-members' ) . '</span>',
		'target' => '_self',
	);
}
...
if ( ! defined( 'PWP_NAME' ) ) {
	...
	$wpengine_more_items[] = array(
		...
		'text'     => $acf_wpengine_logo . '<span class="acf-wp-engine-upsell-pill">' . __( '4 Months Free', 'advanced-members' ) . '</span>',
		'target'   => '_blank',
	...
	<div class="acf-admin-toolbar-inner">
		<div class="acf-nav-wrap">
			<a href="<?php echo admin_url( 'edit.php?post_type=acf-field-group' ); ?>" class="acf-logo">
				<img src="<?php echo acf_get_url( 'assets/images/acf-logo.svg' ); ?>" alt="<?php esc_attr_e( 'Advanced Custom Fields logo', 'advanced-members' ); ?>">
			...
			<?php if ( $acf_more_items || $more_items ) { ?>
				<div class="acf-more acf-header-tab-acf-more" tabindex="0">
					<span class="acf-tab acf-more-tab"><i class="acf-icon acf-icon-more"></i><?php esc_html_e( 'More', 'advanced-members' ); ?> <i class="acf-icon acf-icon-dropdown"></i></span>
					<ul>
						<?php
						if ( $acf_more_items ) {
							if ( $more_items ) {
						...
						if ( $more_items ) {
							echo '<li class="acf-more-section-header"><span class="acf-tab acf-tab-header">' . esc_html__( 'Other', 'advanced-members' ) . ' </span></li>';
		...
		<div class="acf-nav-upgrade-wrap">
			...
			if ( ! acf_is_pro() || ! acf_pro_is_license_active() ) {
				...
				$unlock_text   = __( 'Unlock Extra Features with ACF PRO', 'advanced-members' );

				if ( acf_is_pro() ) {
					if ( function_exists('acf_is_updates_page_visible') && acf_is_updates_page_visible() ) {
					...
					if ( acf_pro_is_license_expired() ) {
						...
						$unlock_target = '_blank';
						$unlock_text   = __( 'Renew ACF PRO License', 'advanced-members' );
					}
				}
				?>
			...
			?>
			<a href="<?php echo esc_url($acf_wpengine_logo_link); ?>" target="_blank" class="acf-nav-wpengine-logo">
				<img src="<?php echo esc_url( acf_get_url( 'assets/images/wp-engine-horizontal-white.svg' ) ); ?>" alt="<?php esc_html_e( 'WP Engine logo', 'advanced-members' ); ?>" />
			</a>
		</div>
	</div>
</div>
...
if ( $screen->id != 'amem-form' ) {
	if ( $plugin_page == 'acf-tools' ) {
		$acf_page_title = __( 'Tools', 'advanced-members' );
	} elseif ( $plugin_page == 'acf-settings-updates' ) {
		$acf_page_title = __( 'Updates', 'advanced-members' );
	} elseif ( $plugin_page == 'acf_options_preview' && ! acf_is_pro() ) {
		$acf_page_title = __( 'Options Pages', 'advanced-members' );
	}
