...
(function($) {
    ...
    setup_form: function( $form ) {
      ...
      form.$el.on('submit', function( e ) {
        ...
        acf.validation.fetch({
          ...
          complete: function() {
            form.$el.find( '.amem-submit-button' ).prop( 'disabled', false );
          }
        });
      });

      $(document).on( 'click', '.amem-notice.-dismiss a.acf-notice-dismiss ', function(e) {
        e.preventDefault();
        $(this).closest('.amem-notice.-dismiss').remove();
      } );

      acf.doAction( 'amem/form/setup', form );
    },

    lock( form ) {
      // Disable button to avoid duplicate submissions
      form.$el.find( '.amem-submit-button' ).prop( 'disabled', true );
  ...
  amem.pages = {
    initialize: function( form ) {
      ...
      if ( $page_fields.exists() ) {
        ...
        form.$page_wrap = $( '<div class="amem-page-wrap">' );
        form.$page_wrap.insertBefore( $page_fields.first() );

        // Create next/previous buttons
        form.$previous_button = $page_fields.first().find( '.amem-previous-button' );
        form.$next_button = $page_fields.first().find( '.amem-next-button' );

        // Read show numbering setting
        form.show_numbering = 'true' === $page_fields.first().find( '.amem-page-button' ).attr( 'data-show-numbering' );
        ...
        var submit_wrap = form.$el.find( '.amem-submit' );
        submit_wrap.prepend( form.$next_button );
        submit_wrap.prepend( form.$previous_button );
        form.$submit_button = submit_wrap.find( '.amem-submit-button' );
        ...
        $page_fields.each(function(_, el) {
          ...
          var $page_button = $page_field.find( '.amem-page-button' ).attr( 'data-index', index );
          $page_button.click(function(e) {
            e.preventDefault();
            amem.pages.navigateToPage( index, form );
          });
    ...
    changePage: function( page, form ) {
      ...
      acf.doAction( 'amem/form/page_changed', page, previousPage, form );
    },
  ...
  amem.ajax = {
    initialize: function( form ) {
      ...
      amem.addSubmissionStep(form, 100, function( callback ) {
    ...
    sendSubmission: function( form ) {
      ...
      formData.append( 'action', 'amem_submission' );

      // Send AJAX request with action "amem_submission"
      $.ajax({
        url: acf.get( 'ajaxurl' ),
        data: formData,
        processData: false,
    ...
    onSuccess: function( form ) {
      return function( resp ) {
        ...
        acf.doAction( 'amem/form/ajax/submission', data, form );
        switch ( data.type ) {
          case 'success_message':
            // Replace form fields with the success message
            var $success_message = $( data.success_message );
            // remove past messages
            form.$el.find('.amem-success-message, .amem-error-message, .amem-updated-message').remove();
            // reset password fields
            form.$el.find('input[name="acf[user_password]"], input[name="acf[user_password_confirm]"], input[name="acf[user_password_current]"], input[name="user_password"], input[name="user_password_confirm"]').val('');
            form.$el.find('.pass-strength-result, .pass-strength-result').html('');
            // show success message
            form.$el.prepend( $success_message );
            break;
          /** @todo Add to form settings and apply, maybe account delete, registration... etc */
          case 'just_success_message':
            // Replace form fields with the success message
            var $success_message = $( data.success_message );
            var $fields = form.$el.find( '.amem-fields' );
  ...
  $(document).ready(function() {
    $( '.amem-form' ).each(function() {
      amem.setup_form( $(this) );
    });
  });
