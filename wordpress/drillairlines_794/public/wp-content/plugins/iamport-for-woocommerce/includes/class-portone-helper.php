...
if ( ! class_exists( 'Portone_Helper' ) ) {

	class Portone_Helper {
		...
		public static function get_customer_uid( $order ) {
			...
			if ( empty( $prefix ) ) {
				...
				if ( ! add_option( '_iamport_customer_prefix', $prefix ) ) {
					throw new Exception( esc_html( __( "정기결제 구매자정보 생성에 실패하였습니다.", 'iamport-for-woocommerce' ) ), 1 );
				}
			}

			$user_id = $order->get_user_id(); // wp_cron에서는 get_current_user_id()가 없다.
			if ( empty( $user_id ) ) {
				throw new Exception( esc_html( __( "정기결제기능은 로그인된 사용자만 사용하실 수 있습니다.", 'iamport-for-woocommerce' ) ), 1 );
			}
		...
		public static function has_excluded_product( $order_id, $product_list, $category_list ) {
			if ( empty( $product_list ) && empty( $category_list ) ) {
				return false;
			}
			...
			foreach ( $items as $it ) {
				...
				if ( $product instanceof WC_Product ) {
					...
					$is_category_included     = Portone_Helper::is_product_in_categories( $product_id, $category_list );

					if ( ! ( $is_subscription_included || $is_product_included || $is_category_included ) ) {
						return true;
					}
				}
			}
		...
		public static function get_all_products( $search = null ) {
			...
			$allProducts = wc_get_products( $condition );

			$productNames = array();
			foreach ( $allProducts as $p ) {
				$productNames[ $p->get_id() ] = $p->get_name();
			}
		...
		public static function get_notice_url() {
			return add_query_arg( array( 'wc-api' => WC_Gateway_Portone_Vbank::class ), site_url() );
		}
		...
		public static function get_max_card_quota() {
		...
		public static function htmlSecondaryPaymentMethod( $settingsText ) {
			...
			ob_start(); ?>
            <select class="iamport_payment_method_secondary" style="display :<?php echo esc_html( $uniquePG ? "none" : "" ); ?>">
				<?php foreach ( $pgOptions as $key => $label ) : ?>
                    <option value="<?php echo esc_html( $key ); ?>" <?php echo esc_html( $uniquePG ? "selected" : "" ); ?>><?php echo esc_html( $label ); ?></option>
				<?php endforeach; ?>
            </select>
			<?php
			return ob_get_clean();
