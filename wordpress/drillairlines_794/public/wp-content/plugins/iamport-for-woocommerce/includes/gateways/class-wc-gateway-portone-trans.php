...
class WC_Gateway_Portone_Trans extends PORTONE_Payment_Gateway {

	const GATEWAY_ID = 'iamport_trans';
	protected $escrow;

	public function __construct() {
		parent::__construct();

		$this->method_title       = __( '포트원(실시간계좌이체)', 'iamport-for-woocommerce' );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array_merge( array(
			'enabled'     => array(
				...
				'type'    => 'checkbox',
				'label'   => __( '포트원(실시간계좌이체) 결제 사용', 'iamport-for-woocommerce' ),
				'default' => 'yes'
			),
			'title'       => array(
			...
			'_pg_auto_title'   => array(
				'title'       => __( '(1) 계좌이체 결제 시, 자동 적용될 PG설정값 세팅', 'iamport-for-woocommerce' ),
				'type'        => 'title',
			...
			'_pg_manual_title' => array(
				'title'       => __( '(2) 계좌이체 결제 시, 적용될 PG설정값을 고객이 직접 지정', 'iamport-for-woocommerce' ),
				'type'        => 'title',
				'description' => '체크아웃(Checkout)페이지에서 구매자가 계좌이체 결제수단 선택 후 세부 결제수단을 한 번 더 선택할 수 있습니다. 아래 기능을 사용하면 위에 설정된 [계좌이체 결제 시, 자동 적용될 PG설정값 세팅] 값은 모두 무시됩니다.',
			),
			'use_manual_pg'    => array(
				'title'       => __( 'PG설정 구매자 선택방식 사용', 'iamport-for-woocommerce' ),
				'type'        => 'checkbox',
				'description' => __( '포트원 계정에 설정된 여러 PG사 / MID를 사용자의 선택에 따라 적용하는 기능을 활성화합니다. 계좌이체 결제수단 선택 시, 세부 결제수단 선택창이 추가로 출력됩니다.', 'iamport-for-woocommerce' ),
				'default'     => 'no',
			),
			'manual_pg_id'     => array(
	...
	public function iamport_order_detail( $order_id ) {
        ...
        <table class="shop_table order_details">
            ...
            <tr>
                ...
                <td><?php esc_html_e( '실시간계좌이체', 'iamport-for-woocommerce' ) ?></td>
            </tr>
            <tr>
