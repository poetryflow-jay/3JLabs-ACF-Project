	...
	static function get_shop_order_screen( $order_types = 'shop_order' ) {
		...
		if ( is_array( $order_types ) ) {
			foreach ( $order_types as $order_type ) {
				$screens[] = self::enabled() ? wc_get_page_screen_id( str_replace( '_', '-', $order_type ) ) : $order_type;
			}
		} else {
			$screens[] = self::enabled() ? wc_get_page_screen_id( str_replace( '_', '-', $order_types ) ) : $order_types;
		}

		return $screens;
	}
	static function is_order( $order_id ) {
		return self::enabled() ? OrderUtil::is_order( $order_id, wc_get_order_types() ) : in_array( get_post_type( $order_id ), wc_get_order_types() );
	}
