...
class IamportSettingTab {
	...
	public static function init() {
		//포트원 Tab설정 추가
		add_filter( 'woocommerce_settings_tabs_array', array( __CLASS__, 'label', ), 50, 1 );
		add_action( 'woocommerce_settings_tabs_iamport', array( __CLASS__, 'settings' ) );
		add_action( 'woocommerce_update_options_iamport', array( __CLASS__, 'save' ) );
		// add_action( 'woocommerce_sections_' . 'iamport', array($this, 'tab') );
	}

	static function label( $settings_tabs ) {
		$settings_tabs[ 'iamport' ] = __( '포트원', 'iamport-for-woocommerce' );
	...
	static function getSettings() {
		$custom_statuses = array_merge( array( 'none' => '사용안함' ), Portone_Helper::getCustomStatuses() );
		$version_list    = array(
			'1.1.7/latest' => '1.1.7',
			'1.1.8/latest' => '1.1.8',
			'1.2.0/latest' => '1.2.0',
		...
		$settings = array(
			array(
				'name' => __( '포트원 공통 결제 설정', 'iamport-for-woocommerce' ),
				'type' => 'title',
				'desc' => '',
				'id'   => 'wc_settings_tab_iamport_section_title'
			),
			array(
				'title'   => __( '포트원 자바스크립트 SDK 버전', 'iamport-for-woocommerce' ),
				'desc'    => __( '포트원 모듈의 버전을 설정해주세요.', 'iamport-for-woocommerce' ),
				'id'      => 'woocommerce_iamport_sdk_version',
				'default' => '1.2.0/latest',
				'type'    => 'select',
			...
			array(
				...
				'desc'    => __( '결제된 주문이 "완료됨" 상태일 때에도 구매자가 교환요청을 할 수 있도록 버튼을 생성합니다.', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '결제된 주문이 "완료됨" 상태일 때에도 구매자가 환불요청을 할 수 있도록 버튼을 생성합니다.', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '구매자 교환요청버튼을 몇 일간 노출할지 결정합니다. (구매 시점 기준으로 기한이 지나면 교환 불가). 빈 값이거나 0이면 기한을 설정하지 않습니다.', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '구매자 환불요청버튼을 몇 일간 노출할지 결정합니다. (구매 시점 기준으로 기한이 지나면 환불 불가). 빈 값이거나 0이면 기한을 설정하지 않습니다.', 'iamport-for-woocommerce' ),
...
//			array(
//				'title'		=> __( '포트원 웹훅(Notification) 통지 URL', 'iamport-for-woocommerce' ),
//				'desc'		=> sprintf( __( '가상계좌 입금통지, 거래정보 동기화 등을 위해 포트원 웹훅을 통지받을 URL을 지정할 수 있습니다.<br>(예시 : %s)' ), home_url() . '?wc-api=WC_Gateway_Iamport_Vbank' ),
//				'id'		=> 'woocommerce_iamport_notice_url',
//				'default'	=> '',
//				'type'		=> 'url'
//			),
			array(
				'title'   => __( '포트원 결제주소 처리', 'iamport-for-woocommerce' ),
			...
			array(
				'name' => __( '포트원 정기 결제 설정', 'iamport-for-woocommerce' ),
				'type' => 'title',
				'desc' => '',
			...
			array(
				...
				'desc'    => __( '0보다 큰 숫자인 경우에만 테스트 결제를 수행합니다.<br>정기결제 카드정보 등록 시, 카드정보의 유효성 검사는 이뤄지지만 결제가 가능한 카드인지 판별하기 어려운 예외적인 경우가 있습니다.(ex. 분실정지된 카드, 한도초과 카드)<br>카드정보 확인과 동시에 테스트 결제를 진행해봄으로써 분실정지되거나 한도초과된 카드의 등록을 사전에 방지할 수 있습니다.<br>테스트결제는 결제 후 3~4초 후에 자동 취소됩니다.', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '정기결제 최초결제 및 KEY-IN결제 시 결제금액이 5만원이상인 경우 할부 개월 선택이 가능합니다. 카드사마다 무이자 할부 제공기준이 모두 달라, 별도의 문구를 통해 고객에게 안내가 필요합니다.', 'iamport-for-woocommerce' ),
			...
			array(
				'name' => __( '포트원 기타 설정', 'iamport-for-woocommerce' ),
				'type' => 'title',
				'desc' => '',
			...
			array(
				...
				'desc'    => __( '5만원 이상 거래건에 대해, PG사 결제창 내 할부선택 옵션이 나타나게 되는데 사용자가 선택가능한 최대 할부개월 수를 제한하는 기능입니다.', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '가상계좌 발급 후 아직 입금되지 않은 주문 건에 대한 상태값 명칭을 변경할 수 있습니다. (기본값 : 가상계좌 입금대기 중)', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '반품요청된 주문 건에 대한 상태값 명칭을 변경할 수 있습니다. (기본값 : 반품요청)', 'iamport-for-woocommerce' ),
			...
			array(
				...
				'desc'    => __( '교환요청된 주문 건에 대한 상태값 명칭을 변경할 수 있습니다. (기본값 : 교환요청)', 'iamport-for-woocommerce' ),
				'id'      => 'woocommerce_iamport_exchange_status_label',
				'default' => '교환요청',
				'type'    => 'text'
			),
			array(
				'title'   => __( '앱 스킴명 설정', 'iamport-for-woocommerce' ),
				'desc'    => __( '앱 이용 시, 스킴명이 필요한 경우 스킴명을 입력합니다.', 'iamport-for-woocommerce' ),
				'id'      => 'woocommerce_iamport_app_scheme',
				'default' => '',
				'type'    => 'text'
			),
			array(
