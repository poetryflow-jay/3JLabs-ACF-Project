=== 우커머스 포트원 플러그인 (국내 모든 PG를 한 번에) ===
Contributors: portone
Donate link: https://admin.portone.io/
Tags: 포트원우커머스, 우커머스결제연동, 포트원워드프레스, PortOne, 아임포트우커머스
...
== Description ==
포트원 서비스를 우커머스(woocommerce)환경에 맞게 적용한 결제 플러그인입니다.
포트원(구 아임포트)은 국내 PG 서비스들을 표준화하고 있는 결제 서비스로
포트원 하나면 국내 여러 PG사들의 결제 기능을 표준화된 동일한 방식으로 사용할 수 있습니다.

[결제수단]
신용카드, 실시간계좌이체, 가상계좌, 휴대폰소액결제, 정기결제 (우커머스 서브스크립션), 간편결제, 해외결제를 지원합니다.

[결제대행사]
토스페이먼츠, 이니시스, 나이스페이먼츠, 다날, 헥토파이낸셜, 모빌리언스,  KCP, KSNET, KICC, 스마트로, 키움페이, 웰컴페이먼츠, 페이코, 토스페이, 카카오페이, 엑심베이, 페이먼트월 등

http://www.portone.io 에서 포트원 서비스에 대한 보다 상세한 내용을 확인하실 수 있습니다.

[설치 안내]
포트원 관리자 페이지( https://admin.portone.io ) 에서 관리자 회원가입을 합니다.
우커머스용 포트원 플러그인을 워드프레스에 설치합니다.
포트원 콘솔 > 결제연동 > 내 식별코드에서 "가맹점 식별코드", "REST API키", "REST API secret"을 플러그인 설정에 저장합니다.

[액션 훅]
iamport_order_status_changed : 포트원에 의해 우커머스 주문 상태가 변경되었을 때 호출($old_status, $new_status, $order) 3개의 파라미터 제공
iamport_simple_order_name : 일반 상품 주문시 적용되는 상품명 filter($order_name, $order) 2개의 파라미터 제공
iamport_recurring_order_name : 정기결제 상품 주문시 적용되는 상품명 filter($order_name, $order, $isInitial) 3개의 파라미터 제공

[External services]
포트원 결제 서비스를 이용하기 위해 아래 도메인을 통해 제공되는 결제 모듈을 이용합니다.
...
1. 포트원 관리자 페이지( https://admin.portone.io ) 에서 관리자 회원가입을 합니다.
2. 우커머스용 포트원 플러그인을 워드프레스에 설치합니다.
3. 포트원 콘솔 > 결제연동 > 내 식별코드에서 "가맹점 식별코드", "REST API키", "REST API secret"을 플러그인 설정에 저장합니다.
...
= 포트원 DOCS =
https://github.com/iamport/wordpress-iamport-for-woocommerce

= 고객센터 =
1670-5176 / wordpress@portone.io

== Screenshots ==
1. 포트원 관리자 로그인 후 "시스템 설정" 페이지에서 "가맹점 식별코드", "REST API키", "REST API secret" 정보를 확인합니다.
...
= 3.3.10 =
* PG설정 구매자 선택방식 사용 시 PG사 설정 HTML에 Escape가 적용되는 현상 수정

= 3.3.9 =
* 정기결제 - 결제창 방식을 이용할때 주문에 고객명이 누락된 경우 잘못된 고객명이 전달되거나 "구매자"로 이름이 전달되는 현상

= 3.3.8 =
* 우커머스 상품 카테고리 페이지에서 네이버상품 카테고리 표시 오류 수정

= 3.3.7 =
* 정기결제 오류 수정

= 3.3.5 =
* 최대 할부개월수 지정 시 일시불 선택이 안되는 현상 수정

== Changelog ==
= 3.3.4 =
* 포트원 설정값 확인을 위한 포트원 관리자 화면 경로 최신화

= 3.3.3 =
* (신)나이스페이 결제채널 추가
* (신)토스페이먼츠 결제채널에 가상계좌, 퀵계좌이체 결제수단 추가

= 3.3.1 =
* 구 버전의 웹훅 설정 영향 추가 제거

= 3.3.0 =
* 구 버전의 웹훅 설정 영향 제거

= 3.2.12 =
* 허브형 네이버페이 PG사 목록에 스마트로 추가

= 3.2.11 =
* 허브형 네이버페이 PG사 목록에 (신)토스페이먼츠 추가

= 3.2.10 =
* 누락된 가상계좌 환불 관련 파일 추가

= 3.2.9 =
* 엑심베이 세부 결제수단 선택 기능 추가
...
= 3.2.6 =
* 결제창 방식 정기결제 수단 PG사 목록에 토스페이먼츠 추가

= 3.2.5 =
* 플러그인 보안 업데이트
* 엑심베이 결제수단 내 결제 방법 선택이 제대로 적용되지 않던 현상 수정

= 3.2.4 =
* 키인 정기결제수단에 PG사 목록 및 수동입력 옵션 추가

= 3.2.3 =
* PHP 8.2 버전에서의 Deprecated 수정
* 정기결제 결제창 방식 PG사에 스마트로 추가

= 3.2.2 =
* 엑심베이, 페이먼트월, 삼성페이 결제수단에 대해 MID 지정 기능 추가
* PHP Warning 수정

= 3.2.1 =
* 신용카드, 실시간 계좌이체, 가상계좌 결제수단의 기본 PG사에 스마트로 추가

= 3.2.0 =
* 허브형 네이버페이 지원
* 결제형 네이버페이를 사용하지 않을 때, 숨김처리 진행(허브형 권장)
* PHP Warning 수정
...
= 3.1.2 =
* 모바일에서 이니시스 정기결제를 진행할 때, 결제 금액이 정확하지 않던 현상 수정

= 3.1.1 =
* 플러그인 내 존재하지 않는 payment.css 파일을 호출하던 현상 수정
...
= 3.0.6 =
* 삼성페이 결제 지원
* PHP 7.4 버전에서 사용 가능하도록 변경

= 3.0.5 =
* 관리자 페이지의 상품 카테고리 영역이 보이지 않던 현상 수정

= 3.0.4 =
* 카카오페이 정기결제 - 갱신결제 요청이 중복 호출되는 현상 수정

= 3.0.3 =
* 설정화면에서 상품목록을 표시할때 PHP Warning 메시지 제거

= 3.0.2 =
* 결제창 언어에 일본어, 중국어 추가

= 3.0.1 =
* 미지원 결제수단 삭제 및 안내문구 업데이트

= 3.0.0 =
* 우커머스 HPOS (High Performance Order Storage) 지원
* 플러그인 소스 리팩토링

= 2.2.37 =
* 지원가능 결제대행사(PG)추가 - 웰컴페이먼츠, 헥토파이낸셜, KSNET
...
= 2.2.34 =
* 카드결제 신토스 추가, JS SDK 1.3.0 선택 가능하도록 추가
