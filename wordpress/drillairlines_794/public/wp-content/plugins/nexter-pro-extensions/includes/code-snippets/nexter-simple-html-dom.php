 ...
 *
 * Modified for Nexter Extension by removing the functionality that is not needed in the plugin context.
 * Also added prefixes to the class/function names to avoid conflicts with other plugins.
 */
...
define( 'NXTEXT_HDOM_TYPE_ELEMENT', 1 );
define( 'NXTEXT_HDOM_TYPE_COMMENT', 2 );
define( 'NXTEXT_HDOM_TYPE_TEXT', 3 );
define( 'NXTEXT_HDOM_TYPE_ENDTAG', 4 );
define( 'NXTEXT_HDOM_TYPE_ROOT', 5 );
define( 'NXTEXT_HDOM_TYPE_UNKNOWN', 6 );
define( 'NXTEXT_HDOM_QUOTE_DOUBLE', 0 );
define( 'NXTEXT_HDOM_QUOTE_SINGLE', 1 );
define( 'NXTEXT_HDOM_QUOTE_NO', 3 );
define( 'NXTEXT_HDOM_INFO_BEGIN', 0 );
define( 'NXTEXT_HDOM_INFO_END', 1 );
define( 'NXTEXT_HDOM_INFO_QUOTE', 2 );
define( 'NXTEXT_HDOM_INFO_SPACE', 3 );
define( 'NXTEXT_HDOM_INFO_TEXT', 4 );
define( 'NXTEXT_HDOM_INFO_INNER', 5 );
define( 'NXTEXT_HDOM_INFO_OUTER', 6 );
define( 'NXTEXT_HDOM_INFO_ENDSPACE', 7 );

defined( 'NXTEXT_DEFAULT_TARGET_CHARSET' ) || define( 'NXTEXT_DEFAULT_TARGET_CHARSET', 'UTF-8' );
defined( 'NXTEXT_DEFAULT_BR_TEXT' ) || define( 'NXTEXT_DEFAULT_BR_TEXT', "\r\n" );
defined( 'NXTEXT_DEFAULT_SPAN_TEXT' ) || define( 'NXTEXT_DEFAULT_SPAN_TEXT', ' ' );
defined( 'NXTEXT_MAX_FILE_SIZE' ) || define( 'NXTEXT_MAX_FILE_SIZE', 100000000 );
define( 'NXTEXT_HDOM_SMARTY_AS_TEXT', 1 );

/**
 * Prefixed version of the Simple HTML DOM str_get_html.
 * See: http://simplehtmldom.sourceforge.net/manual.htm
 *
 * @param string $str The HTML document string.
 * @param bool   $lowercase Tag names are parsed in lowercase letters if enabled.
 * @param bool   $force_tags_closed Tags inside block tags are forcefully closed if the closing tag was omitted.
 * @param string $target_charset Defines the target charset for text returned by the parser.
 * @param bool   $strip_r_n Newline characters are replaced by whitespace if enabled.
 * @param string $default_br_text Defines the default text to return for <br> elements.
 * @param string $default_span_text Defines the default text to return for <span> elements.
 *
 * @return NXTEXT_Simple_HTML_DOM|false
 */
function nxtext_str_get_html1(
	$str,
	$lowercase = false,
	$force_tags_closed = false,
	$target_charset = NXTEXT_DEFAULT_TARGET_CHARSET,
	$strip_r_n = false,
	$default_br_text = NXTEXT_DEFAULT_BR_TEXT,
	$default_span_text = NXTEXT_DEFAULT_SPAN_TEXT
) {
	$dom = new NXTEXT_Simple_HTML_DOM(
		null,
		$lowercase,
		$force_tags_closed,
		$target_charset,
	...
	if ( empty( $str ) || strlen( $str ) > NXTEXT_MAX_FILE_SIZE ) {
		$dom->clear();

		return false;
	}

	return $dom->load( $str, $lowercase, $strip_r_n );
}

class NXTEXT_Simple_HTML_DOM_Node1 {
	public $nodetype = NXTEXT_HDOM_TYPE_TEXT;
	public $tag = 'text';
	public $attr = array();
	...
	function dump_node( $echo = true ) {
		...
		if ( isset( $node->_[ NXTEXT_HDOM_INFO_INNER ] ) ) {
			$string .= "'" . $node->_[ NXTEXT_HDOM_INFO_INNER ] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext() {
		if ( isset( $this->_[ NXTEXT_HDOM_INFO_INNER ] ) ) {
			return $this->_[ NXTEXT_HDOM_INFO_INNER ];
		}

		if ( isset( $this->_[ NXTEXT_HDOM_INFO_TEXT ] ) ) {
			return $this->dom->restore_noise( $this->_[ NXTEXT_HDOM_INFO_TEXT ] );
		}
	...
	function outertext() {
		...
		if ( isset( $this->_[ NXTEXT_HDOM_INFO_OUTER ] ) ) {
			return $this->_[ NXTEXT_HDOM_INFO_OUTER ];
		}
		...
		if ( $this->dom && $this->dom->nodes[ $this->_[ NXTEXT_HDOM_INFO_BEGIN ] ] ) {
			$ret = $this->dom->nodes[ $this->_[ NXTEXT_HDOM_INFO_BEGIN ] ]->makeup();
		}

		if ( isset( $this->_[ NXTEXT_HDOM_INFO_INNER ] ) ) {
			// todo: <br> should either never have HDOM_INFO_INNER or always
			if ( $this->tag !== 'br' ) {
				$ret .= $this->_[ NXTEXT_HDOM_INFO_INNER ];
			}
		} elseif ( $this->nodes ) {
			foreach ( $this->nodes as $n ) {
		...
		if ( isset( $this->_[ NXTEXT_HDOM_INFO_END ] ) && $this->_[ NXTEXT_HDOM_INFO_END ] != 0 ) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text() {
		...
		switch ( $this->nodetype ) {
			case NXTEXT_HDOM_TYPE_TEXT:
				return $this->dom->restore_noise( $this->_[ NXTEXT_HDOM_INFO_TEXT ] );
			case NXTEXT_HDOM_TYPE_COMMENT:
				return '';
			case NXTEXT_HDOM_TYPE_UNKNOWN:
				return '';
		}
	...
	function makeup() {
		...
		foreach ( $this->attr as $key => $val ) {
			...
			$ret .= $this->_[ NXTEXT_HDOM_INFO_SPACE ][ $i ][0];

			//no value attr: nowrap, checked selected...
			if ( $val === true ) {
				$ret .= $key;
			} else {
				switch ( $this->_[ NXTEXT_HDOM_INFO_QUOTE ][ $i ] ) {
					case NXTEXT_HDOM_QUOTE_DOUBLE:
						$quote = '"';
						break;
					case NXTEXT_HDOM_QUOTE_SINGLE:
						$quote = '\'';
						break;
					default:
						$quote = '';
				...
				$ret .= $key
				        . $this->_[ NXTEXT_HDOM_INFO_SPACE ][ $i ][1]
				        . '='
				        . $this->_[ NXTEXT_HDOM_INFO_SPACE ][ $i ][2]
				        . $quote
				        . $val
				        . $quote;
			}
		...
		return $ret . $this->_[ NXTEXT_HDOM_INFO_ENDSPACE ] . '>';
	}
	...
	function find( $selector, $idx = null, $lowercase = false ) {
		...
		for ( $c = 0; $c < $count; ++ $c ) {
			...
			if ( ! isset( $this->_[ NXTEXT_HDOM_INFO_BEGIN ] ) ) {
				return array();
			}

			$head = array( $this->_[ NXTEXT_HDOM_INFO_BEGIN ] => 1 );
			$cmd  = ' '; // Combinator
			...
			for ( $l = 0; $l < $levle; ++ $l ) {
				...
				foreach ( $head as $k => $v ) {
					...
					// Pass this optional parameter on to the seek function
	...
	protected function seek( $selector, &$ret, $parent_cmd, $lowercase = false ) {
		...
		if ( $parent_cmd === ' ' ) { // Descendant Combinator
			...
			$end = ( ! empty( $this->_[ NXTEXT_HDOM_INFO_END ] ) ) ? $this->_[ NXTEXT_HDOM_INFO_END ] : 0;
			if ( $end == 0 ) {
				$parent = $this->parent;
				while ( ! isset( $parent->_[ NXTEXT_HDOM_INFO_END ] ) && $parent !== null ) {
					$end    -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[ NXTEXT_HDOM_INFO_END ];
			}

			// Get list of target nodes
			$nodes_start = $this->_[ NXTEXT_HDOM_INFO_BEGIN ] + 1;
		...
		foreach ( $nodes as $node ) {
			...
			if ( $pass ) {
				$ret[ $node->_[ NXTEXT_HDOM_INFO_BEGIN ] ] = 1;
			}
			unset( $node );
		}
	...
	protected function parse_selector( $selector_string ) {
		 ...
		 *
		 * Add the colon to the attribute, so that it properly finds
	...
	function __set( $name, $value ) {
		...
		switch ( $name ) {
			case 'outertext':
				return $this->_[ NXTEXT_HDOM_INFO_OUTER ] = $value;
			case 'innertext':
				if ( isset( $this->_[ NXTEXT_HDOM_INFO_TEXT ] ) ) {
					return $this->_[ NXTEXT_HDOM_INFO_TEXT ] = $value;
				}

				return $this->_[ NXTEXT_HDOM_INFO_INNER ] = $value;
		}

		if ( ! isset( $this->attr[ $name ] ) ) {
			$this->_[ NXTEXT_HDOM_INFO_SPACE ][] = array( ' ', '', '' );
			$this->_[ NXTEXT_HDOM_INFO_QUOTE ][] = NXTEXT_HDOM_QUOTE_DOUBLE;
		}
...
class NXTEXT_Simple_HTML_DOM {
	...
	function __construct(
		...
		$target_charset = NXTEXT_DEFAULT_TARGET_CHARSET,
		$stripRN = true,
		$defaultBRText = NXTEXT_DEFAULT_BR_TEXT,
		$defaultSpanText = NXTEXT_DEFAULT_SPAN_TEXT,
		$options = 0
	) {
		if ( $str ) {
	...
	function load(
		...
		if ( $options & NXTEXT_HDOM_SMARTY_AS_TEXT ) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[ NXTEXT_HDOM_INFO_END ] = $this->cursor;
		$this->parse_charset();
	...
	function clear() {
		...
		// Fix for ongoing memory leaks that occur even with the use of clear
	...
	protected function prepare(
		...
		$this->root                              = new NXTEXT_Simple_HTML_DOM_Node1( $this );
		$this->root->tag                         = 'root';
		$this->root->_[ NXTEXT_HDOM_INFO_BEGIN ] = - 1;
		$this->root->nodetype                    = NXTEXT_HDOM_TYPE_ROOT;
	...
	protected function parse() {
		while ( true ) {
			...
			$node = new NXTEXT_Simple_HTML_DOM_Node1( $this );
			++ $this->cursor;
			$node->_[ NXTEXT_HDOM_INFO_TEXT ] = $s;
	...
	protected function read_tag() {
		...
		if ( $this->char === '/' ) {
			...
			if ( $parent_lower !== $tag_lower ) {
				...
				if ( isset( $this->optional_closing_tags[ $parent_lower ] )
					...
					$this->parent->_[ NXTEXT_HDOM_INFO_END ] = 0;
					...
					if ( strtolower( $this->parent->tag ) !== $tag_lower ) {
						...
						$this->parent->_[ NXTEXT_HDOM_INFO_END ] = $this->cursor;
					...
					$this->parent->_[ NXTEXT_HDOM_INFO_END ] = 0; // No end tag
					$org_parent                              = $this->parent;
		...
		// start tag
		$node                              = new NXTEXT_Simple_HTML_DOM_Node1( $this );
		$node->_[ NXTEXT_HDOM_INFO_BEGIN ] = $this->cursor;
		++ $this->cursor;
		...
		if ( isset( $tag[0] ) && $tag[0] === '!' ) {
			$node->_[ NXTEXT_HDOM_INFO_TEXT ] = '<' . $tag . $this->copy_until_char( '>' );

			if ( isset( $tag[2] ) && $tag[1] === '-' && $tag[2] === '-' ) { // Comment ("<!--")
				$node->nodetype = NXTEXT_HDOM_TYPE_COMMENT;
				$node->tag      = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = NXTEXT_HDOM_TYPE_UNKNOWN;
				$node->tag      = 'unknown';
			}

			if ( $this->char === '>' ) {
				$node->_[ NXTEXT_HDOM_INFO_TEXT ] .= '>';
			}
		...
		if ( $pos = strpos( $tag, '<' ) !== false ) {
			$tag                              = '<' . substr( $tag, 0, - 1 );
			$node->_[ NXTEXT_HDOM_INFO_TEXT ] = $tag;
			$this->link_nodes( $node, false );
		...
		if ( ! preg_match( '/^\w[\w:-]*$/', $tag ) ) {
			$node->_[ NXTEXT_HDOM_INFO_TEXT ] = '<' . $tag . $this->copy_until( '<>' );
		...
		$node->nodetype = NXTEXT_HDOM_TYPE_ELEMENT;
		$tag_lower      = strtolower( $tag );
		...
		do {
			...
			if ( $this->pos >= $this->size - 1 && $this->char !== '>' ) {
				$node->nodetype                   = NXTEXT_HDOM_TYPE_TEXT;
				$node->_[ NXTEXT_HDOM_INFO_END ]  = 0;
				$node->_[ NXTEXT_HDOM_INFO_TEXT ] = '<' . $tag . $space[0] . $name;
				$node->tag                        = 'text';
				$this->link_nodes( $node, false );
			...
			if ( $this->doc[ $this->pos - 1 ] == '<' ) {
				...
				$node->attr                       = array();
				$node->_[ NXTEXT_HDOM_INFO_END ]  = 0;
				$node->_[ NXTEXT_HDOM_INFO_TEXT ] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ( $name !== '/' && $name !== '' ) { // this is a attribute name
				...
				if ( $this->char === '=' ) { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[ NXTEXT_HDOM_INFO_QUOTE ][] = NXTEXT_HDOM_QUOTE_NO;
					$node->attr[ $name ]                 = true;
					if ( $this->char != '>' ) {
				...
				$node->_[ NXTEXT_HDOM_INFO_SPACE ][] = $space;
		...
		$this->link_nodes( $node, true );
		$node->_[ NXTEXT_HDOM_INFO_ENDSPACE ] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ( $this->copy_until_char( '>' ) === '/' ) {
			$node->_[ NXTEXT_HDOM_INFO_ENDSPACE ] .= '/';
			$node->_[ NXTEXT_HDOM_INFO_END ]      = 0;
		} else {
			// reset parent
		...
		if ( $node->tag === 'br' ) {
			$node->_[ NXTEXT_HDOM_INFO_INNER ] = $this->default_br_text;
		}
	...
	protected function parse_attr( $node, $name, &$space ) {
		...
		switch ( $this->char ) {
			case '"':
				$quote_type = NXTEXT_HDOM_QUOTE_DOUBLE;
				$this->char = ( ++ $this->pos < $this->size ) ? $this->doc[ $this->pos ] : null; // next
				$value      = $this->copy_until_char( '"' );
				$this->char = ( ++ $this->pos < $this->size ) ? $this->doc[ $this->pos ] : null; // next
				break;
			case '\'':
				$quote_type = NXTEXT_HDOM_QUOTE_SINGLE;
				$this->char = ( ++ $this->pos < $this->size ) ? $this->doc[ $this->pos ] : null; // next
				$value      = $this->copy_until_char( '\'' );
				$this->char = ( ++ $this->pos < $this->size ) ? $this->doc[ $this->pos ] : null; // next
				break;
			default:
				$quote_type = NXTEXT_HDOM_QUOTE_NO;
		...
		// If this is a "class" selector, lets get rid of the preceeding
		...
		if ( ! $is_duplicate ) {
			$node->_[ NXTEXT_HDOM_INFO_QUOTE ][] = $quote_type;
	...
	function createElement( $name, $value = null ) {
		return @nxtext_str_get_html1( "<$name>$value</$name>" )->firstChild();
	}

	function createTextNode( $value ) {
		return @end( nxtext_str_get_html1( $value )->nodes );
	}
