<?php
class Nexter_Two_Factor_Management_Provider_Recovery_Code extends Nexter_Two_Factor_Management_Providers {
	 ...
	 * @type string
	 */
	const NEXTER_TWO_FACTOR_BACKUP_CODES = '_nexter_two_factor_backup_codes';
	...
	public function admin_notices() {
		...
		if ( ! in_array( __CLASS__, Nexter_Two_Factor_Management::get_enabled_providers_for_user( $user->ID ), true ) ) {
			return;
		}
				...
				<span>
					...
					echo wp_kses(
						...
						/* translators: %s: URL for code regeneration */
							__( 'Nexter: You have exhausted your recovery codes and need to <a href="%s">regenerate them</a>!', 'nexter-pro-extensions' ),
	...
	public function get_label() {
		return _x( 'Recovery Codes', 'Provider Label', 'nexter-pro-extensions' );
	}
	...
	public function get_alternative_provider_label() {
		return __( 'Use a recovery code', 'nexter-pro-extensions' );
	}
		...
		<p id="two-factor-backup-codes">
			...
			<?php
				echo esc_html(
					sprintf(
						/* translators: %s: count */
						_n( '%s unused code remaining; each recovery code can only be used once.', '%s unused codes remaining, each recovery code can only be used once.', $count, 'nexter-pro-extensions' ),
						$count
					)
				);
			?>
			...
			<p>
				<button type="button" class="button button-two-factor-backup-codes-generate button-secondary hide-if-no-js">
					<?php esc_html_e( 'Generate new recovery codes', 'nexter-pro-extensions' ); ?>
				</button>
			</p>
		</p>
		<div class="two-factor-backup-codes-wrapper" style="display:none;">
			<ol class="two-factor-backup-codes-unused-codes"></ol>
			<p class="description"><?php esc_html_e( 'Please make a note of these codes! Once you leave this page, you won\'t be able to view them again.', 'nexter-pro-extensions' ); ?></p>
			<p>
				<a class="button button-two-factor-backup-codes-download button-secondary hide-if-no-js" href="javascript:void(0);" id="two-factor-backup-codes-download-link" download="two-factor-backup-codes.txt"><?php esc_html_e( 'Download Codes', 'nexter-pro-extensions' ); ?></a>
			<p>
		</div>
		<script type="text/javascript">
			( function( $ ) {
				$( '.button-two-factor-backup-codes-generate' ).click( function() {
					wp.apiRequest( {
						method: 'POST',
						path: <?php echo wp_json_encode( Nexter_Two_Factor_Management::REST_NAMESPACE . '/generate-backup-codes' ); ?>,
						data: {
	...
	public function generate_codes( $user, $args = '' ) {
		...
		if ( isset( $args['method'] ) && 'append' === $args['method'] ) {
			$codes_hashed = (array) get_user_meta( $user->ID, self::NEXTER_TWO_FACTOR_BACKUP_CODES, true );
		}
		...
		update_user_meta( $user->ID, self::NEXTER_TWO_FACTOR_BACKUP_CODES, $codes_hashed );
	...
	public function rest_generate_codes( $request ) {
		...
		$title = sprintf(
			...
			__( 'Two-Factor Recovery Codes for %s', 'nexter-pro-extensions' ),
			home_url( '/' )
		);
		...
		$i18n = array(
			/* translators: %s: count */
			'count' => esc_html( sprintf( _n( '%s unused code remaining; each recovery code can only be used once.', '%s unused codes remaining, each recovery code can only be used once.', $count, 'nexter-pro-extensions' ), $count ) ),
		);

		if ( $request->get_param( 'enable_provider' ) && ! Nexter_Two_Factor_Management_Providers::enable_provider_for_user( $user_id, 'Two_Factor_Backup_Codes' ) ) {
			return new WP_Error( 'db_error', __( 'Cannot activate recovery codes for this user.', 'nexter-pro-extensions' ), array( 'status' => 500 ) );
		}
	...
	public static function codes_remaining_for_user( $user ) {
		$backup_codes = get_user_meta( $user->ID, self::NEXTER_TWO_FACTOR_BACKUP_CODES, true );
	...
	public function authentication_page( $user ) {
		...
		?>
		<p class="two-factor-prompt"><?php esc_html_e( 'Enter a recovery code.', 'nexter-pro-extensions' ); ?></p><br/>
		<p>
			<label for="authcode"><?php esc_html_e( 'Recovery Code:', 'nexter-pro-extensions' ); ?></label>
	...
	public function delete_code( $user, $code_hashed ) {
		...
		update_user_meta( $user->ID, self::NEXTER_TWO_FACTOR_BACKUP_CODES, $backup_codes );
	}
