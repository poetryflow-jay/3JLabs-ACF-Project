<?php
class Nexter_Two_Factor_Management_Provider_Totp extends Nexter_Two_Factor_Management_Providers {
     ...
     * @var string
     */
    const NEXTER_TWO_FACTOR_TOTP_TOKEN = '_nexter_two_factor_totp_token';
     ...
     * @var string
     */
    const NEXTER_LAST_SUCCESSFUL_LOGIN_TOKEN = '_nexter_last_successful_login_token';
    ...
    public function register_rest_routes() {
        register_rest_route(
	        Nexter_Two_Factor_Management::REST_NAMESPACE,
            '/totp',
            array(
                array(
                    ...
                    'permission_callback' => function( $request ) {
                        return Nexter_Two_Factor_Management::rest_api_can_edit_user_and_update_two_factor_options( $request['user_id'] );
                    },
                    'args'                => array(
                        'user_id' => array(
                            'required' => true,
    ...
    public function get_label() {
        return _x( 'Authenticator app', 'Provider Label', 'nexter-pro-extensions' );
    }
    ...
    public function get_alternative_provider_label() {
        return __( 'Use your authenticator app', 'nexter-pro-extensions' );
    }
    ...
    public function enqueue_assets( $hook_suffix ) {
        wp_register_script(
	        ...
	        NXT_PRO_EXT_URI . 'includes/settings-options/2fa/qrcode.js',
            [],
            NXT_PRO_EXT_VER,
            true 
        );
    ...
    public function rest_setup_totp( $request ) {
        ...
        if ( ! $this->is_valid_key( $key ) ) {
            return new WP_Error( 'invalid_key', __( 'Invalid Two Factor Authentication secret key.', 'nexter-pro-extensions' ), array( 'status' => 400 ) );
        }

        if ( ! $this->is_valid_authcode( $key, $code ) ) {
            return new WP_Error( 'invalid_key_code', __( 'Invalid Two Factor Authentication code.', 'nexter-pro-extensions' ), array( 'status' => 400 ) );
        }

        if ( ! $this->set_user_totp_key( $user_id, $key ) ) {
            return new WP_Error( 'db_error', __( 'Failed to save the Two-Factor Authentication code. Please rescan the QR code and enter the code provided by your application.', 'nexter-pro-extensions' ), array( 'status' => 500 ) );
        }

        if ( $request->get_param( 'enable_provider' ) && ! Nexter_Two_Factor_Management::enable_provider_for_user( $user_id, 'Two_Factor_Totp' ) ) {
            return new WP_Error( 'db_error', __( 'Cannot activate TOTP provider for this user.', 'nexter-pro-extensions' ), array( 'status' => 500 ) );
        }
    ...
    public function user_two_factor_options( $user ) {
        ...
        <div id="two-factor-totp-options">
            ...
            if ( empty( $key ) ) :
                ...
                <p>
                    <?php esc_html_e( 'Scan the QR code or manually enter the key, and then input an authentication code from your app to complete the setup.', 'nexter-pro-extensions' ); ?>
                </p>
                ...
                <p>
                    ...
                    <label for="two-factor-totp-authcode">
                        <?php esc_html_e( 'Authentication Code:', 'nexter-pro-extensions' ); ?>
                        <?php
                        /* translators: Example auth code. */
                        $placeholder = sprintf( __( 'eg. %s', 'nexter-pro-extensions' ), '123456' );
                        ?>
                    ...
                    <input type="submit" class="button totp-submit" name="two-factor-totp-submit" value="<?php esc_attr_e( 'Submit', 'nexter-pro-extensions' ); ?>" />
                </p>
                ...
                <script>
                    (function($){
                        $('.totp-submit').click( function( e ) {
                            ...
                            wp.apiRequest( {
                                method: 'POST',
                                path: <?php echo wp_json_encode( Nexter_Two_Factor_Management::REST_NAMESPACE . '/totp' ); ?>,
                                data: {
            ...
            <?php else : ?>
                <p class="success">
                    <?php esc_html_e( 'An authenticator app is currently configured. If reset is necessary, you will need to re-scan the QR code on all devices.', 'nexter-pro-extensions' ); ?>
                </p>
                <p>
                    <button type="button" class="button button-secondary reset-totp-key hide-if-no-js">
                        <?php esc_html_e( 'Reset authenticator app', 'nexter-pro-extensions' ); ?>
                    </button>
                    <script>
                        ( function( $ ) {
    ...
    public function get_user_totp_key( $user_id ) {
        return (string) get_user_meta( $user_id, self::NEXTER_TWO_FACTOR_TOTP_TOKEN, true );
    }
    ...
    public function set_user_totp_key( $user_id, $key ) {
        return update_user_meta( $user_id, self::NEXTER_TWO_FACTOR_TOTP_TOKEN, $key );
    }
    ...
    public function delete_user_totp_key( $user_id ) {
        delete_user_meta( $user_id, self::NEXTER_LAST_SUCCESSFUL_LOGIN_TOKEN );
        return delete_user_meta( $user_id, self::NEXTER_TWO_FACTOR_TOTP_TOKEN );
    }
    ...
    public function validate_code_for_user( $user, $code ) {
        ...
        $last_totp_login = (int) get_user_meta( $user->ID, self::NEXTER_LAST_SUCCESSFUL_LOGIN_TOKEN, true );
        ...
        update_user_meta( $user->ID, self::NEXTER_LAST_SUCCESSFUL_LOGIN_TOKEN, $valid_timestamp );
    ...
    public function authentication_page( $user ) {
        ...
        <p class="two-factor-prompt">
            <?php esc_html_e( 'Enter the code generated by your authenticator app.', 'nexter-pro-extensions' ); ?>
        </p>
        <p>
            <label for="authcode"><?php esc_html_e( 'Authentication Code:', 'nexter-pro-extensions' ); ?></label>
        ...
        <?php
        submit_button( __( 'Submit', 'nexter-pro-extensions' ) );
    }
    ...
    public function pre_process_authentication(){
        return false;
        }
}
