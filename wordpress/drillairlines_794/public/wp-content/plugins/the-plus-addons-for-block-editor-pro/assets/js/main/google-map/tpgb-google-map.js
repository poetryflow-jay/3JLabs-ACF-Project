				...
				var build = function() {
					...
					for (let i = 0; i < data.places.length; i++) {
                        ...
                        if (place.latOrAddr === "address" && place.addr) {
                            // Handle address-based location with geocoding
                            const geocoder = new google.maps.Geocoder();
                            geocoder.geocode({ address: place.addr }, function (results, status) {
                                if (status === 'OK' && results[0]) {
                                    ...
                                    google.maps.event.addListener(marker, 'click', (function(marker, i) {
                                        return function() {
                                            if (place.address && place.address.length > 1) {
                                                infoWindow.setContent('<div class="gmap_info_content"><p>' + place.address + '</p></div>');
                                            }
