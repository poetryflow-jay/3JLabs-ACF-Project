...
function tpgb_tp_countdown_callback( $attributes, $content) {
    ...
    if($attributes['countdownExpiry'] == 'redirect') {
        if(class_exists('Tpgbp_Pro_Blocks_Helper')){
            $encodedUrl = (isset($attributes['expiryRedirect']['dynamic'])) ? Tpgbp_Pro_Blocks_Helper::tpgb_dynamic_repeat_url($attributes['expiryRedirect']) : (!empty($attributes['expiryRedirect']['url']) ? $attributes['expiryRedirect']['url'] : '');
        }else{
    ...
    $dataAttr .= 'data-countdata="' . htmlspecialchars( wp_json_encode( $countData ), ENT_QUOTES, 'UTF-8' ) . '"';
