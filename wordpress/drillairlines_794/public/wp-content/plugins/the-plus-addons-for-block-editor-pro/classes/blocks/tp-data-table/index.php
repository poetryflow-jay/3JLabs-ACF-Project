                    ...
                    if ( $row_count > 1 ) {
                        ...
                        foreach ( $Tablebody as $index => $item ) {
                            if( $item['trAction'] == 'cell' ){
                                ...
                                $DTBody .= '<'.Tpgbp_Pro_Blocks_Helper::validate_html_tag($Tag).' class="tpgb-table-col tp-repeater-item-'.esc_attr($item['_key']).' '.( !empty($item['TrLink']) && !empty($item['TrLink']['url']) ? ' tpgb-td-flex' : '' ).'" colspan="'.esc_attr($TrColumnSpan).'" rowspan="'.esc_attr($TrRowSpan).'">';
                                    ...
                                    if( (!empty($item['Trbtn'])) && $item['Trbtn'] == TRUE ){
                                        ...
                                        $DTBody .='<div class="pt_tpgb_button tp-repeater-item-'.esc_attr($item['_key']).' button-style-8">';
                                            $DTBody .='<a '.(!empty($Btnlink) ? $Btnlink : 'href="#"').' class="button-link-wrap" '.$CA.' '.$target.' '.$nofollow.' '.$btn_attr.'>'.wp_kses_post($Btntx).( !empty($item['btnIcon']) ? '<i class="'.esc_attr($item['btnIcon']).'"></i>' : '' ).'</a>';
                                        $DTBody .='</div>';
                                    }
