...
function tpgb_mailchimp_render_callback( $attributes, $content) {
	...
	if(!empty($attributes["thankYouPage"])){
		$redirect_thankyou['url'] = (isset($attributes['thankYouPageLink']['dynamic'])) ? Tpgbp_Pro_Blocks_Helper::tpgb_dynamic_repeat_url($attributes['thankYouPageLink']) : (!empty($attributes['thankYouPageLink']['url']) ? $attributes['thankYouPageLink']['url'] : '');
		$redirect_thankyou['target'] = (!empty($attributes['thankYouPageLink']['target']) ? '_blank' : '');
		$redirect_thankyou['nofollow'] = (!empty($attributes['thankYouPageLink']['nofollow']) ? 'nofollow' : '');
	}
	...
	$settings_opt = htmlspecialchars(wp_json_encode($settings), ENT_QUOTES, 'UTF-8');
	
	//Gdpr Checkbox
	$getgdprCom = '';
	if($gdprCompli == 'yes'){
		$getgdprCom .= '<div class="tpgb-gdpr-wrap">';
			$getgdprCom .= '<input type="checkbox" name="tpgb_gdpr_check" id="tpgbgppr'.esc_attr( $block_id ).'" class="tpgb-gdpr-checkbox">';
			$getgdprCom .= '<label class="tpgb-gdpr-label" for="tpgbgppr'.esc_attr( $block_id ).'"></label>';
			if(!empty($gdprSub)){
				$getgdprCom .= '<label for="tpgbgppr'.esc_attr( $block_id ).'" class="tpgb-gdpr-desc">'.wp_kses_post($gdprSub).'</label>';
			}
		$getgdprCom .= '</div>';
	}
    ...
    $output .= '<div class="tpgb-mailchimp tpgb-relative-block tpgb-block-'.esc_attr($block_id).' form-'.esc_attr($styleType).' '.esc_attr($blockClass).'">';
		$output .='<form action="'.site_url().'/wp-admin/admin-ajax.php" class="tpgb-mailchimp-form tpgb-mail-'.esc_attr($block_id).' text-center text--tablet text--mobile" data-id="'.esc_attr($block_id).'" data-thank-you="'.esc_attr(json_encode($redirect_thankyou)).'" data-mail-option=\'' . $settings_opt . '\'>';
			$output .='<div class="plus-newsletter-input-wrapper">';
				$output .='<input type="hidden" name="nonce" value="'. wp_create_nonce("tpgb-mailchamp").'" />';
				if($styleType!='style-1'){
					...
					if($styleType=='style-3' && !empty($DPhoneField)){
						$output .='<input type="tel" name="PHONE" placeholder="'.esc_html($phoneField).'" required pattern="[0-9]*" class="form-control tp-mailchimp-phone"/>';
					}
				}
				...
				if((!empty($attributes["mGroup"])) && !empty($attributes["mGroupIds"]) && is_array($attributes["mGroupIds"])){
					$groupValues = array_column($attributes["mGroupIds"], 'value');
					$catIds = implode(' | ', $groupValues);
...
function tpgb_tp_mailchimp() {
	...
	// 				(object) [
	// 					'condition' => [(object) ['key' => 'styleType', 'relation' => '==', 'value' => 'style-3' ]],
    //                     'selector' => '{{PLUS_WRAP}}.tpgb-mailchimp.form-style-3 .plus-newsletter-input-wrapper{ text-align: {{buttonAlign}};} {{PLUS_WRAP}}.tpgb-mailchimp.form-style-3 .subscribe-btn-submit{ display: inline-block; margin-left: auto !important; margin-right: auto !important; margin-top: 10px;}',
	// 				],
	// 			],
	// 			'scopy' => true,
	// 		],
	...
	// 			'style' => [
	// 				(object) [
	// 					'selector' => '{{PLUS_WRAP}} .tpgb-mailchimp-form .plus-newsletter-input-wrapper{margin: {{outerPadding}};}'
	// 				],
	// 			],
	// 			'scopy' => true,
	// 		],
    ...
    if(method_exists('Tpgb_Blocks_Global_Options', 'merge_options_json')){
		$block_data = Tpgb_Blocks_Global_Options::merge_options_json(__DIR__, 'tpgb_mailchimp_render_callback');
...
function tpgb_mailchimp_subscribe(){
	...
	if(!isset($_POST['nonce']) || empty($_POST['nonce']) || ! wp_verify_nonce( $_POST['nonce'], 'tpgb-mailchamp' )){	
