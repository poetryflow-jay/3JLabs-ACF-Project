...
function tpgb_tp_scroll_sequence_render_callback( $attributes ) {
	...
	if($scrollType=='image'){
		...
		}else if(!empty($imagePath) && !empty($totalImage) && in_array($imageType, ['jpg', 'png','webp'])){
    ...
    $output .= '<div class="tpgb-scroll-sequence tpgb-relative-block tpgb-block-'.esc_attr($block_id).' '.esc_attr($blockClass).'" '.$data_attr.'></div>';
...
function tpgb_tp_scroll_sequence() {
    ...
    if(method_exists('Tpgb_Blocks_Global_Options', 'merge_options_json')){
		$block_data = Tpgb_Blocks_Global_Options::merge_options_json(__DIR__, 'tpgb_tp_scroll_sequence_render_callback');
