...
function tpgbp_social_reviews_callback($attributes, $content) {
	...
	if(!empty($txtLimt)){
	   ...
	   $txtlimitData = wp_json_encode($txtlimitDataa, true);
	}
			...
			if( ($Performance == false) || ($Performance == true && $Perfo_transient === false) ){
				...
				foreach ($Repeater as $index => $R) {
					...
					if($RRT == 'facebook'){
						...
						$AllData[] = tpgbp_Google_Reviews($R, $review_id);
					}else if($RRT == 'custom'){
...
function tpgbp_Facebook_Reviews($RData,$attr){
    ...
    if(!empty($API)){
        ...
        $FbArr[] = tpgbp_Review_Error_array( $ErrorData, $Key, $Fb_Icon, $ReviewsType, $RCategory );
    }
    return $FbArr;
}
function tpgbp_Google_Reviews($RData, $review_id){
    $Key = (!empty($RData['_key']) ? $RData['_key'] : '');
    $Token = (!empty($RData['Token']) ? $RData['Token'] : '');
    ...
    if(!empty($API)){
        ...
        if($G_status == 200 && empty($Gdata['error_message']) && $Gdata['status'] == 'OK'){
            ...
            $GG_Databash = get_option("gutenberg_google_review_{$PlaceName}_{$review_id}");
            if ( !empty($GR) && (empty($GG_Databash) || $GG_Databash == false) ) {
                add_option( "gutenberg_google_review_{$PlaceName}_{$review_id}", $GR, "", "yes" );
                    ...
                    foreach ($GG_Databash as $i2 => $DataTwo){
                        ...
                        if( $AuthorUrlOne != $AuthorUrlTwo ){
                            if( !in_array( $AuthorUrlOne, $AarayTemp ) ){
                                ...
                                update_option( "gutenberg_google_review_{$PlaceName}_{$review_id}", $GG_Databash);
                            }
                        }
                    }
                }
		...
		$ErrorData['error']['message'] = $Msg;
        $GArr[] = tpgbp_Review_Error_array( $ErrorData, $Key, $GG_Icon, $ReviewsType, $RCategory );
    }
...
function tpgbp_Custom_Reviews($RData){
    ...
    $Platform=$logo="";
    if($CType == 'custom'){
        $Platform = (!empty($RData['CcuSname']) ? $RData['CcuSname'] : 'Custom');
    ...
    $PImg = ( isset($RData['CUImg']['url']) && !empty($RData['CUImg']['url']) ) ? array("Profile" => $RData['CUImg']['url']) : '';
    ...
    foreach ($Name as $key => $value){
        $FImg = (!empty($PImg) ? $PImg : array("Profile" => TPGB_ASSETS_URL.'assets/images/tpgb-placeholder.jpg') );
...
function tpgbp_Beach_Reviews($attr){
    ...
    if($BType == "b-facebook"){
        $API = "https://graph.facebook.com/v20.0/{$BPPId}?access_token={$BToken}&fields=ratings.fields(reviewer{id,name,picture.width(120).height(120)},created_time,rating,recommendation_type,review_text,open_graph_story{id}).limit(100),overall_star_rating,rating_count,username";
        $Type = $FTitle;
        $Logo = TPGB_ASSETS_URL.'assets/images/social-review/facebook.svg';
...
function tpgbp_social_reviews() {
    ...
    //             (object) [
    //                 'condition' => [(object) ['key' => 'RType', 'relation' => '==', 'value' => 'review' ]],
    //                 'selector' => '{{PLUS_WRAP}}.tpgb-social-reviews .facebook .tpgb-sr-profile, {{PLUS_WRAP}}.tpgb-social-reviews .google .tpgb-sr-profile {border-radius:{{FbPRs}};}',
    //             ],
    //         ],
	// 		'scopy' => true,
    //     ],
    ...
    if(method_exists('Tpgb_Blocks_Global_Options', 'merge_options_json')){
		$block_data = Tpgb_Blocks_Global_Options::merge_options_json(__DIR__, 'tpgbp_social_reviews_callback',true,true,false,true);
