...
function tpgb_social_sharing( $attributes, $content) {
	...
	$get_title = urlencode(get_the_title($post_id));
			...
			if(!empty($socialSharing)){
				...
				foreach ( $socialSharing as $index => $network ) {
					...
					$output .= '<div class="tpgb-social-menu tp-repeater-item-'.esc_attr($network['_key']).'">';
						...
						if(!empty($network['customURL']) && ( !empty($network['customLink']['url']) || isset($network['customLink']['dynamic']) ) ){
							$getCustomLink = (isset($network['customLink']['dynamic'])) ? Tpgbp_Pro_Blocks_Helper::tpgb_dynamic_repeat_url($network['customLink']) : (!empty($network['customLink']['url']) ? $network['customLink']['url'] : '');
						...
						$target = (!empty($network['customLink']['target'])) ? 'target="_blank"' : '';
						$nofollow = (!empty($network['customLink']['nofollow'])) ? 'rel="nofollow"' : '';
						...
						$output .= '<a href="'.esc_url($getCustomLink).'" '.$target.' '.$nofollow.' class="share-btn tpgb-rel-flex tpgb-trans-easeinout '.esc_attr($contentAlign).'" '.$link_attr.' aria-label="'.$ariaLabel.'">';
...
function tpgb_tp_social_sharing() {
    ...
    if(method_exists('Tpgb_Blocks_Global_Options', 'merge_options_json')){
		$block_data = Tpgb_Blocks_Global_Options::merge_options_json(__DIR__, 'tpgb_social_sharing');
