...
use Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\TemporaryFileCachedRemoteGetRequest;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec;
use ReflectionClass;
...
final class TransformationEngine
{
     ...
     */
    public function __construct(?Configuration $configuration = null, ?RemoteGetRequest $remoteRequest = null, ?Spec $spec = null)
    {
    ...
    private function getTransformerDependencies($transformerClass)
    {
        ...
        foreach ($constructor->getParameters() as $parameter) {
            ...
            if (\is_a($dependencyType->name, TransformerConfiguration::class, \true)) {
                $dependencies[] = $this->configuration->getTransformerConfiguration($transformerClass);
                continue;
            }
            if (\is_a($dependencyType->name, RemoteGetRequest::class, \true)) {
            ...
            if (\is_a($dependencyType->name, Spec::class, \true)) {
                if ($this->spec === null) {
                    $this->spec = new Spec();
                }
