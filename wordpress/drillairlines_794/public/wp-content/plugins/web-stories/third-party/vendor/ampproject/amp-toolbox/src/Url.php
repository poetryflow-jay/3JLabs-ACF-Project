...
final class Url
{
     ...
     * }
     */
    const URL_DEFAULT_PARTS = [self::SCHEME => 'https', self::HOST => 'example.com', self::PORT => null, self::USER => null, self::PASS => null, self::PATH => null, self::QUERY => null, self::FRAGMENT => null];
    /**
     * Url constructor.
     *
    ...
    private function unrelativizePath($path)
    {
        ...
        do {
            $path = \preg_replace(self::COLLAPSE_CURRENT_PATHS_REGEX_PATTERN, '/', $path, -1, $count);
        } while (0 !== $count);
        // Collapse relative paths, like <foo/bar/../../baz.css> => <baz.css>.
        do {
            $path = \preg_replace(self::COLLAPSE_RELATIVE_PATHS_REGEX_PATTERN, '', $path, 1, $count);
        } while (0 !== $count);
        return $path;
    }
    /**
    ...
    public function isValidNonDataUrl()
    {
        ...
        $parts = \array_merge(self::URL_DEFAULT_PARTS, $this->toArray(\true));
    ...
    {
        $array = [self::SCHEME => $this->scheme, self::HOST => $this->host, self::PORT => $this->port, self::USER => $this->user, self::PASS => $this->pass, self::PATH => $this->path, self::QUERY => $this->query, self::FRAGMENT => $this->fragment];
