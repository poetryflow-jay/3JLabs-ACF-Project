...
class AMP_Validation_Manager
{
    ...
    public static function init()
    {
        ...
        if (!\amp_is_canonical()) {
            $filter_hooks = ['default_option_' . \Google\Web_Stories_Dependencies\AMP_Options_Manager::OPTION_NAME, 'option_' . \Google\Web_Stories_Dependencies\AMP_Options_Manager::OPTION_NAME];
    ...
    public static function post_supports_validation($post)
    {
        ...
        return \in_array($post->post_type, \Google\Web_Stories_Dependencies\AMP_Post_Type_Support::get_eligible_post_types(), \true) && !\wp_is_post_autosave($post) && !\wp_is_post_revision($post) && 'auto-draft' !== $post->post_status && 'trash' !== $post->post_status && \amp_is_post_supported($post);
    }
    /**
    ...
    public static function is_sanitization_auto_accepted($error = null)
    {
        if ($error && \amp_is_canonical()) {
            ...
            $accepted = \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::STYLESHEET_TOO_LONG !== $error['code'];
        } else {
            $accepted = \true;
        }
        /**
    ...
    public static function add_admin_bar_menu_items($wp_admin_bar)
    {
        ...
        if (\amp_is_canonical()) {
            $amp_url = $current_url;
            $non_amp_url = \add_query_arg(QueryVar::NOAMP, QueryVar::NOAMP_AVAILABLE, $current_url);
        } elseif ($is_amp_request) {
            $amp_url = $current_url;
            $non_amp_url = \add_query_arg(QueryVar::NOAMP, QueryVar::NOAMP_MOBILE, \amp_remove_paired_endpoint($current_url));
        } else {
        ...
        if ($is_amp_request) {
            $icon = Icon::valid();
            // This will get overridden in AMP_Validation_Manager::finalize_validation() if there are unaccepted errors.
            $href = $validate_url;
        } else {
            $icon = Icon::link();
            $href = $amp_url;
        }
        $icon_html = $icon->to_html(['id' => 'amp-admin-bar-item-status-icon', 'class' => 'ab-icon']);
        $validate_url_title = \__('Validate URL', 'amp');
        $parent = ['id' => 'amp', 'title' => \sprintf('%s %s', $icon_html, \esc_html__('AMP', 'amp')), 'href' => \esc_url($href), 'meta' => ['title' => \esc_attr($is_amp_request ? $validate_url_title : \__('View AMP version', 'amp'))]];
        // Construct admin bar item for validation.
        $validate_item = ['parent' => 'amp', 'id' => 'amp-validity', 'title' => \esc_html($validate_url_title), 'href' => \esc_url($validate_url)];
        // Construct admin bar item to link to AMP version or non-AMP version.
        $wp_admin_bar->remove_node('amp-view');
        // Remove so we can re-add in the right position.
        $link_item = ['parent' => 'amp', 'id' => 'amp-view', 'href' => \esc_url($is_amp_request ? $non_amp_url : $amp_url)];
        if (\amp_is_canonical()) {
        ...
        if (\current_user_can('manage_options')) {
            $wp_admin_bar->add_node(['parent' => 'amp', 'id' => 'amp-settings', 'title' => \esc_html__('Settings', 'amp'), 'href' => \esc_url(\admin_url(\add_query_arg('page', \Google\Web_Stories_Dependencies\AMP_Options_Manager::OPTION_NAME, 'admin.php')))]);
        }
        self::$amp_admin_bar_item_added = \true;
    }
    /**
    ...
    {
        $override_validation_error_statuses = isset($_REQUEST['preview']) && !empty($_REQUEST[\Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY]) && \is_array($_REQUEST[\Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY]);
        ...
        if (!isset($_REQUEST['_wpnonce']) || !\wp_verify_nonce(\sanitize_key($_REQUEST['_wpnonce']), self::MARKUP_STATUS_PREVIEW_ACTION)) {
            \wp_die(\esc_html__('Preview link expired. Please try again.', 'amp'), \esc_html__('Error', 'amp'), ['response' => 401]);
        }
        /*
    ...
    {
        $defaults = [self::VALIDATE_QUERY_VAR_NONCE => null, self::VALIDATE_QUERY_VAR_CACHE => \false, self::VALIDATE_QUERY_VAR_CACHED_IF_FRESH => \false, self::VALIDATE_QUERY_VAR_OMIT_STYLESHEETS => \false, self::VALIDATE_QUERY_VAR_FORCE_STANDARD_MODE => \false];
        if (!isset($_GET[self::VALIDATE_QUERY_VAR])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return $defaults;
        }
        $unsanitized_values = $_GET[self::VALIDATE_QUERY_VAR];
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        if (\is_string($unsanitized_values)) {
            $unsanitized_values = [self::VALIDATE_QUERY_VAR_NONCE => $unsanitized_values];
    ...
    public static function init_validate_request()
    {
        ...
        if (\true === $should_validate_response) {
            self::add_validation_error_sourcing();
            self::$is_validate_request = \true;
            ...
            if ($should_validate_response instanceof \WP_Error) {
                \wp_send_json(['code' => $should_validate_response->get_error_code(), 'message' => $should_validate_response->get_error_message()], 401);
            }
        }
    }
    /**
    ...
    public static function add_validation_error_sourcing()
    {
        ...
        $int_min = \defined('PHP_INT_MIN') ? \PHP_INT_MIN : ~\PHP_INT_MAX;
        ...
        if (\class_exists('WP_Block_Type_Registry')) {
            \add_filter('the_content', [__CLASS__, 'add_block_source_comments'], 8);
    ...
    public static function filter_the_editor_to_detect_sources($output)
    {
        ...
        // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_debug_backtrace -- Only way to find theme/plugin responsible for calling.
        foreach ($backtrace as $call) {
            if (!empty($call['function']) && !empty($call['file']) && 'wp_editor' === $call['function']) {
    ...
    public static function map_meta_cap($caps, $cap)
    {
        if (self::VALIDATE_CAPABILITY === $cap) {
            ...
            $position = \array_search($cap, $caps, \true);
            if (\false !== $position) {
    ...
    public static function add_validation_error(array $error, array $data = [])
    {
        ...
        $sanitization = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::get_validation_error_sanitization($error);
        $sanitized = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_NEW_ACCEPTED_STATUS === $sanitization['status'] || \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_ACK_ACCEPTED_STATUS === $sanitization['status'];
        /*
     ...
     */
    public static function get_source_comment(array $source, $is_start = \true)
    {
        unset($source['reflection']);
        return \sprintf('<!--%samp-source-stack %s-->', $is_start ? '' : '/', \str_replace('--', '', \wp_json_encode($source)));
    }
    /**
    ...
    public static function parse_source_comment(\DOMComment $comment)
    {
        ...
        $source = \json_decode($matches['args'], \true);
    ...
    protected static function is_matching_script(\DOMElement $element, $script_handle)
    {
        ...
        return \false !== \strpos($element->getAttribute('src'), \preg_replace('#^https?:(?=//)#', '', $script_dependency->src));
    }
    /**
    ...
    public static function locate_sources(\DOMNode $node)
    {
        ...
        $is_enqueued_link = $node instanceof \DOMElement && 'link' === $node->nodeName && \preg_match('/(?P<handle>.+)-css$/', (string) $node->getAttribute(Attribute::ID), $matches) && \wp_styles()->query($matches['handle']);
        if ($is_enqueued_link) {
            // Directly enqueued stylesheet.
            if (isset(self::$enqueued_style_sources[$matches['handle']])) {
                $sources = \array_merge(self::$enqueued_style_sources[$matches['handle']], $sources);
            }
            ...
            foreach (\wp_styles()->done as $style_handle) {
                ...
                foreach (self::find_done_dependent_handles(\wp_styles(), $style_handle, \array_keys(self::$enqueued_style_sources)) as $enqueued_style_sources_handle) {
                    $sources = \array_merge(\array_map(static function ($enqueued_style_source) use($style_handle) {
                        $enqueued_style_source['dependency_handle'] = $style_handle;
                        return $enqueued_style_source;
                    }, self::$enqueued_style_sources[$enqueued_style_sources_handle]), $sources);
                }
            }
        }
        $is_inline_style = $node instanceof \DOMElement && 'style' === $node->nodeName && $node->firstChild instanceof \DOMText && $node->hasAttribute(Attribute::ID) && \preg_match('/^(?P<handle>.+)-inline-css$/', $node->getAttribute(Attribute::ID), $matches) && \wp_styles()->query($matches['handle']) && isset(self::$extra_style_sources[$matches['handle']]);
        if ($is_inline_style) {
            $text = $node->textContent;
            foreach (self::$extra_style_sources[$matches['handle']] as $css => $extra_sources) {
                if (\false !== \strpos($text, $css)) {
                    $sources = \array_merge($sources, $extra_sources);
                }
            }
        }
        if ($node instanceof \DOMElement && 'script' === $node->nodeName) {
            ...
            if ($node->hasAttribute('src')) {
                ...
                foreach ($enqueued_script_handles as $enqueued_script_handle) {
                    ...
                    $sources = \array_merge(self::$enqueued_script_sources[$enqueued_script_handle], $sources);
                    break;
                }
                ...
                foreach (\wp_scripts()->done as $script_handle) {
                    ...
                    foreach (self::find_done_dependent_handles(\wp_scripts(), $script_handle, \array_keys(self::$enqueued_script_sources)) as $enqueued_script_sources_handle) {
                        $sources = \array_merge(\array_map(static function ($enqueued_script_source) use($script_handle) {
                            $enqueued_script_source['dependency_handle'] = $script_handle;
                            return $enqueued_script_source;
                        }, self::$enqueued_script_sources[$enqueued_script_sources_handle]), $sources);
                    }
                }
                ...
                $script_handle = null;
                $script_type = null;
                if ($node->hasAttribute(Attribute::ID) && \preg_match('/^(.+)-js-(extra|after|before|translations)/', $node->getAttribute(Attribute::ID), $matches)) {
    ...
    private static function find_done_dependent_handles(\WP_Dependencies $dependencies, $handle, $enqueued_handles)
    {
        ...
        foreach ($enqueued_handles as $enqueued_handle) {
            if ($enqueued_handle !== $handle && $dependencies->query($enqueued_handle, 'done') && self::has_dependency($dependencies, $enqueued_handle, $handle)) {
    ...
    public static function add_block_source_comments($content)
    {
        ...
        $start_block_pattern = \implode('', ['#<!--\\s+', '(?P<closing>/)?', 'wp:(?P<name>\\S+)', '(?:\\s+(?P<attributes>\\{.*?\\}))?', '\\s+(?P<self_closing>\\/)?', '-->#s']);
        return \preg_replace_callback($start_block_pattern, [__CLASS__, 'handle_block_source_comment_replacement'], $content);
    }
    /**
    ...
    protected static function handle_block_source_comment_replacement($matches)
    {
        ...
        if ($block_type && $block_type->is_dynamic()) {
            ...
            while ($render_callback instanceof \Google\Web_Stories_Dependencies\AMP_Validation_Callback_Wrapper) {
            ...
            if ($callback_source && 'plugin' === $callback_source['type'] && 'gutenberg' === $callback_source['name'] && \array_key_exists($source['block_name'], self::$original_block_render_callbacks)) {
                $callback_source = $callback_reflection->get_source(self::$original_block_render_callbacks[$source['block_name']]);
            }
            if ($callback_source) {
                $source = \array_merge($source, $callback_source);
            }
        }
        if (!empty($matches['closing'])) {
            $replaced .= self::get_source_comment($source, \false);
        } else {
            $replaced = self::get_source_comment($source, \true) . $replaced;
    ...
    public static function wrap_block_callbacks($args)
    {
        ...
        unset($source['reflection']);
        // Omit from stored source.
        $original_function = $args['render_callback'];
        if (isset($args['name'])) {
            self::$original_block_render_callbacks[$args['name']] = $original_function;
        }
        $args['render_callback'] = new \Google\Web_Stories_Dependencies\AMP_Validation_Callback_Wrapper(['function' => $original_function, 'source' => $source, 'accepted_args' => 3]);
        return $args;
    }
    /**
    ...
    public static function wrap_widget_callbacks()
    {
        ...
        foreach ($wp_registered_widgets as $widget_id => &$registered_widget) {
            ...
            $accepted_args = 2;
            // For the $instance and $args arguments.
            $callback = \compact('function', 'accepted_args', 'source');
            $registered_widget['callback'] = new \Google\Web_Stories_Dependencies\AMP_Validation_Callback_Wrapper($callback);
        }
    }
    /**
    ...
    public static function wrap_hook_callbacks($hook)
    {
        ...
        foreach ($wp_filter[$hook]->callbacks as $priority => &$callbacks) {
            foreach ($callbacks as &$callback) {
                ...
                $wrapped_callback = new \Google\Web_Stories_Dependencies\AMP_Validation_Callback_Wrapper(\array_merge($callback, \compact('priority', 'source', 'indirect_sources')), static function () use(&$callback, $original_function) {
    ...
    public static function decorate_shortcode_source($output, $tag)
    {
        ...
        $output = \implode('', [self::get_source_comment($source, \true), $output, self::get_source_comment($source, \false)]);
        return $output;
    }
    /**
    ...
    {
        $source = ['embed' => $url, 'attr' => $attr];
        return \implode('', [self::get_source_comment($source, \true), \trim($output), self::get_source_comment($source, \false)]);
    }
    /**
    ...
    public static function decorate_filter_source($value)
    {
        ...
        $post = \get_post();
        $source = ['hook' => \current_filter(), 'filter' => \true];
        if ($post) {
        ...
        return \implode('', [self::get_source_comment($source, \true), $value, self::get_source_comment($source, \false)]);
    }
    /**
    ...
    {
        \_deprecated_function(__METHOD__, '2.0.2', '\\AmpProject\\AmpWP\\DevTools\\CallbackReflection::get_source');
        return Services::get('dev_tools.callback_reflection')->get_source($callback);
    }
    /**
    ...
    {
        return new \Google\Web_Stories_Dependencies\AMP_Validation_Callback_Wrapper($callback);
    }
    /**
    ...
    {
        $data = ['results' => self::$validation_results, 'queried_object' => null, 'url' => \amp_get_current_url()];
        ...
        foreach ($sanitizers as $class_name => $sanitizer) {
            ...
            if (!empty($conflicting_keys)) {
                \_doing_it_wrong(\esc_html("{$class_name}::get_validate_response_data"), \esc_html('Method is returning array with conflicting keys: ' . \implode(', ', $conflicting_keys)), '1.5');
            } else {
    ...
    public static function send_validate_response($sanitization_results, $status_code, $last_error)
    {
        ...
        $data = ['http_status_code' => $status_code, 'php_fatal_error' => \false];
        if ($last_error && \in_array($last_error['type'], [\E_ERROR, \E_RECOVERABLE_ERROR, \E_CORE_ERROR, \E_COMPILE_ERROR, \E_USER_ERROR, \E_PARSE], \true)) {
            $data['php_fatal_error'] = $last_error;
        }
        $data = \array_merge($data, self::get_validate_response_data($sanitization_results));
        $args = self::get_validate_request_args();
        $data['revalidated'] = \true;
        if ($args[self::VALIDATE_QUERY_VAR_CACHE]) {
            $validation_errors = \wp_list_pluck($data['results'], 'error');
            $validated_url_post_id = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::store_validation_errors($validation_errors, \amp_get_current_url(), $data);
            if (\is_wp_error($validated_url_post_id)) {
                \status_header(500);
                return \wp_json_encode(['code' => $validated_url_post_id->get_error_code(), 'message' => $validated_url_post_id->get_error_message()]);
            } else {
                \status_header(201);
                $data['validated_url_post'] = ['id' => $validated_url_post_id, 'edit_link' => \get_edit_post_link($validated_url_post_id, 'raw')];
            }
        }
    ...
    public static function maybe_send_cached_validate_response()
    {
        ...
        $post = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::get_invalid_url_post(\amp_get_current_url());
        if (!$post instanceof \WP_Post) {
            return;
        }
        $staleness = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::get_post_staleness($post);
        if (\count($staleness) > 0) {
            return;
        }
        $response = [
            'http_status_code' => 200,
            // Note: This is not currently cached in postmeta.
            'php_fatal_error' => \false,
            'results' => [],
            'queried_object' => null,
            'url' => null,
            'revalidated' => \false,
            // Since cached was used.
            'validated_url_post' => ['id' => $post->ID, 'edit_link' => \get_edit_post_link($post->ID, 'raw')],
        ];
        if (!$args[self::VALIDATE_QUERY_VAR_OMIT_STYLESHEETS]) {
            $stylesheets = \get_post_meta($post->ID, \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::STYLESHEETS_POST_META_KEY, \true);
            if ($stylesheets) {
                $response['stylesheets'] = \json_decode($stylesheets, \true);
            }
        }
        $stored_validation_errors = \json_decode($post->post_content, \true);
        if (\is_array($stored_validation_errors)) {
            $response['results'] = \array_map(static function ($stored_validation_error) {
                $error = $stored_validation_error['data'];
                $sanitized = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::is_validation_error_sanitized($error);
                return \compact('error', 'sanitized');
            }, $stored_validation_errors);
        }
        $queried_object = \get_post_meta($post->ID, \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::QUERIED_OBJECT_POST_META_KEY, \true);
        if ($queried_object) {
            $response['queried_object'] = $queried_object;
        }
        $php_fatal_error = \get_post_meta($post->ID, \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::PHP_FATAL_ERROR_POST_META_KEY, \true);
        if ($php_fatal_error) {
            $response['php_fatal_error'] = $php_fatal_error;
        }
        $response['url'] = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::get_url_from_post($post);
    ...
    private static function update_admin_bar_item(Document $dom, $total_count, $kept_count, $unreviewed_count)
    {
        ...
        if ($unreviewed_count > 0) {
            if ($unreviewed_count === $total_count) {
                ...
                $items[] = \_n('unreviewed', 'all unreviewed', $unreviewed_count, 'amp');
            } else {
                $items[] = \sprintf(
                    /* translators: %s the total count of unreviewed validation errors */
                    \_n('%s unreviewed', '%s unreviewed', $unreviewed_count, 'amp'),
                    \number_format_i18n($unreviewed_count)
                );
            }
        }
        if ($kept_count > 0) {
            if ($kept_count === $total_count) {
                /* translators: text is describing validation issue(s) */
                $items[] = \_n('kept', 'all kept', $kept_count, 'amp');
            } else {
                $items[] = \sprintf(
                    /* translators: %s the total count of unreviewed validation errors */
                    \_n('%s kept', '%s kept', $kept_count, 'amp'),
                    \number_format_i18n($kept_count)
                );
            }
        }
        if (empty($items)) {
            /* translators: text is describing validation issue(s) */
            $items[] = \_n('reviewed', 'all reviewed', $total_count, 'amp');
        }
        $text = \sprintf(
            /* translators: %s is total count of validation errors */
            \_n('%s issue:', '%s issues:', $total_count, 'amp'),
            \number_format_i18n($total_count)
        );
        $text .= ' ' . \implode(', ', $items);
    ...
    public static function filter_sanitizer_args($sanitizers)
    {
        ...
        if (isset($sanitizers[\Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::class])) {
            $sanitizers[\Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::class]['should_locate_sources'] = self::is_validate_request();
            $css_validation_errors = [];
            foreach (self::$validation_error_status_overrides as $slug => $status) {
                $term = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::get_term($slug);
                if (!$term) {
                    continue;
                }
                $validation_error = \json_decode($term->description, \true);
                $is_css_validation_error = \is_array($validation_error) && isset($validation_error['code']) && \in_array($validation_error['code'], \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::get_css_parser_validation_error_codes(), \true);
                if ($is_css_validation_error) {
                    $css_validation_errors[$slug] = $status;
                }
            }
            if (!empty($css_validation_errors)) {
                $sanitizers[\Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::class]['parsed_cache_variant'] = \md5(\wp_json_encode($css_validation_errors));
            }
        }
        return $sanitizers;
    }
    ...
    public static function validate_url($url)
    {
        ...
        $added_query_vars = [self::VALIDATE_QUERY_VAR => [self::VALIDATE_QUERY_VAR_NONCE => self::get_amp_validate_nonce(), self::VALIDATE_QUERY_VAR_CACHE_BUST => \wp_rand()]];
        ...
        for ($redirect_count = 0; $redirect_count < $allowed_redirects; $redirect_count++) {
            $r = \wp_remote_get($validation_url, [
                ...
                'redirection' => 0,
                // Because we're in a loop for redirection.
            ...
            $is_redirect = $response_code && $response_code > 300 && $response_code < 400 && $location_header;
            if (!$is_redirect) {
                break;
            }
            // Ensure absolute URL.
        ...
        $response = \trim(\wp_remote_retrieve_body($r));
        if (\wp_remote_retrieve_response_code($r) >= 400) {
            $data = \json_decode($response, \true);
            return new \WP_Error(\is_array($data) && isset($data['code']) ? $data['code'] : \wp_remote_retrieve_response_code($r), \is_array($data) && isset($data['message']) ? $data['message'] : \wp_remote_retrieve_response_message($r));
        }
        if (\wp_remote_retrieve_response_code($r) >= 300) {
            return new \WP_Error('http_request_failed', \__('Too many redirects.', 'amp'));
        }
        $url = \remove_query_arg(\array_keys($added_query_vars), $validation_url);
        ...
        $validation = \json_decode($response, \true);
        ...
        return \array_merge($validation, \compact('url'));
    }
    /**
    ...
    public static function validate_url_and_store($url, $post = null)
    {
        ...
        $result = \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::store_validation_errors(\wp_list_pluck($validity['results'], 'error'), $validity['url'], $args);
    ...
    public static function unserialize_validation_error_messages($serialized)
    {
        ...
        if (\count($parts) !== 2 || !\hash_equals($parts[0], \wp_hash($parts[1] . \wp_nonce_tick(), 'nonce'))) {
            return null;
        }
    ...
    public static function get_validate_url_error_message($error_code, $error_message = '')
    {
        ...
        switch ($error_code) {
            case 'http_request_failed':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('Failed to fetch URL to validate.', 'amp'), \esc_html($error_message), $site_health_message, $support_forum_message]);
            case 'white_screen_of_death':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('Unable to validate URL. A white screen of death was encountered which is likely due to a PHP fatal error.', 'amp'), \esc_html($error_message), $check_error_log, $support_forum_message]);
            case '404':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('The fetched URL was not found. It may have been deleted. If so, you can trash this.', 'amp'), \esc_html($error_message), $support_forum_message]);
            case '500':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('An internal server error occurred when fetching the URL for validation.', 'amp'), \esc_html($error_message), $check_error_log, $support_forum_message]);
            case 'fatal_error_during_validation':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('A PHP fatal error occurred while validating the URL. This may indicate either a bug in theme/plugin code or it may be due to an issue in the AMP plugin itself.', 'amp'), \defined('WP_DEBUG_DISPLAY') && \WP_DEBUG_DISPLAY ? \esc_html__('The error details appear below.', 'amp') : $check_error_log . ' ' . \wp_kses_post(\sprintf(\__('Alternatively, you may enable %s to show the error details below.', 'amp'), '<code>WP_DEBUG_DISPLAY</code>')), $support_forum_message]);
            case 'response_not_json':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('URL validation failed due to the AMP validation request not returning JSON data. This may be due to a PHP fatal error occurring.', 'amp'), \esc_html($error_message), $check_error_log, $support_forum_message]);
            case 'malformed_json_validation_errors':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('URL validation failed due to unexpected JSON in AMP validation response.', 'amp'), \esc_html($error_message), $support_forum_message]);
            default:
    ...
    public static function enqueue_block_validation()
    {
        ...
        \wp_enqueue_script($slug, \amp_get_asset_url("js/{$slug}.js"), $dependencies, $version, \true);
        \wp_enqueue_style($slug, \amp_get_asset_url("css/{$slug}.css"), \false, \AMP__VERSION);
