...
class AutopurgeCommand extends WP_CLI_Command {
	 ...
	 *     +----------------------+-------------------------------------------------------------+---------+
	 *     | wp_option_controller | Purge cache when options are updated.                       | enabled |
	 *     | elementor_controller | Purge cache on Elementor updates that affect the front-end. | enabled |
	 *     +----------------------+-------------------------------------------------------------+---------+
	 *
	 ...
	 *     $ wp kinsta cache autopurge set options_update true
	...
	public function set( array $args, array $assoc_args ) {
        ...
        $key = $args[0];
        $status = $args[1];
        $status = in_array($status, ['true', '1', 1, true], true) ? true : false;
