...
class WC_Gateway_Portone_Kakao extends PORTONE_Payment_Gateway {

	const GATEWAY_ID = 'iamport_kakao';
	protected $use_new_version;

	public function __construct() {
		parent::__construct();

		//settings
		$this->method_title       = __( '포트원(카카오페이)', 'iamport-for-woocommerce' );
		...
		$this->use_new_version = portone_get( $this->settings, 'use_new_version' );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array_merge( array(
			'enabled'                   => array(
				...
				'type'    => 'checkbox',
				'label'   => __( '포트원(카카오페이) 결제 사용. (카카오페이를 사용하시려면, <a href="https://admin.portone.io/settings" target="_blank">포트원 관리자페이지의 PG설정화면</a>에서 "추가PG사"로 카카오페이를 추가 후 사용해주세요.)', 'iamport-for-woocommerce' ),
				'default' => 'yes'
			),
			'title'                     => array(
			...
			'use_manual_pg' => array(
				...
				'type'        => 'checkbox',
				'description' => __( '(신규 카카오페이 버전에서만 사용가능) 포트원 계정에 설정된 여러 PG사 / MID를 사용자의 선택에 따라 적용하는 기능을 활성화합니다. 카카오페이 결제수단 선택 시, 세부 결제수단 선택창이 추가로 출력됩니다.', 'iamport-for-woocommerce' ),
				'default'     => 'no',
			),
			'manual_pg_id'  => array(
	...
	public function iamport_order_detail( $order_id ) {
        ...
        <table class="shop_table order_details">
            ...
            <tr>
                ...
                <td><?php esc_html_e( '카카오페이', 'iamport-for-woocommerce' ) ?></td>
            </tr>
            <tr>
	...
	public function iamport_payment_info( $order_id ) {
		...
		if ( $useNewVersion ) {
			...
			if ( $this->has_subscription( $order_id ) ) { //정기결제
				if ( portone_get( $this->settings, 'recurring_mid' ) ) {
					$response[ "pg" ] = sprintf( "%s.%s", $response[ "pg" ], portone_get( $this->settings, 'recurring_mid' ) );
				}
				...
				if ( portone_get( $this->settings, 'onetime_mid' ) ) {
					$response[ "pg" ] = sprintf( "%s.%s", $response[ "pg" ], portone_get( $this->settings, 'onetime_mid' ) );
				}
			}
		} else {
			$response[ 'pg' ] = 'kakao';
	...
	public function get_display_categories() {
		...
		if ( $categories === 'all' || ( is_array( $categories ) && in_array( 'all', $categories ) ) ) {
			return 'all';
		}
	...
	public function get_disabled_categories() {
		...
		if ( $categories === 'none' || ( is_array( $categories ) && in_array( 'none', $categories ) ) ) {
			return array();
		}
	...
	private function all_products_in_categories( $product_ids, $categories ) {
		...
		foreach ( $product_ids as $id ) {
			$all_match = $all_match && Portone_Helper::is_product_in_categories( $id, $categories );
		}
	...
	public function kakao_unset_gateway_by_category( $available_gateways ) {
		...
		if ( class_exists( 'WC_Subscriptions_Change_Payment_Gateway' ) && WC_Subscriptions_Change_Payment_Gateway::$is_request_to_change_payment ) {
			$subscription_obj = wcs_get_subscription( portone_get( $_GET, 'change_payment_method' ) );
		...
		if ( ! $enabled || ! ( $categories === 'all' || $this->all_products_in_categories( $product_ids, $categories ) ) ) {
			unset( $available_gateways[ 'iamport_kakao' ] );
		}
