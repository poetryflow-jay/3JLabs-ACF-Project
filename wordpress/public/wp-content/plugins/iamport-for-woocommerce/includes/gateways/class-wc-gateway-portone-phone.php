...
class WC_Gateway_Portone_Phone extends PORTONE_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( '포트원(휴대폰소액결제)', 'iamport-for-woocommerce' );
		$this->method_description = __( '=> 포트원 서비스를 이용해 결제모듈을 연동할 수 있습니다.<br>=> 다날과 계약 후 이용하시면 보다 합리적인 수수료, 정산주기 조건으로 휴대폰결제 서비스를 이용하실 수 있습니다.</a><br>=> 궁금한 사항이 있다면 헬프센터를 방문해 주세요.<br><a href="https://help.portone.io/" target="_blank">헬프센터 방문</a>', 'iamport-for-woocommerce' );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array_merge( array(
			'enabled'          => array(
				...
				'type'    => 'checkbox',
				'label'   => __( '포트원(휴대폰소액결제) 결제 사용', 'iamport-for-woocommerce' ),
				'default' => 'yes'
			),
			'title'            => array(
			...
			'_pg_auto_title'   => array(
				'title'       => __( '(1) 휴대폰소액결제 시, 자동 적용될 PG설정값 세팅', 'iamport-for-woocommerce' ),
				'type'        => 'title',
			...
			'_pg_manual_title' => array(
				'title'       => __( '(2) 휴대폰소액결제 시, 적용될 PG설정값을 고객이 직접 지정', 'iamport-for-woocommerce' ),
				'type'        => 'title',
			...
			'use_manual_pg'    => array(
				...
				'type'        => 'checkbox',
				'description' => __( '포트원 계정에 설정된 여러 PG사 / MID를 사용자의 선택에 따라 적용하는 기능을 활성화합니다. 휴대폰소액결제 결제수단 선택 시, 세부 결제수단 선택창이 추가로 출력됩니다.', 'iamport-for-woocommerce' ),
				'default'     => 'no',
			),
			'manual_pg_id'     => array(
	...
	public function init_settings() {
		...
		if ( $this->legacy_danal_user() ) {
			$p = portone_get( $this->settings, 'pg_provider' );
			if ( empty( $p ) || $p === 'none' ) {
	...
	public function iamport_order_detail( $order_id ) {
        ...
        <table class="shop_table order_details">
            ...
            <tr>
                ...
                <td><?php esc_html_e( '휴대폰소액결제', 'iamport-for-woocommerce' ) ?></td>
            </tr>
            <tr>
	...
	public function iamport_payment_info( $order_id ) {
		...
		$iamport_info[ 'digital' ] = filter_var( portone_get( $this->settings, 'digital_contents' ), FILTER_VALIDATE_BOOLEAN );
