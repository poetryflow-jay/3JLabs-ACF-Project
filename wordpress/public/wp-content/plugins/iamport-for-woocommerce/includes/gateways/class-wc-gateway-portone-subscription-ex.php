...
class WC_Gateway_Portone_Subscription_Ex extends PORTONE_Payment_Gateway {
	...
	public function __construct() {
		...
		//settings
		$this->method_title       = __( '포트원(정기결제-결제창방식)', 'iamport-for-woocommerce' );
		$this->method_description = __( 'KG이니시스, 다날과 같이 결제창을 통해 빌링키 발급이 이뤄지는 정기결제의 경우 본 결제 방식을 통해 정기결제를 이용하실 수 있습니다. (정기결제 기능을 위해서는 Woocommerce-Subscription 플러그인 설치가 필요합니다.)', 'iamport-for-woocommerce' );
		$this->has_fields         = true;
		$this->supports           = array( 'products', 'subscriptions', 'subscription_reactivation'/*이것이 있어야 subscription 후 active상태로 들어갈 수 있음*/, 'subscription_suspension', 'subscription_cancellation', 'refunds', 'subscription_date_changes', 'subscription_amount_changes', 'multiple_subscriptions' );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array_merge( array(
			'enabled'     => array(
				...
				'type'    => 'checkbox',
				'label'   => __( '포트원(정기결제-결제창방식) 결제 사용', 'iamport-for-woocommerce' ),
				'default' => 'yes'
			),
			'title'       => array(
				...
				'default'     => __( '포트원 정기결제', 'iamport-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'description' => array(
	...
	public function iamport_payment_info( $order_id ) {
		...
		if ( ! $this->isEmptyProvider() && ! empty( $this->pg_id ) ) {
			$iamport_info[ 'pg' ] = $this->pg_provider . '.' . $this->pg_id;
		} //TODO : [2019-08-12] 왜 pg_id까지 체크하지...
	...
	public function check_payment_response() {
		...
		$called_from_iamport = empty( $http_param[ 'order_id' ] ); //wp_safe_redirect 안하기 위해서 boolean 기록
		...
		if ( ! empty( $http_param[ 'imp_uid' ] ) ) {
			...
			if ( $result->success ) {
				...
				if ( $this->isAlreadyPaidOrder( $order_id ) ) {
				...
				if ( $payment_data->status === 'paid' ) {
					if ( $called_from_iamport ) {
						exit( 'Ignored' );
					} //빌링키 발급 후 전송되는 Notification 은 무시
					...
					if ( in_array( $payment_data->pg_provider, array( 'danal', 'danal_tpay', 'mobilians' ) ) ) { //JTNet, 다날-휴대폰 소액결제 정기결제, 다날-신용카드, 모빌리언스-휴대폰 소액결제 정기결제는 이미 결제를 하고 돌아옴
							...
							if ( $amountLeft ) { //한 번 더 환불이 가능함. 다음 번 환불이 가능하도록 status는 바꾸지 않음
								...
								for ( $i = 0; $i < $increment; $i ++ ) {
									...
									if ( is_wp_error( $refund ) ) {
										...
										$order->add_order_note( sprintf( __( '포트원 관리자 페이지(https://admin.portone.io)에서 부분취소(%s원)하였습니다.', 'iamport-for-woocommerce' ), number_format( $cancelItem->amount ) ) );
									}
								}
							} else {
								$order->update_status( 'refunded' ); //imp_uid
								$order->add_order_note( __( '포트원 관리자 페이지(https://admin.portone.io)에서 취소하여 우커머스 결제 상태를 "환불됨"으로 수정합니다.', 'iamport-for-woocommerce' ) );
	...
	private function doPayment( $creds, $order, $total, $customer_uid, $number_of_tried = 0 ) {
		...
		if ( wc_tax_enabled() ) {
			$pay_data[ "tax_free" ] = Portone_Helper::get_tax_free_amount( $order );
		}
		if ( $notice_url ) {
	...
	private function doFakePayment( $payment_data, $total ) {
		...
		return new WP_Error( 'iamport_error', sprintf( __( '정기결제 최초 과금(signup fee)시 금액 검증에 실패하였습니다(요청 결제금액 : %1$s, 실제 결제금액 : %2$s)', 'iamport-for-woocommerce' ), number_format( $total ), number_format( $payment_data->amount ) ) );
	}
	...
	private function isAlreadyPaidOrder( $order_id ) {
		...
		try {
			...
			$alreadyPaid = $this->has_status( $order_status, wc_get_is_paid_statuses() );
		} catch ( Exception $e ) {
		} //finally 는 5.5부터 지원됨. 5.5미만 환경이 있을 수 있으므로 finally block은 사용하지 않는다.
	...
	protected function get_order_name( $order, $initial_payment = true ) {
		...
		if ( $initial_payment ) {
			...
			$order_name = sprintf( '%1$s-정기결제(%2$s회차)', $base_order_name, $order->suspension_count );
		}
