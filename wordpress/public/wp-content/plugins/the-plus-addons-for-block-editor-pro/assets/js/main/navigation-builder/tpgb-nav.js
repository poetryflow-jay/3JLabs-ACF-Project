            ...
            if (navLinks) {
                navLinks.forEach(function (navLink) {
                    var navPath =
                        ...
                        navLink.getAttribute("href") != "javascript:void(0);"
                            ? navLink.getAttribute("href").replace(/\/$/, "")
                            : "";
                    var menuItemPathname = navPath
                        ? new URL(navPath, window.location.href).pathname
                        : "";
                    var menuItemHash = navPath
                        ? navPath.substring(navPath.indexOf("#"))
                        : "";
            
                    if (menuItemHash === currentHash) {
                        // clear all active before setting
                        ...
                        navPath === window.location.href.replace(/\/$/, "") &&
                        navLink.closest("li")
                    ) {
                        var closestLi = navLink.closest("li");
            
                        // remove active from siblings inside same parent <ul>
                        var siblings = closestLi.parentNode.querySelectorAll("li.active");
...
function tpNav(doc){
    ...
    if (getNavBuilder) {
        getNavBuilder = Array.from(getNavBuilder).filter(nav => nav.parentElement.closest('.tpgb-navbuilder') === null);
        getNavBuilder.forEach((nav)=> {
                ...
                menusubClickItems.forEach((item) => {
                    item.addEventListener('click', (e) => {
                        ...
                        if (e.currentTarget.classList.contains('menu-item-has-children')) {
                            e.currentTarget.querySelectorAll('.dropdown-menu a').forEach(link => {
                        ...
                        if (closestNavInner) {
                            var navSideBut = e.currentTarget,
                                navSideItem = navSideBut.parentNode,
                                navSideUl = navSideItem.parentNode,
                                navSideItemSub = navSideBut.querySelector('ul.dropdown-menu');

                            if(navSideItemSub.classList.contains('open-menu')){
                                slideUpP(navSideItemSub, 400);
                                navSideItemSub.classList.remove('open-menu');
                                navSideItemSub.parentNode.classList.remove('open');
                            }else{
                                slideDownP(navSideItemSub, 400);
                                navSideItemSub.classList.add('open-menu');
                                navSideItemSub.parentNode.classList.add('open');
                            }
                        }

                        // Initialize Splide.js if necessary
                        if (navSideUl.querySelectorAll('li.dropdown.open ul.dropdown-menu .tpgb-carousel').length) {
                            var splideDiv = navSideUl.querySelector('li.dropdown.open ul.dropdown-menu');
                            var scope = splideDiv.querySelectorAll('.tpgb-carousel');
                            navsplider(scope);
                        ...
                        // Trigger isotope layout if necessary
                        if (navSideUl.querySelectorAll('li.dropdown.open ul.dropdown-menu .tpgb-isotope .post-loop-inner').length) {
                            navSideUl.querySelectorAll('li.dropdown.open ul.dropdown-menu .tpgb-isotope .post-loop-inner').forEach((el) => {
                                el.isotope('layout');
                            });
            ...
            if(menuHoverNav){
                ...
                menuHoverItems.forEach((item)=> {
                    ...
                    let triggeredOnLoad = false;
                    if (item.matches(':hover') && triggeredOnLoad==false) {                                
                        menuSubMenuHover('enter', item)
                ...
                nav.querySelectorAll(".tpgb-toggle-menu.hamburger-toggle").forEach((item)=> {
                    ...
                    item.addEventListener('click', ()=> {
                        ...
                        if (gtParent.classList.contains("tpgb-cont-in")) {
                            ...
                            offsetLeft = 0 - gtParent.offsetLeft;
                            windowWidth = gtParent.offsetWidth;
            ...
            if (getoffCan) {
                ...
                let activeMobileItem = nav.querySelector('.tpgb-mobile-menu .menu-item.active');

                if (activeMobileItem) {
                    let mobileClasses = activeMobileItem.className;
                    let desktopMenu = nav.querySelector('.tpgb-normal-menu .nav.navbar-nav');

                    if (desktopMenu) {
                        desktopMenu.querySelectorAll('.menu-item.active').forEach(item => {
                            item.classList.remove('active');
                        });

                        let itemId = mobileClasses.match(/tp-repeater-item-\w+/);
                        if (itemId) {
                            let matchingDesktopItem = desktopMenu.querySelector(`.${itemId[0]}`);
...
function tpgbresleftCss(offset_left , window_width , menu_content , pareEle = '' ){
...
/* Js For Mega Menu */
document.addEventListener('DOMContentLoaded', tpnavmResize);
