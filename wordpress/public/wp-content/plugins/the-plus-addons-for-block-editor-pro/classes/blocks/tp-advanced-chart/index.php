...
function tpgb_advanced_chart_render_callback( $attributes, $content) {
	...
	$dotBColor=$dotBG='';
		...
		$dataPrePost = htmlspecialchars(wp_json_encode($dataPrePost), ENT_QUOTES, 'UTF-8');
	$output = '';
...
function tpgb_advanced_chart() {
	...
	if(method_exists('Tpgb_Blocks_Global_Options', 'merge_options_json')){
		$block_data = Tpgb_Blocks_Global_Options::merge_options_json(__DIR__, 'tpgb_advanced_chart_render_callback');
