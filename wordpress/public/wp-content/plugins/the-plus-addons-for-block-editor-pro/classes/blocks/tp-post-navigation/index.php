...
function tpgb_tp_post_nav_render_callback( $attributes, $content) {
	...
	if( function_exists('tribe_get_events') && is_singular('tribe_events') ){
        ...
        foreach ($events as $event) {
			if( $event->ID != $current_event_id && !$prev_event ){
				$currnt =  $event;
			}
            ...
            if ($event->ID === $current_event_id && !$prev_event) {
                $prev_event = $currnt;
				$nextev = true;
            } elseif ($nextev && !$next_event) {
                $next_event = $event;
            }
        }

		$prev_post = $prev_event;
		$next_post = $next_event;
    }
	...
	if (has_post_thumbnail( $prev_id ) ){
		$preImg .= '<div class="post-image">';
			$preImg .= get_the_post_thumbnail( $prev_id, 'thumbnail', array( 'class' => 'tpgb-nav-trans', 'alt'=> esc_attr__('Previous Post Image','tpgbp') ) );
		$preImg .= '</div>';
	}else{
		$preImg .= '<div class="post-image">';
			$preImg .= '<img src="'.TPGB_URL .'/assets/images/tpgb-placeholder.jpg" class="tpgb-nav-trans" alt="'.esc_attr__('Previous Post Image','tpgbp').'"/>';
		$preImg .= '</div>';
	}
	...
	if (has_post_thumbnail( $next_id ) ){
		$nextImg .= '<div class="post-image">';
			$nextImg .= get_the_post_thumbnail( $next_id, 'thumbnail', array( 'class' => 'tpgb-nav-trans', 'alt'=> esc_attr__('Next Post Image','tpgbp') ) );
		$nextImg .= '</div>';
	}else{
		$nextImg .= '<div class="post-image">';
			$nextImg .= '<img src="'.TPGB_URL .'/assets/images/tpgb-placeholder.jpg" class="tpgb-nav-trans" alt="'.esc_attr__('Next Post Image','tpgbp').'"/>';
		$nextImg .= '</div>';
	}
...
function tpgb_post_nav_content() {
    ...
    if(method_exists('Tpgb_Blocks_Global_Options', 'merge_options_json')){
		$block_data = Tpgb_Blocks_Global_Options::merge_options_json(__DIR__, 'tpgb_tp_post_nav_render_callback');
