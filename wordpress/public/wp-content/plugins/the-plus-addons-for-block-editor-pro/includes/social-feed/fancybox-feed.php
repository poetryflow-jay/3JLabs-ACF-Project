...
if($FancyStyle == 'style-1'){
	...
	if(empty($FbAlbum)){
		...
		if( $PopupOption == "Donothing" ){
            ...
            $PopupLink = 'href="' . ( ($selectFeed == 'Instagram' && isset($PostLink) && !empty($PostLink)) ? $PostLink : esc_url($videoURL) ) . '"';
		}else if( $PopupOption == "OnFancyBox" ){
			$PopupLink = 'href="'. ( ( is_array($videoURL) && isset($videoURL[0]['link']) && !empty($videoURL[0]['link']) ) ? esc_url($videoURL[0]['link']) : esc_url($videoURL) ) .'"';
		}
	}
		...
		if( ($Type == 'video' || $Type == 'photo') && (!empty($ImageURL)) ){
			if($style == "style-1" || $style == "style-2"){ ?> 
				<a <?php echo $PopupLink . $PopupTarget . $FancyBoxJS; ?> class="tpgb-soc-img-cls tpgb-relative-block" aria-label="<?php echo esc_attr__('Social Media Post','tpgbp'); ?>">
					<?php echo $IGGP_Icon ?>
					<?php echo ( $style == "style-1" && $PopupOption == "GoWebsite" ) ? '' : '<img class="tpgb-post-thumb" src="'.esc_url($ImageURL).'" alt="'.esc_attr__('Social Media Image','tpgbp').'"/>'; ?>
				</a>
