<?php
    ...
    if(!empty($txtLimt)){
        ...
        if($TextType == 'char'){
            ...
            $firstdesc = mb_substr($Description, 0, $TextCount);
            $totaldesc = mb_substr($Description, $TextCount, $ltn);
        ...
        // First text
            ...
            $firstdesc = preg_replace($url, '<a href="$0" target="_blank" rel="noopener noreferrer" class="tpgb-feedurl" title="" aria-label="'.esc_attr__('Hashtag','tpgbp').'">$0</a>', $HashTag);
        // Total text
            $Mantion = preg_replace('/(^|\s)@([\w.]+)/', '$1<span class="tpgb-mantion" >@$2</span>', $totaldesc);
            $HashTag = preg_replace("/#(\\S+)/", '<span class="tpgb-hashtag" >$0 </span>', $Mantion);
            $totaldesc = preg_replace($url, '<a href="$0" target="_blank" rel="noopener noreferrer" class="tpgb-feedurl" title="" aria-label="'.esc_attr__('Hashtag','tpgbp').'">$0</a>', $HashTag);
        ?> 
        ...
        $feedurl = preg_replace($url, '<a href="$0" target="_blank" rel="noopener noreferrer" class="tpgb-feedurl" title="" aria-label="'.esc_attr__('Description','tpgbp').'">$0</a>', $Description);
