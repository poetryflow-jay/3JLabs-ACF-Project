...
use Google\Web_Stories_Dependencies\AmpProject\Html\Parser\DocLocator;
use Google\Web_Stories_Dependencies\AmpProject\Html\Parser\HtmlSaxHandlerWithLocation;
...
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\DisallowedManufacturedBody;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\DuplicateAttribute;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\DuplicateUniqueTag;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\InvalidDoctypeHtml;
/**
...
final class ValidationHandler implements HtmlSaxHandlerWithLocation
{
    ...
    public function startTag(ParsedTag $tag)
    {
        ...
        if ($maybeDuplicateAttributeName !== null) {
            $this->context->addWarning(DuplicateAttribute::CODE, $this->context->getFilePosition(), [$tag->lowerName(), $maybeDuplicateAttributeName], '', $this->validationResult);
    ...
    {
        $this->context->addError(DisallowedManufacturedBody::CODE, $this->context->getFilePosition(), [], '', $this->validationResult);
    }
    /**
    ...
    public function effectiveBodyTag($attributes)
    {
        ...
        $this->context->addError(DuplicateUniqueTag::CODE, $this->context->getEncounteredBodyFilePosition(), [Tag::BODY], '', $this->validationResult);
    }
    /**
    ...
    private function validateDocType(ParsedTag $tag)
    {
        ...
        if (\count($attributes) === 2 && ($attributes[0]->name() === 'html' && $attributes[1]->name() === 'lang' || $attributes[0]->name() === 'lang' && $attributes[1]->name() === 'html')) {
            return;
        }
        if (\count($attributes) !== 1 || $attributes[0]->name() !== 'html') {
            $this->context->addError(InvalidDoctypeHtml::CODE, $this->context->getFilePosition(), [], 'https://amp.dev/documentation/guides-and-tutorials/start/create/basic_markup/', $this->validationResult);
        }
    }
    /**
    ...
    private function validateTag($encounteredTag, $bestMatchReferencePoint, $context)
    {
        ...
        if ($tagSpecDispatch === null || !$tagSpecDispatch->hasDispatchKeys() && \count($filteredTagSpecs) === 0) {
            ...
            $context->addError(ErrorCode::DISALLOWED_TAG, $context->getFilePosition(), [$encounteredTag->lowerName()], $specUrl, $result);
        ...
        if ($tagSpecDispatch->hasDispatchKeys()) {
            foreach ($encounteredTag->attributes() as $attribute) {
                ...
                foreach ($tagSpecIds as $tagSpecId) {
                    ...
                    $attempt = $this->validateTagAgainstSpec($parsedTagSpec, $bestMatchReferencePoint, $context, $encounteredTag);
                    if ($context->getRules()->betterValidationResultThan($attempt->getValidationResult(), $bestAttempt->getValidationResult())) {
                        $bestAttempt = $attempt;
        ...
        if (\count($filteredTagSpecs) === 0) {
            ...
            if ($encounteredTag->upperName() === Tag::SCRIPT) {
                ...
                $context->addError(ErrorCode::DISALLOWED_SCRIPT_TAG, $context->getFilePosition(), [], $context->getRules()->getScriptSpecUrl(), $result);
            } else {
                $context->addError(ErrorCode::GENERAL_DISALLOWED_TAG, $context->getFilePosition(), [$encounteredTag->lowerName()], '', $result);
            }
            return new ValidateTagResult($result);
        }
