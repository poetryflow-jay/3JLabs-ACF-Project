...
use Google\Web_Stories_Dependencies\AmpProject\Html\LowerCaseTag;
use Google\Web_Stories_Dependencies\AmpProject\Html\Parser\ParsedAttribute;
use Google\Web_Stories_Dependencies\AmpProject\Html\Parser\ParsedTag;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\DevModeOnly;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\DisallowedAttr;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\InvalidAttrValue;
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\Error\MandatoryAttrMissing;
final class ValidatorRules
{
     ...
     * @var array<string>
     */
    const TYPE_IDENTIFIERS = [Attribute::AMP_EMOJI, Attribute::AMP_EMOJI_ALT, Attribute::AMP, Attribute::AMP4ADS_EMOJI, Attribute::AMP4ADS_EMOJI_ALT, Attribute::AMP4ADS, Attribute::AMP4EMAIL_EMOJI, Attribute::AMP4EMAIL_EMOJI_ALT, Attribute::AMP4EMAIL, Attribute::TRANSFORMED, Attribute::DATA_AMPDEVMODE, Attribute::DATA_CSS_STRICT];
    /**
     * Set of type identifiers that are not mandatory.
     *
     * @var array<string>
     */
    const OPTIONAL_TYPE_IDENTIFIERS = [Attribute::TRANSFORMED, Attribute::DATA_AMPDEVMODE, Attribute::DATA_CSS_STRICT];
    /**
    ...
    public function validateHtmlTag(ParsedTag $htmlTag, Context $context, ValidationResult $validationResult)
    {
        switch ($this->htmlFormat) {
            case Format::AMP:
                $this->validateTypeIdentifiers($htmlTag->attributes(), [Attribute::AMP_EMOJI, Attribute::AMP_EMOJI_ALT, Attribute::AMP, Attribute::TRANSFORMED, Attribute::DATA_AMPDEVMODE], $context, $validationResult);
                break;
            case Format::AMP4ADS:
                $this->validateTypeIdentifiers($htmlTag->attributes(), [Attribute::AMP4ADS_EMOJI, Attribute::AMP4ADS_EMOJI_ALT, Attribute::AMP4ADS, Attribute::DATA_AMPDEVMODE], $context, $validationResult);
                break;
            case Format::AMP4EMAIL:
                $this->validateTypeIdentifiers($htmlTag->attributes(), [Attribute::AMP4EMAIL_EMOJI, Attribute::AMP4EMAIL_EMOJI_ALT, Attribute::AMP4EMAIL, Attribute::DATA_AMPDEVMODE, Attribute::DATA_CSS_STRICT], $context, $validationResult);
                break;
        }
    }
    /**
     ...
     * @param ValidationResult  $validationResult  Validation result.
     */
    private function validateTypeIdentifiers($attributes, $formatIdentifiers, Context $context, ValidationResult $validationResult)
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ($this->isTypeIdentifier($attribute->name())) {
                ...
                if (\in_array($attribute->name(), $formatIdentifiers, \true)) {
                    // Only add the type identifier once per representation. That is, both "⚡" and "amp", which
                    // represent the same type identifier.
                    $typeIdentifier = \str_replace([Attribute::AMP_EMOJI_ALT, Attribute::AMP_EMOJI], Attribute::AMP, $attribute->name());
                    if (!\in_array($typeIdentifier, $validationResult->getTypeIdentifiers(), \true)) {
                    ...
                    if (!\in_array($typeIdentifier, self::OPTIONAL_TYPE_IDENTIFIERS, \true)) {
                        $hasMandatoryTypeIdentifier = \true;
                    }
                    ...
                    if ($typeIdentifier === Attribute::TRANSFORMED && $attribute->value() !== '') {
                        ...
                        if (\preg_match(self::TRANSFORMED_VALUE_REGEX, $attribute->value(), $matches)) {
                            ...
                            $context->addError(InvalidAttrValue::CODE, $context->getFilePosition(), [$attribute->name(), LowerCaseTag::HTML, $attribute->value()], 'https://amp.dev/documentation/guides-and-tutorials/learn/spec/amphtml#required-markup', $validationResult);
                        }
                    }
                    if ($typeIdentifier === Attribute::DATA_AMPDEVMODE) {
                        ...
                        $context->addError(DevModeOnly::CODE, $context->getFilePosition(), [], '', $validationResult);
                    }
                } else {
                    $context->addError(DisallowedAttr::CODE, $context->getFilePosition(), [$attribute->name(), LowerCaseTag::HTML], 'https://amp.dev/documentation/guides-and-tutorials/learn/spec/amphtml#required-markup', $validationResult);
                }
            }
        }
        if (!$hasMandatoryTypeIdentifier) {
            // Missing mandatory type identifier (any AMP variant but "transformed").
            $context->addError(MandatoryAttrMissing::CODE, $context->getFilePosition(), [$formatIdentifiers[0], LowerCaseTag::HTML], 'https://amp.dev/documentation/guides-and-tutorials/learn/spec/amphtml#required-markup', $validationResult);
        }
    }
    /**
    ...
    {
        return \in_array($attribute, self::TYPE_IDENTIFIERS, \true);
    }
}
