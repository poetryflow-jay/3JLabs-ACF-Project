...
use Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\AtRuleBlockList;
use Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\CSSList;
use Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\Document as CSSDocument;
use Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\KeyFrame;
...
use Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList;
 ...
 * @internal
 */
class AMP_Style_Sanitizer extends \Google\Web_Stories_Dependencies\AMP_Base_Sanitizer
{
    ...
    // @todo The correctness of using "5" should be validated.
    /**
     ...
     * @var array
     */
    protected $DEFAULT_ARGS = ['disable_style_processing' => \false, 'dynamic_element_selectors' => ['amp-img', 'amp-anim', 'amp-list', 'amp-live-list', '[submit-error]', '[submit-success]', 'amp-script', 'amp-story-captions'], 'should_locate_sources' => \false, 'parsed_cache_variant' => null, 'focus_within_classes' => ['focus'], 'low_priority_plugins' => ['query-monitor'], 'allow_transient_caching' => \true, 'skip_tree_shaking' => \false, 'allow_excessive_css' => \false, 'transform_important_qualifiers' => \true, 'font_face_display_overrides' => ['NonBreakingSpaceOverride' => 'optional', 'Inter var' => 'optional', 'Genericons' => 'block']];
    /**
     ...
     * @var array
     */
    private $used_attributes = ['autofocus' => \true, 'checked' => \true, 'controls' => \true, 'disabled' => \true, 'hidden' => \true, 'loop' => \true, 'multiple' => \true, 'readonly' => \true, 'required' => \true, 'selected' => \true];
    /**
    ...
    {
        return [self::CSS_SYNTAX_INVALID_AT_RULE, self::CSS_SYNTAX_INVALID_DECLARATION, self::CSS_SYNTAX_INVALID_PROPERTY, self::CSS_SYNTAX_INVALID_PROPERTY_NOLIST, self::CSS_SYNTAX_INVALID_IMPORTANT, self::CSS_SYNTAX_PARSE_ERROR, self::CSS_DISALLOWED_SELECTOR, self::STYLESHEET_FETCH_ERROR, self::STYLESHEET_TOO_LONG];
    }
    /**
    ...
    {
        $reflection = new \ReflectionClass('Google\\Web_Stories_Dependencies\\Sabberworm\\CSS\\OutputFormat');
        $has_output_format_extensions = $reflection->hasProperty('sBeforeAtRuleBlock') && $reflection->hasProperty('sAfterAtRuleBlock') && $reflection->hasProperty('sBeforeDeclarationBlock') && $reflection->hasProperty('sAfterDeclarationBlockSelectors') && $reflection->hasProperty('sAfterDeclarationBlock');
    ...
    public function __construct($dom, array $args = [])
    {
        ...
        foreach (\Google\Web_Stories_Dependencies\AMP_Allowed_Tags_Generated::get_allowed_tag('style') as $spec_rule) {
            if (!isset($spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['spec_name'])) {
                continue;
            }
            if (self::STYLE_AMP_KEYFRAMES_SPEC_NAME === $spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['spec_name']) {
                $this->style_keyframes_cdata_spec = $spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA];
            } elseif (self::STYLE_AMP_CUSTOM_SPEC_NAME === $spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['spec_name']) {
                $this->style_custom_cdata_spec = $spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA];
            }
        }
        $spec_name = 'link rel=stylesheet for fonts';
        // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedStylesheet
        foreach (\Google\Web_Stories_Dependencies\AMP_Allowed_Tags_Generated::get_allowed_tag('link') as $spec_rule) {
            if (isset($spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['spec_name']) && $spec_name === $spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['spec_name']) {
                $this->allowed_font_src_regex = '/^(' . $spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST]['href']['value_regex'] . ')$/';
                break;
            }
        }
        $guessurl = \site_url();
    ...
    {
        return \wp_list_pluck(\array_filter($this->pending_stylesheets, static function ($pending_stylesheet) {
    ...
    private function get_used_class_names()
    {
        ...
        foreach ($this->dom->xpath->query('//*/@class') as $class_attribute) {
            ...
            $mutated = \false;
            $element_classes = [];
            foreach (\array_filter(\preg_split('/\\s+/', \trim($class_attribute->nodeValue))) as $class_name) {
                if (\substr($class_name, 0, $i_amphtml_prefix_length) === $i_amphtml_prefix) {
                    $error = ['code' => self::DISALLOWED_ATTR_CLASS_NAME, 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::HTML_ATTRIBUTE_ERROR_TYPE, 'class_name' => $class_name];
        ...
        foreach ($this->dom->xpath->query('//*/@' . Amp::BIND_DATA_ATTR_PREFIX . 'class') as $bound_class_attribute) {
            if (\preg_match_all('/([\'"])([^\\1]*?)\\1/', $bound_class_attribute->nodeValue, $matches)) {
                $classes = \array_merge($classes, \preg_split('/\\s+/', \trim(\implode(' ', $matches[2]))));
            }
        }
        $class_names = \array_merge($dynamic_class_names, \array_unique(\array_filter($classes)));
        ...
        foreach ($this->dom->xpath->query('//*/@on[ contains( ., "toggleClass" ) ]') as $on_attribute) {
            if (\preg_match_all('/\\.\\s*toggleClass\\s*\\(\\s*class\\s*=\\s*(([\'"])([^\\1]*?)\\2|[a-zA-Z0-9_\\-]+)/', $on_attribute->nodeValue, $matches)) {
                $class_names = \array_merge($class_names, \array_map(static function ($match) {
                    return \trim($match, '"\'');
                }, $matches[1]));
            }
        }
        ...
        $this->used_class_names = \array_fill_keys($class_names, \true);
        return $this->used_class_names;
    }
    /**
    ...
    {
        static $class_prefixes = ['CalendarDay', 'CalendarMonth', 'CalendarMonthGrid', 'DayPicker', 'DayPickerKeyboardShortcuts', 'DayPickerNavigation', 'KeyboardShortcutRow'];
        return \in_array(\strtok($class, '_'), $class_prefixes, \true);
    }
    /**
    ...
    public function sanitize()
    {
        ...
        foreach ($this->sanitizers as $sanitizer) {
            foreach ($sanitizer->get_selector_conversion_mapping() as $html_selectors => $amp_selectors) {
                if (!isset($this->selector_mappings[$html_selectors])) {
                    ...
                    $this->selector_mappings[$html_selectors] = \array_unique(\array_merge($this->selector_mappings[$html_selectors], $amp_selectors));
                }
                // Prevent selectors like `amp-img img` getting deleted since `img` does not occur in the DOM.
                if ($sanitizer->has_light_shadow_dom()) {
                    $this->args['dynamic_element_selectors'] = \array_merge($this->args['dynamic_element_selectors'], $this->selector_mappings[$html_selectors]);
                }
            }
        }
        $elements = [];
        $this->focus_class_name_selector_pattern = !empty($this->args['focus_within_classes']) ? self::get_class_name_selector_pattern($this->args['focus_within_classes']) : null;
        /*
        ...
        if (DevMode::isActiveForDocument($this->dom)) {
            $dev_mode_predicate = \sprintf(' and not ( @%s )', \Google\Web_Stories_Dependencies\AMP_Rule_Spec::DEV_MODE_ATTRIBUTE);
        }
        $lower_case = 'translate( %s, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz" )';
        // In XPath 2.0 this is lower-case().
        $predicates = [\sprintf('( self::style and not( @amp-boilerplate ) and ( not( @type ) or %s = "text/css" ) %s )', \sprintf($lower_case, '@type'), $dev_mode_predicate), \sprintf('( self::link and @href and %s = "stylesheet" %s )', \sprintf($lower_case, '@rel'), $dev_mode_predicate)];
         ...
         */
        \do_action('amp_server_timing_log', 'amp_parse_css', '', ['dur' => $parse_css_duration * 1000], \true);
        /**
         ...
         *                                    event in verbose mode.
         */
        \do_action('amp_server_timing_log', 'amp_shake_css', '', ['dur' => $shake_css_duration * 1000], \true);
    }
    /**
    ...
    private function get_stylesheet_priority(\DOMNode $node)
    {
        ...
        if ($node instanceof \DOMElement && 'link' === $node->tagName) {
            ...
            $plugin = null;
            if (\preg_match(\sprintf('#^(?:%s|%s)(?<plugin>[^/]+)#i', \preg_quote($remove_url_scheme(\trailingslashit(\WP_PLUGIN_URL)), '#'), \preg_quote($remove_url_scheme(\trailingslashit(\WPMU_PLUGIN_URL)), '#')), $schemeless_href, $matches)) {
            ...
            $core_frontend_handles = ['wp-block-library', 'wp-block-library-theme'];
            $non_amp_handles = ['mediaelement', 'wp-mediaelement', 'thickbox'];
            if (\in_array($style_handle, $non_amp_handles, \true)) {
                ...
                $priority = 90;
            } elseif (\false !== \strpos($schemeless_href, $remove_url_scheme(\trailingslashit(\get_template_directory_uri())))) {
                // Highest priority are parent theme styles.
                $priority = 1;
            } elseif (\false !== \strpos($schemeless_href, $remove_url_scheme(\trailingslashit(\get_stylesheet_directory_uri())))) {
                // Penultimate highest priority are child theme styles.
                $priority = 10;
            } elseif (\in_array($style_handle, $core_frontend_handles, \true)) {
                ...
                $priority = \in_array($plugin, $this->args['low_priority_plugins'], \true) ? 150 : 30;
            ...
            if ($dependency && (0 === \strpos($dependency->src, \get_template_directory_uri()) || 'twenty-twenty-one-style' === $dependency->handle)) {
                // Parent theme inline style.
                $priority = 2;
            } elseif ($dependency && \get_stylesheet() !== \get_template() && 0 === \strpos($dependency->src, \get_stylesheet_directory_uri())) {
    ...
    public function get_validated_url_file_path($url, $allowed_extensions = [])
    {
        ...
        $needs_base_url = !\preg_match('|^(https?:)?//|', $url) && !($this->content_url && 0 === \strpos($url, $this->content_url));
        if ($needs_base_url) {
        ...
        $allowed_hosts = [\wp_parse_url($includes_url, \PHP_URL_HOST), \wp_parse_url($content_url, \PHP_URL_HOST), \wp_parse_url($admin_url, \PHP_URL_HOST)];
        ...
        if (!\in_array($parsed_url['host'], $allowed_hosts, \true)) {
            /* translators: %s: the file URL */
        ...
        } elseif (0 === \strpos($url, $site_url . \trailingslashit($wp_content))) {
    ...
    private function process_style_element(\DOMElement $element)
    {
        ...
        $stylesheet = \trim($element->textContent);
        ...
        $parsed = $this->get_parsed_stylesheet($stylesheet, ['allowed_at_rules' => $cdata_spec['css_spec']['allowed_at_rules'], 'property_allowlist' => $cdata_spec['css_spec']['declaration'], 'validate_keyframes' => $cdata_spec['css_spec']['validate_keyframes'], 'spec_name' => $is_keyframes ? self::STYLE_AMP_KEYFRAMES_SPEC_NAME : self::STYLE_AMP_CUSTOM_SPEC_NAME]);
        if ($parsed['viewport_rules']) {
            $this->create_meta_viewport($element, $parsed['viewport_rules']);
        }
        $this->pending_stylesheets[] = ['group' => $is_keyframes ? self::STYLE_AMP_KEYFRAMES_GROUP_INDEX : self::STYLE_AMP_CUSTOM_GROUP_INDEX, 'original_size' => (int) \strlen($stylesheet), 'final_size' => null, 'element' => $element, 'origin' => 'style_element', 'sources' => $this->current_sources, 'priority' => $this->get_stylesheet_priority($element), 'tokens' => $parsed['tokens'], 'hash' => $parsed['hash'], 'parse_time' => $parsed['parse_time'], 'shake_time' => null, 'cached' => $parsed['cached'], 'imported_font_urls' => $parsed['imported_font_urls'], 'important_count' => $parsed['important_count'], 'kept_error_count' => $parsed['kept_error_count'], 'preload_font_urls' => $parsed['preload_font_urls']];
    ...
    private function process_link_element(\DOMElement $element)
    {
        ...
        if ($this->allowed_font_src_regex && \preg_match($this->allowed_font_src_regex, $normalized_url)) {
             ...
             */
            $needs_preconnect_link = 'https://fonts.googleapis.com/' === \substr($normalized_url, 0, 29) && 0 === $this->dom->xpath->query('//link[ @rel = "preconnect" and @crossorigin and starts-with( @href, "https://fonts.gstatic.com" ) ]', $this->dom->head)->length;
            if ($needs_preconnect_link) {
                $link = \Google\Web_Stories_Dependencies\AMP_DOM_Utils::create_node($this->dom, 'link', ['rel' => 'preconnect', 'href' => 'https://fonts.gstatic.com/', 'crossorigin' => '']);
                $this->dom->head->insertBefore($link);
                // Note that \AMP_Theme_Support::ensure_required_markup() will put this in the optimal order.
            }
            return;
        }
        $stylesheet = $this->get_stylesheet_from_url($href);
        if ($stylesheet instanceof \WP_Error) {
            $this->remove_invalid_child($element, ['code' => self::STYLESHEET_FETCH_ERROR, 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'url' => $normalized_url, 'message' => $stylesheet->get_error_message()]);
            return;
        }
        ...
        // @todo If ValidationExemption::is_px_verified_for_node( $element ) then keep !important.
        // @todo If ValidationExemption::is_amp_unvalidated_for_node( $element ) then keep invalid markup.
        $parsed = $this->get_parsed_stylesheet($stylesheet, ['allowed_at_rules' => $this->style_custom_cdata_spec['css_spec']['allowed_at_rules'], 'property_allowlist' => $this->style_custom_cdata_spec['css_spec']['declaration'], 'stylesheet_url' => $href, 'spec_name' => self::STYLE_AMP_CUSTOM_SPEC_NAME]);
        if ($parsed['viewport_rules']) {
            $this->create_meta_viewport($element, $parsed['viewport_rules']);
        }
        $this->pending_stylesheets[] = [
            ...
            // Needed because node is removed below.
    ...
    private function get_stylesheet_from_url($stylesheet_url)
    {
        ...
        if (!\is_wp_error($css_file_path)) {
            ...
            // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- It's a local filesystem path not a remote request.
        }
    ...
    private function fetch_external_stylesheet($url)
    {
        ...
        if (!empty($content_type) && !\preg_match('#^text/css#', $content_type[0])) {
            return new \WP_Error('no_css_content_type', \__('Response did not contain the expected text/css content type.', 'amp'));
        }
    ...
    {
        $cached = \true;
        $cache_group = 'amp-parsed-stylesheet-v40';
        ...
        $cache_impacting_options = \array_merge(\wp_array_slice_assoc($options, ['property_allowlist', 'property_denylist', 'stylesheet_url', 'allowed_at_rules']), \wp_array_slice_assoc($this->args, ['should_locate_sources', 'parsed_cache_variant', 'dynamic_element_selectors', 'transform_important_qualifiers', 'font_face_display_overrides']), [
            'language' => \get_bloginfo('language'),
            // Used to tree-shake html[lang] selectors.
        ...
        if (!empty($parsed['validation_results'])) {
            foreach ($parsed['validation_results'] as $validation_result) {
                ...
                if ($sanitized !== $validation_result['sanitized']) {
                    $parsed = null;
                    // Change to sanitization of validation error detected, so cache cannot be used.
                    break;
                }
            }
        }
    ...
    private function should_use_transient_caching()
    {
        ...
        if (\Google\Web_Stories_Dependencies\AMP_Options_Manager::get_option(Option::DISABLE_CSS_TRANSIENT_CACHING, \false)) {
            return \false;
        }
        return \true;
    }
    ...
    private function splice_imported_stylesheet(Import $item, CSSList $css_list, $options)
    {
        ...
        $this->processed_imported_stylesheet_urls[$import_stylesheet_url] = \true;
        ...
        if ($this->allowed_font_src_regex && \preg_match($this->allowed_font_src_regex, $https_import_stylesheet_url)) {
            ...
            $css_list->remove($item);
            \_doing_it_wrong('wp_enqueue_style', \esc_html(\sprintf(
        ...
        if ($stylesheet instanceof \WP_Error) {
            $error = ['code' => self::STYLESHEET_FETCH_ERROR, 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'url' => $import_stylesheet_url, 'message' => $stylesheet->get_error_message()];
    ...
    {
        $contents = \array_values($css_list->getContents());
        // Required to obtain the offset instead of the index.
        $offset = \array_search($old_item, $contents, \true);
        if (\false !== $offset) {
    ...
    private function create_validated_css_document($stylesheet_string, $options)
    {
        ...
        try {
            ...
            $parser_settings = \Google\Web_Stories_Dependencies\Sabberworm\CSS\Settings::create();
            $css_parser = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Parser($stylesheet_string, $parser_settings);
            $css_document = $css_parser->parse();
            // @todo If 'utf-8' is not $css_parser->getCharset() then issue warning?
            if (!empty($options['stylesheet_url'])) {
                $this->real_path_urls(\array_filter($css_document->getAllValues(), static function ($value) {
                    return $value instanceof URL;
                }), $options['stylesheet_url']);
            }
            $processed_css_list = $this->process_css_list($css_document, $options);
            $validation_results = \array_merge($validation_results, $processed_css_list['validation_results']);
            $viewport_rules = \array_merge($viewport_rules, $processed_css_list['viewport_rules']);
            ...
            $error = ['code' => self::CSS_SYNTAX_PARSE_ERROR, 'message' => $exception->getMessage(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
            /*
    ...
    private function parse_stylesheet($stylesheet_string, $options = [])
    {
        ...
        $options = \array_merge(['allowed_at_rules' => [], 'property_denylist' => [
            // See <https://www.ampproject.org/docs/design/responsive/style_pages#disallowed-styles>.
            'behavior',
            '-moz-binding',
        ], 'property_allowlist' => [], 'validate_keyframes' => \false, 'stylesheet_url' => null, 'spec_name' => null], $options);
        ...
        if (!empty($parsed_stylesheet['css_document'])) {
            $css_document = $parsed_stylesheet['css_document'];
            $output_format = \Google\Web_Stories_Dependencies\Sabberworm\CSS\OutputFormat::createCompact();
            ...
            if (!empty($this->args['dynamic_element_selectors'])) {
                $dynamic_selector_pattern = \implode('|', \array_map(static function ($selector) {
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                if ($before_declaration_block === $split_stylesheet[$i]) {
                    ...
                    if (\preg_match('/^((from|to)\\b|-?\\d+(\\.\\d+)?%)/i', $split_stylesheet[$i + 1])) {
                        $tokens[] = \str_replace($between_selectors, '', $split_stylesheet[++$i]) . \str_replace($between_properties, '', $split_stylesheet[++$i]);
                        continue;
                    }
                    $selectors = \explode($between_selectors . ',', $split_stylesheet[++$i]);
                    $declaration = \explode(';' . $between_properties, \trim($split_stylesheet[++$i], '{}'));
                    ...
                    foreach ($selectors as $selector) {
                         ...
                         */
                        $reduced_selector = \preg_replace_callback('/\\[([A-Za-z0-9_:-]+)(\\W?=[^\\]]+)?\\]/', static function ($matches) use($selector, &$selectors_parsed) {
                        ...
                        // Extract IDs.
                        $reduced_selector = \preg_replace_callback('/#([a-zA-Z0-9_-]+)/', static function ($matches) use($selector, &$selectors_parsed) {
                    ...
                    $tokens[] = [$selectors_parsed, $declaration];
                } else {
                    $tokens[] = \str_replace($between_properties, '', $split_stylesheet[$i]);
                }
            }
        }
        return \array_merge(\compact('tokens', 'validation_results'), ['imported_font_urls' => $parsed_stylesheet['imported_font_urls'], 'hash' => \md5(\wp_json_encode($tokens)), 'parse_time' => \microtime(\true) - $start_time, 'viewport_rules' => $parsed_stylesheet['viewport_rules'], 'important_count' => $parsed_stylesheet['important_count'], 'preload_font_urls' => $parsed_stylesheet['preload_font_urls']]);
    }
    /**
    ...
    private function process_css_list(CSSList $css_list, $options)
    {
        ...
        foreach ($css_list->getContents() as $css_item) {
            ...
            if ($css_item instanceof DeclarationBlock && empty($options['validate_keyframes'])) {
                ...
                if (!\in_array($css_item->atRuleName(), $options['allowed_at_rules'], \true)) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_AT_RULE, 'at_rule' => $css_item->atRuleName(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    $validation_results[] = \compact('error', 'sanitized');
                }
                if (!$sanitized) {
                ...
                } elseif (!\in_array($css_item->atRuleName(), $options['allowed_at_rules'], \true)) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_AT_RULE, 'at_rule' => $css_item->atRuleName(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    $validation_results[] = \compact('error', 'sanitized');
                }
                ...
                if (!\in_array('keyframes', $options['allowed_at_rules'], \true)) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_AT_RULE, 'at_rule' => $css_item->atRuleName(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    $validation_results[] = \compact('error', 'sanitized');
                }
                ...
                $error = ['code' => self::CSS_SYNTAX_INVALID_DECLARATION, 'item' => \get_class($css_item), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                $sanitized = $this->should_sanitize_validation_error($error);
                $validation_results[] = \compact('error', 'sanitized');
            }
            if ($sanitized) {
    ...
    private function process_css_declaration_block(RuleSet $ruleset, CSSList $css_list, $options)
    {
        ...
        if ($ruleset instanceof DeclarationBlock) {
            $validation_results = \array_merge($validation_results, $this->ampify_ruleset_selectors($ruleset));
        ...
        if (!empty($options['property_allowlist'])) {
            ...
            foreach ($properties as $property) {
                ...
                if (!\in_array($vendorless_property_name, $options['property_allowlist'], \true)) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_PROPERTY, 'css_property_name' => $property->getRule(), 'css_property_value' => $property->getValue(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    if ($sanitized) {
            ...
            foreach ($options['property_denylist'] as $illegal_property_name) {
                ...
                foreach ($properties as $property) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_PROPERTY_NOLIST, 'css_property_name' => $property->getRule(), 'css_property_value' => (string) $property->getValue(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    if ($sanitized) {
                        $ruleset->removeRule($property->getRule());
                    }
        ...
        $validation_results = \array_merge($validation_results, $transformed['validation_results']);
    ...
    private function process_font_face_at_rule(AtRuleSet $ruleset, $options)
    {
        ...
        if ($property instanceof Rule) {
            $font_family = \trim($property->getValue(), '"\'');
            // Remove all non-word characters from the font family to serve as the filename.
            $font_basename = \preg_replace('/[^A-Za-z0-9_\\-]/', '', $font_family);
            // Same as sanitize_key() minus case changes.
        }
        // Obtain the stylesheet base URL from which to guess font file locations.
        $stylesheet_base_url = null;
        if (!empty($options['stylesheet_url'])) {
            $stylesheet_base_url = \preg_replace(':[^/]+(\\?.*)?(#.*)?$:', '', $options['stylesheet_url']);
            $stylesheet_base_url = \trailingslashit($stylesheet_base_url);
        }
        ...
        foreach ($src_properties as $src_property) {
            ...
            foreach ($sources as $source) {
                ...
                if (!$url instanceof URL || !$format instanceof CSSFunction || $format->getName() !== 'format' || \count($format->getArguments()) !== 1) {
                    continue;
                }
                list($format_value) = $format->getArguments();
                $format_value = \trim($format_value, '"\'');
            ...
            foreach ($source_data_url_objects as $format => $data_url) {
                ...
                foreach ($source_file_urls as $source_file_url) {
                    $guessed_url = \preg_replace(
                        ':(?<=\\.)\\w+(\\?.*)?(#.*)?$:',
                        // Match the file extension in the URL.
                        $extension,
                        $source_file_url,
                        1,
                        $count
                ...
                $bundled_fonts = ['nonbreakingspaceoverride.woff', 'nonbreakingspaceoverride.woff2', 'genericons.woff'];
                if (\in_array($font_filename, $bundled_fonts, \true)) {
        ...
        if ($font_family && \in_array($font_family, \array_keys($this->args['font_face_display_overrides']), \true)) {
        ...
        $property = \end($properties);
        // Last since the last property wins in CSS.
        /** @var RuleValueList|string|null */
        $property_value = $property instanceof Rule ? $property->getValue() : '';
        if ((!$property instanceof Rule || \in_array($property_value, ['auto', 'block', 'swap'], \true)) && 'file' === $first_src_type && !empty($font_file)) {
    ...
    private function process_css_keyframes(KeyFrame $css_list, $options)
    {
        ...
        foreach ($css_list->getContents() as $rules) {
            if (!$rules instanceof DeclarationBlock) {
                $error = ['code' => self::CSS_SYNTAX_INVALID_DECLARATION, 'item' => \get_class($rules), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                $sanitized = $this->should_sanitize_validation_error($error);
                if ($sanitized) {
            ...
            if (!empty($options['property_allowlist'])) {
                ...
                foreach ($properties as $property) {
                    $vendorless_property_name = \preg_replace('/^-\\w+-/', '', $property->getRule());
                    if (!\in_array($vendorless_property_name, $options['property_allowlist'], \true)) {
                        $error = ['code' => self::CSS_SYNTAX_INVALID_PROPERTY, 'css_property_name' => $property->getRule(), 'css_property_value' => (string) $property->getValue(), 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                        $sanitized = $this->should_sanitize_validation_error($error);
                        if ($sanitized) {
    ...
    private function transform_important_qualifiers(RuleSet $ruleset, CSSList $css_list, $options)
    {
        ...
        $allow_transformation = $ruleset instanceof DeclarationBlock && !$css_list instanceof KeyFrame;
        $properties = $ruleset->getRules();
        $importants = [];
        foreach ($properties as $property) {
            ...
            if (!$this->args['transform_important_qualifiers']) {
                ...
                $error = ['code' => self::CSS_SYNTAX_INVALID_IMPORTANT, 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'css_property_name' => $property->getRule(), 'css_property_value' => $property->getValue(), 'spec_name' => $options['spec_name']];
                $sanitized = $this->should_sanitize_validation_error($error);
                if ($sanitized) {
                    $property->setIsImportant(\false);
                } else {
        ...
        $important_ruleset->setSelectors(\array_map(
            ...
            static function (Selector $old_selector) {
                ...
                $specificity_multiplier = \Google\Web_Stories_Dependencies\AMP_Style_Sanitizer::INLINE_SPECIFICITY_MULTIPLIER + 1 + \floor($old_selector->getSpecificity() / 100);
                ...
                if ('#_)' === \substr($new_selector, -3)) {
                    ...
                    $new_selector .= ':not(' . \str_repeat('#_', $specificity_multiplier) . ')';
                    // Here "_" is just a short single-char ID.
                }
        ...
        $contents = \array_values($css_list->getContents());
        // Ensure keys are 0-indexed and sequential.
        $offset = \array_search($ruleset, $contents, \true);
        if (\false !== $offset) {
    ...
    {
        $preload_font_urls = [];
        $stylesheet_groups = [self::STYLE_AMP_CUSTOM_GROUP_INDEX => [
            'source_map_comment' => "\n\n/*# sourceURL=amp-custom.css */",
            'cdata_spec' => $this->style_custom_cdata_spec,
            'included_count' => 0,
            'import_front_matter' => '',
            // Extra @import statements that are prepended when fetch fails and validation error is rejected.
            'important_count' => 0,
            'kept_error_count' => 0,
            'is_excessive_size' => \false,
            'preload_font_urls' => [],
        ], self::STYLE_AMP_KEYFRAMES_GROUP_INDEX => ['source_map_comment' => "\n\n/*# sourceURL=amp-keyframes.css */", 'cdata_spec' => $this->style_keyframes_cdata_spec, 'included_count' => 0, 'import_front_matter' => '', 'important_count' => 0, 'kept_error_count' => 0, 'is_excessive_size' => \false, 'preload_font_urls' => []]];
        $imported_font_urls = [];
        ...
        foreach ($this->pending_stylesheets as $i => $pending_stylesheet) {
            foreach ($pending_stylesheet['tokens'] as $j => $part) {
                if (\is_string($part) && 0 === \strpos($part, '@import')) {
                    ...
                    // @todo Not currently relayed in stylesheet data.
        ...
        foreach (\array_keys($stylesheet_groups) as $group) {
            $stylesheet_groups[$group] = \array_merge($stylesheet_groups[$group], $this->finalize_stylesheet_group($group, $stylesheet_groups[$group]));
        }
        ...
        if ($stylesheet_groups[self::STYLE_AMP_CUSTOM_GROUP_INDEX]['included_count'] > 0) {
            ...
            } elseif ($stylesheet_groups[self::STYLE_AMP_CUSTOM_GROUP_INDEX]['important_count'] > 0 || $stylesheet_groups[self::STYLE_AMP_CUSTOM_GROUP_INDEX]['is_excessive_size']) {
        ...
        if ($stylesheet_groups[self::STYLE_AMP_KEYFRAMES_GROUP_INDEX]['included_count'] > 0) {
            ...
            $css .= \implode('', \wp_list_pluck(\array_filter($this->pending_stylesheets, static function ($pending_stylesheet) {
    ...
    private function remove_admin_bar_if_css_excluded()
    {
        ...
        $included = \true;
        foreach ($this->pending_stylesheets as &$pending_stylesheet) {
            $is_admin_bar_css = self::STYLE_AMP_CUSTOM_GROUP_INDEX === $pending_stylesheet['group'] && 'admin-bar-css' === $pending_stylesheet['element']->getAttribute('id');
            if ($is_admin_bar_css) {
        ...
        if (!$included) {
            ...
            if ($this->dom->body->hasAttribute('class')) {
                $this->dom->body->setAttribute('class', \preg_replace('/(^|\\s)admin-bar(\\s|$)/', ' ', $this->dom->body->getAttribute('class')));
            }
            ...
            $admin_bar->parentNode->replaceChild($this->dom->createComment(' ' . $comment_text . ' '), $admin_bar);
        }
    }
    /**
    ...
    public function get_validate_response_data()
    {
        ...
        foreach ($this->pending_stylesheets as $pending_stylesheet) {
            ...
            $pending_stylesheet['element'] = ['name' => $pending_stylesheet['element']->nodeName, 'attributes' => $attributes];
    ...
    public function add_css_budget_to_admin_bar()
    {
         ...
         */
        $stylesheets_li_element = $validity_li_element->cloneNode(\true);
        ...
        $stylesheets_a_element->setAttribute('href', $stylesheets_a_element->getAttribute('href') . '#amp_stylesheets');
    ...
    private function ampify_ruleset_selectors($ruleset)
    {
        ...
        foreach ($ruleset->getSelectors() as $old_selector) {
            ...
            if (\preg_match('/(^|\\W)i-amphtml-/', $selector)) {
                $error = ['code' => self::CSS_DISALLOWED_SELECTOR, 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'css_selector' => $selector];
                $sanitized = $this->should_sanitize_validation_error($error);
                $results[] = \compact('error', 'sanitized');
                if ($sanitized) {
                    $has_changed_selectors = \true;
                    continue;
                }
            }
            ...
            $is_other_language_root = \preg_match('/^html\\[lang(?P<starts_with>\\^)?=([\'"]?)(?P<lang>.+?)\\2\\]/', \strtolower($selector), $matches) && (empty($matches['starts_with']) ? $language !== $matches['lang'] : \substr($language, 0, \strlen($matches['lang'])) !== $matches['lang']);
            ...
            if (\preg_match('/:lang\\((?P<languages>.+?)\\)/', $selector, $matches)) {
                ...
                $selector_languages = \array_map(static function ($selector_language) {
                    return \trim($selector_language, '"\'');
                }, \preg_split('/\\s*,\\s*/', \strtolower(\trim($matches['languages']))));
                foreach ($selector_languages as $selector_language) {
                     ...
                     * N: en-US && en-UK
                     */
                    if (\substr($language, 0, \strlen($selector_language)) === $selector_language || \substr($selector_language, 0, \strlen($language)) === $language) {
                        $has_matching_language = \true;
                        break;
                    }
                }
            ...
            if ($this->focus_class_name_selector_pattern) {
                $count = 0;
                $selector = \preg_replace_callback($this->focus_class_name_selector_pattern, static function ($matches) {
                    $replacement = ':focus-within';
                    if ('focus' === $matches['class'] && (!empty($matches['beginning']) || !empty($matches['combinator']) && '' === \trim($matches['combinator']))) {
                        /*
            ...
            if ($this->args['transform_important_qualifiers']) {
                $selector = \preg_replace('/(?<=\\[)style(?=([*$~]?=.*?)?])/is', self::ORIGINAL_STYLE_ATTRIBUTE_NAME, $selector, -1, $count);
                if ($count > 0) {
                    $has_changed_selectors = \true;
                }
            }
            ...
            foreach ($this->selector_mappings as $html_tag => $amp_tags) {
                 ...
                 */
                for ($i = 0; $i < \count($edited_selectors); $i++) {
                    ...
                    if (\preg_match($amp_tag_pattern, $edited_selectors[$i], $matches) && \in_array($matches[0], $amp_tags, \true)) {
                        continue;
                    }
                    ...
                    foreach (\array_slice($amp_tags, 1) as $amp_tag) {
                        // Note: This array contains only a couple items.
    ...
    {
        $class_pattern = \implode('|', \array_map(static function ($class_name) {
    ...
    private function finalize_stylesheet_group($group, $group_config)
    {
        ...
        foreach ($this->pending_stylesheets as $pending_stylesheet_index => &$pending_stylesheet) {
            ...
            foreach ($pending_stylesheet['tokens'] as $token) {
                ...
                foreach ($selectors_parsed as $selector => $parsed_selector) {
                    $should_include = $this->args['skip_tree_shaking'] || (empty($parsed_selector[self::SELECTOR_EXTRACTED_CLASSES]) || $this->has_used_class_name($parsed_selector[self::SELECTOR_EXTRACTED_CLASSES])) && (empty($parsed_selector[self::SELECTOR_EXTRACTED_IDS]) || 0 === \count(\array_filter($parsed_selector[self::SELECTOR_EXTRACTED_IDS], function ($id) {
                        return !$this->dom->getElementById($id);
                    }))) && (empty($parsed_selector[self::SELECTOR_EXTRACTED_TAGS]) || $this->has_used_tag_names($parsed_selector[self::SELECTOR_EXTRACTED_TAGS])) && (empty($parsed_selector[self::SELECTOR_EXTRACTED_ATTRIBUTES]) || $this->has_used_attributes($parsed_selector[self::SELECTOR_EXTRACTED_ATTRIBUTES]));
                    $selectors[$selector] = $should_include;
                    if ($should_include) {
                        $used_selector_count++;
                    }
                }
                $shaken_tokens[] = [0 !== $used_selector_count, $selectors, $declaration_block];
            }
            ...
            for ($i = 0; $i < $stylesheet_part_count; $i++) {
                ...
                if ('{' === \substr($shaken_tokens[$i][1], -1)) {
                    ...
                    for ($j = $i + 1; $j < $stylesheet_part_count; $j++) {
                        if (\is_array($shaken_tokens[$j][1])) {
                            // Is declaration block.
                            if (\true === $shaken_tokens[$j][0]) {
                        ...
                        if ($is_at_rule && '{}' === \substr($shaken_tokens[$j][1], -2)) {
                            continue;
                            // The rule opened is empty from the start.
                        }
                        ...
                        if (0 === $open_braces) {
                            ...
                            $i = $j;
                            // Jump the outer loop ahead to skip over what has been already marked as removed.
                            continue 2;
                        }
                    }
                }
            ...
            $pending_stylesheet['serialized'] = \implode('', \array_map(
                ...
                \array_filter($pending_stylesheet['shaken_tokens'], static function ($shaken_token) {
                    return \false !== $shaken_token[0];
                })
            ));
            $pending_stylesheet['included'] = null;
            ...
            if (isset($previously_seen_stylesheet_index[$pending_stylesheet['hash']])) {
                ...
                $this->pending_stylesheets[$previously_seen_stylesheet_index[$pending_stylesheet['hash']]]['duplicate'] = \true;
            }
            $previously_seen_stylesheet_index[$pending_stylesheet['hash']] = $pending_stylesheet_index;
            $pending_stylesheet['shake_time'] = \microtime(\true) - $start_time;
        }
        ...
        \usort($pending_stylesheet_indices, function ($a, $b) {
        ...
        foreach ($pending_stylesheet_indices as $i) {
            ...
            if (0 === $this->pending_stylesheets[$i]['final_size']) {
                $this->pending_stylesheets[$i]['included'] = \true;
                continue;
            }
            ...
            if (!$this->args['allow_excessive_css'] && $is_stylesheet_excessive) {
                $validation_error = ['code' => self::STYLESHEET_TOO_LONG, 'type' => \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => self::STYLE_AMP_KEYFRAMES_GROUP_INDEX === $group ? self::STYLE_AMP_KEYFRAMES_SPEC_NAME : self::STYLE_AMP_CUSTOM_SPEC_NAME];
                if (isset($this->pending_stylesheets[$i]['sources'])) {
                    $validation_error['sources'] = $this->pending_stylesheets[$i]['sources'];
                }
                $data = ['node' => $this->pending_stylesheets[$i]['element']];
    ...
    private function create_meta_viewport(\DOMElement $element, $viewport_rules)
    {
        ...
        $viewport_meta->setAttribute('content', \implode(',', \array_map(static function ($property_name) use($viewport_rules) {
