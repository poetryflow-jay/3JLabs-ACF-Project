 ...
 * @internal
 */
class AMP_Tag_And_Attribute_Sanitizer extends \Google\Web_Stories_Dependencies\AMP_Base_Sanitizer
{
    ...
    public function __construct($dom, $args = [])
    {
        ...
        foreach (\Google\Web_Stories_Dependencies\AMP_Rule_Spec::$additional_allowed_tags as $tag_name => $tag_rule_spec) {
        ...
        if (!empty($this->args['use_document_element'])) {
            foreach ($this->allowed_tags['html'] as &$rule_spec) {
                unset($rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT]);
            }
            unset($rule_spec);
        }
        foreach ($this->allowed_tags as &$tag_specs) {
            foreach ($tag_specs as &$tag_spec) {
                if (isset($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST])) {
                    $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST] = $this->process_alternate_names($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST]);
                }
            }
            unset($tag_spec);
        }
    ...
    {
        return \array_fill_keys(\array_unique($this->script_components), \true);
    }
    /**
    ...
    private function process_alternate_names($attr_spec_list)
    {
        foreach ($attr_spec_list as $attr_name => &$attr_spec) {
            ...
            if (isset($attr_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES])) {
                foreach ($attr_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES] as $alternative_name) {
    ...
    private function get_rule_spec_list_to_validate(\DOMElement $node, $rule_spec)
    {
        ...
        if (isset($rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['extension_spec'])) {
            $extension_spec = $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['extension_spec'];
            ...
            $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST][$custom_attr] = [\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE => $extension_spec['name'], \Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY => \true];
            $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST]['src'] = [\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX => \implode('', ['^', \preg_quote('https://cdn.ampproject.org/v0/' . $extension_spec['name'] . '-', '/'), '(' . \implode('|', \array_merge($extension_spec['version'], ['latest'])) . ')', '\\.js$'])];
        }
        ...
        if (!empty($node->parentNode) && isset($this->allowed_tags[$node->parentNode->nodeName])) {
            foreach ($this->allowed_tags[$node->parentNode->nodeName] as $parent_rule_spec) {
                if (empty($parent_rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['reference_points'])) {
                    continue;
                }
                foreach ($parent_rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['reference_points'] as $reference_point_spec_name => $reference_point_spec_instance_attrs) {
                    $reference_point = \Google\Web_Stories_Dependencies\AMP_Allowed_Tags_Generated::get_reference_point_spec($reference_point_spec_name);
                    if (empty($reference_point[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST])) {
                        /*
                    ...
                    foreach ($reference_point[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST] as $attr_name => $reference_point_spec_attr) {
                        $reference_point_spec_attr = \array_merge($reference_point_spec_attr, $reference_point_spec_instance_attrs);
                        /*
                        ...
                        $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST][$attr_name] = $reference_point_spec_attr;
                    }
                }
            }
        }
    ...
    private function process_node(\DOMElement $node)
    {
        ...
        foreach ($rule_spec_list as $id => $rule_spec) {
            ...
            if (\true === $validity) {
                ...
                $validation_errors[] = \array_merge($validity, ['spec_name' => $this->get_spec_name($node, $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC])]);
            }
        }
        ...
        if (empty($rule_spec_list_to_validate)) {
            if (1 === \count($validation_errors)) {
                ...
                $this->remove_invalid_child($node, $validation_errors[0]);
            } else {
                $spec_names = \wp_list_pluck($validation_errors, 'spec_name');
                $unique_validation_error_count = \count(\array_unique(\array_map(static function ($validation_error) {
                    unset($validation_error['spec_name'], $validation_error['required_parent_name'], $validation_error['required_ancestor_name'], $validation_error['required_child_count'], $validation_error['required_min_child_count'], $validation_error['required_attr_value']);
                    return $validation_error;
                }, $validation_errors), \SORT_REGULAR));
                if (1 === $unique_validation_error_count) {
                    ...
                    $this->remove_invalid_child($node, \array_merge($validation_error, \compact('spec_names')));
                } else {
                    // Otherwise, we have a rare condition where multiple tag specs fail for different reasons.
                    foreach ($validation_errors as $validation_error) {
                        if (\true === $this->remove_invalid_child($node, $validation_error)) {
                            break;
                            // Once removed, ignore remaining errors.
                        }
                    }
                }
            }
        ...
        if (1 === \count($rule_spec_list_to_validate)) {
            ...
            $attr_spec_list = $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST];
            $tag_spec = $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC];
            if (isset($rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA])) {
                $cdata = $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA];
            }
        } else {
            /*
            ...
            foreach ($rule_spec_list_to_validate as $spec_id => $rule_spec) {
                $attr_spec_scores[$spec_id] = $this->validate_attr_spec_list_for_node($node, $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST]);
            }
            ...
            $spec_ids_sorted = \array_keys($attr_spec_scores, \max($attr_spec_scores), \true);
            // If there is exactly one attr_spec with a max score, use that one.
            if (1 === \count($spec_ids_sorted)) {
                $attr_spec_list = $rule_spec_list_to_validate[$spec_ids_sorted[0]][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST];
                $tag_spec = $rule_spec_list_to_validate[$spec_ids_sorted[0]][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC];
                if (isset($rule_spec_list_to_validate[$spec_ids_sorted[0]][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA])) {
                    $cdata = $rule_spec_list_to_validate[$spec_ids_sorted[0]][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA];
                }
            } else {
                ...
                foreach ($spec_ids_sorted as $id) {
                    $attr_spec_list = \array_merge($attr_spec_list, $rule_spec_list_to_validate[$id][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST]);
                    $tag_spec = \array_merge($tag_spec, $rule_spec_list_to_validate[$id][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]);
                    if (isset($rule_spec_list_to_validate[$id][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA])) {
                        $cdata = \array_merge($cdata, $rule_spec_list_to_validate[$id][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CDATA]);
                    }
                }
                $first_spec = \reset($rule_spec_list_to_validate);
                if (empty($attr_spec_list) && isset($first_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST])) {
                    $attr_spec_list = $first_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST];
                }
            }
        }
        $attr_spec_list = \array_merge($this->globally_allowed_attributes, $attr_spec_list);
        // Remove element if it has illegal CDATA.
        if (!empty($cdata)) {
            $validity = $this->validate_cdata_for_node($node, $cdata);
            if (\true !== $validity) {
                $sanitized = $this->remove_invalid_child($node, \array_merge($validity, ['spec_name' => $this->get_spec_name($node, $tag_spec)]));
        ...
        if (isset($tag_spec['amp_layout'])) {
            ...
            if (isset($tag_spec['amp_layout']['supported_layouts'])) {
                ...
                $attr_spec_list['layout'][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX_CASEI] = '(' . \implode('|', $layouts) . ')';
            }
        }
        ...
        if (!empty($tag_spec['unique'])) {
            ...
            if (!empty($this->visited_unique_tag_specs[$node->nodeName][$tag_spec_key])) {
                $removed = $this->remove_invalid_child($node, ['code' => self::DUPLICATE_UNIQUE_TAG, 'spec_name' => $this->get_spec_name($node, $tag_spec)]);
            }
            $this->visited_unique_tag_specs[$node->nodeName][$tag_spec_key] = \true;
            if ($removed) {
                return null;
            }
        }
        ...
        if (!empty($disallowed_attributes)) {
            ...
            foreach ($disallowed_attributes as $disallowed_attribute) {
                ...
                $validation_error = ['code' => $error_code, 'element_attributes' => $element_attributes];
                if (self::DISALLOWED_PROPERTY_IN_ATTR_VALUE === $error_code) {
                    ...
                    $validation_error['meta_property_required_value'] = $error_data['required_value'];
                    if ($this->should_sanitize_validation_error($validation_error, ['node' => $attr_node])) {
                        $properties = \array_merge($this->parse_properties_attribute($attr_node->nodeValue), [$error_data['name'] => $error_data['required_value']]);
                        $node->setAttribute($attr_node->nodeName, $this->serialize_properties_attribute($properties));
                    }
                } else {
        ...
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DESCENDANT_TAG_LIST])) {
            $allowed_tags = \Google\Web_Stories_Dependencies\AMP_Allowed_Tags_Generated::get_descendant_tag_list($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DESCENDANT_TAG_LIST]);
        ...
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::SIBLINGS_DISALLOWED])) {
        ...
        if (!empty($missing_mandatory_attributes)) {
            $sanitized = $this->remove_invalid_child($node, ['code' => self::ATTR_REQUIRED_BUT_MISSING, 'attributes' => $missing_mandatory_attributes, 'spec_name' => $this->get_spec_name($node, $tag_spec)]);
            return $sanitized ? null : $this->get_required_script_components($node, $tag_spec, $attr_spec_list);
        }
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ANYOF])) {
            $anyof_attributes = $this->get_element_attribute_intersection($node, $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ANYOF]);
            if (0 === \count($anyof_attributes)) {
                $sanitized = $this->remove_invalid_child($node, [
                    'code' => self::MANDATORY_ANYOF_ATTR_MISSING,
                    'mandatory_anyof_attrs' => $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ANYOF],
                    // @todo Temporary as value can be looked up via spec name. See https://github.com/ampproject/amp-wp/pull/3817.
                    'spec_name' => $this->get_spec_name($node, $tag_spec),
                ]);
                return $sanitized ? null : $this->get_required_script_components($node, $tag_spec, $attr_spec_list);
            }
        }
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ONEOF])) {
            $oneof_attributes = $this->get_element_attribute_intersection($node, $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ONEOF]);
            if (0 === \count($oneof_attributes)) {
                $sanitized = $this->remove_invalid_child($node, [
                    'code' => self::MANDATORY_ONEOF_ATTR_MISSING,
                    'mandatory_oneof_attrs' => $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ONEOF],
                    // @todo Temporary as value can be looked up via spec name. See https://github.com/ampproject/amp-wp/pull/3817.
                    'spec_name' => $this->get_spec_name($node, $tag_spec),
                ]);
                return $sanitized ? null : $this->get_required_script_components($node, $tag_spec, $attr_spec_list);
            } elseif (\count($oneof_attributes) > 1) {
                $sanitized = $this->remove_invalid_child($node, ['code' => self::DUPLICATE_ONEOF_ATTRS, 'duplicate_oneof_attrs' => $oneof_attributes, 'spec_name' => $this->get_spec_name($node, $tag_spec)]);
                return $sanitized ? null : $this->get_required_script_components($node, $tag_spec, $attr_spec_list);
            }
        }
    ...
    private function get_required_script_components(\DOMElement $node, $tag_spec, $attr_spec_list)
    {
        ...
        if (!\in_array('amp-bind', $this->script_components, \true)) {
    ...
    private function get_missing_mandatory_attributes($attr_spec, \DOMElement $node)
    {
        ...
        foreach ($attr_spec as $attr_name => $attr_spec_rule_value) {
            if (empty($attr_spec_rule_value[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY])) {
                continue;
            }
            if ('\\u' === \substr($attr_name, 0, 2)) {
                $attr_name = \html_entity_decode('&#x' . \substr($attr_name, 2) . ';');
                // Probably ⚡.
            }
            if (!$node->hasAttribute($attr_name) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_mandatory($node, $attr_name, $attr_spec_rule_value)) {
    ...
    {
        if (isset($cdata_spec['max_bytes']) && \strlen($element->textContent) > $cdata_spec['max_bytes'] && !('style' === $element->nodeName && $element->hasAttribute('amp-custom'))) {
            return ['code' => self::CDATA_TOO_LONG];
        }
    ...
    {
        static $possible_json_errors = ['JSON_ERROR_CTRL_CHAR', 'JSON_ERROR_DEPTH', 'JSON_ERROR_STATE_MISMATCH', 'JSON_ERROR_SYNTAX', 'JSON_ERROR_UTF8'];
    ...
    {
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT])) {
            $missing_required_parent = \false;
            if ('subscriptions-section content swg_amp_cache_nonce' === $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT]) {
                // Special case for mandatory parent name which doesn't follow the pattern where mandatory_parent is just a tag name.
                if (!($this->has_parent($node, 'section') && $node->parentNode instanceof \DOMElement && $node->parentNode->getAttribute('subscriptions-section') === 'content' && $node->parentNode->hasAttribute('swg_amp_cache_nonce'))) {
                    $missing_required_parent = \true;
                }
            } elseif (!$this->has_parent($node, $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT])) {
                $missing_required_parent = \true;
            }
            if ($missing_required_parent) {
                return ['code' => self::WRONG_PARENT_TAG, 'required_parent_name' => $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT]];
            }
        }
        ...
        if (isset($tag_spec['extension_spec']) && !$this->has_parent($node, 'head')) {
            return ['code' => self::WRONG_PARENT_TAG, 'required_parent_name' => 'head'];
        }
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DISALLOWED_ANCESTOR])) {
            foreach ($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DISALLOWED_ANCESTOR] as $disallowed_ancestor_node_name) {
                if ($this->has_ancestor($node, $disallowed_ancestor_node_name)) {
                    return ['code' => self::DISALLOWED_TAG_ANCESTOR, 'disallowed_ancestor' => $disallowed_ancestor_node_name];
                }
            }
        }
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ANCESTOR]) && !$this->has_ancestor($node, $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ANCESTOR])) {
            return ['code' => self::MANDATORY_TAG_ANCESTOR, 'required_ancestor_name' => $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_ANCESTOR]];
        }
        if (empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CHILD_TAGS])) {
            return \true;
        }
        $validity = $this->check_valid_children($node, $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::CHILD_TAGS]);
        if (\true !== $validity) {
    ...
    private function validate_attr_spec_list_for_node(\DOMElement $node, $attr_spec_list)
    {
        ...
        if (!$node->hasAttributes()) {
            foreach ($attr_spec_list as $attr_name => $attr_spec_rule) {
                if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY])) {
                    return 0;
                }
            }
            return 1;
        ...
        foreach ($node->attributes as $attr_name => $attr_node) {
            if (!isset($attr_spec_list[$attr_name][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES])) {
                continue;
            }
            foreach ($attr_spec_list[$attr_name][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES] as $attr_alt_name) {
                ...
                $attr_spec_list[$attr_alt_name][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES] = [$attr_name];
            }
        }
        $score = 0;
        /*
        ...
        foreach ($attr_spec_list as $attr_name => $attr_spec_rule) {
            ...
            if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY])) {
                ...
                if (\Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS === $result) {
                    $score += 2;
                } elseif (\Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $result) {
                    return 0;
                }
            }
            /*
             ...
             */
            if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE])) {
             ...
             * the rule to pass.
             */
            if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_CASEI])) {
    ...
    private function get_spec_name(\DOMElement $element, $tag_spec)
    {
        if (isset($tag_spec['spec_name'])) {
            ...
            return \sprintf('script[%s=%s]', 'amp-mustache' === $tag_spec['extension_spec']['name'] ? 'custom-template' : 'custom-element', \strtolower($tag_spec['extension_spec']['name']));
        } else {
    ...
    private function sanitize_disallowed_attribute_values_in_node(\DOMElement $node, $attr_spec_list)
    {
        ...
        foreach ($node->attributes as $attr_name => $attr_node) {
             ...
             */
            if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_CASEI]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value_casei($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE_CASEI, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value_regex($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE_REGEX, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX_CASEI]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value_regex_casei($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE_REGEX_CASEI, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOWED_PROTOCOL]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_allowed_protocol($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_URL_PROTOCOL, null];
                // @todo A javascript: protocol could be treated differently. It should have a JS error type.
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_valid_url($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_URL, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_disallowed_empty($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::MISSING_URL, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOW_RELATIVE]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_disallowed_relative($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::DISALLOWED_RELATIVE_URL, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DISALLOWED_VALUE_REGEX]) && \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_disallowed_value_regex($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_DISALLOWED_VALUE_REGEX, null];
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_PROPERTIES])) {
                $result = $this->check_attr_spec_rule_value_properties($node, $attr_name, $attr_spec_rule);
                if (\Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL === $result[0]) {
    ...
    private function is_valid_layout($tag_spec, $node)
    {
        ...
        if (!$input_width->isValid()) {
            return ['code' => self::INVALID_LAYOUT_WIDTH, 'attribute' => Attribute::WIDTH];
        }
        ...
        if (!$input_height->isValid()) {
            return ['code' => self::INVALID_LAYOUT_HEIGHT, 'attribute' => Attribute::HEIGHT];
        }
        ...
        if (!$this->supports_layout($tag_spec, $layout)) {
             ...
             */
            if (!$layout_attr && Layout::CONTAINER === $layout && $this->supports_layout($tag_spec, Layout::RESPONSIVE)) {
                return ['code' => self::MISSING_LAYOUT_ATTRIBUTES, 'node_name' => $node->tagName];
            }
            return ['code' => !$layout_attr ? self::IMPLIED_LAYOUT_INVALID : self::SPECIFIED_LAYOUT_INVALID, 'layout' => $layout, 'node_name' => $node->tagName];
        }
        ...
        if ($height->isAuto() && Layout::FLEX_ITEM !== $layout) {
            return ['code' => self::INVALID_LAYOUT_AUTO_HEIGHT, 'attribute' => Attribute::HEIGHT];
        }
        // FIXED, FIXED_HEIGHT, INTRINSIC, RESPONSIVE must have height set.
        if (\in_array($layout, [Layout::FIXED, Layout::FIXED_HEIGHT, Layout::INTRINSIC, Layout::RESPONSIVE], \true) && !$height->isDefined()) {
            return ['code' => self::INVALID_LAYOUT_NO_HEIGHT, 'attribute' => Attribute::HEIGHT];
        }
        // For FIXED_HEIGHT if width is set it must be auto.
        if (Layout::FIXED_HEIGHT === $layout && $width->isDefined() && !$width->isAuto()) {
            return ['code' => self::INVALID_LAYOUT_FIXED_HEIGHT, 'attribute' => Attribute::WIDTH, 'required_attr_value' => CssLength::AUTO];
        }
        // FIXED, INTRINSIC, RESPONSIVE must have width set and not be auto.
        if (\in_array($layout, [Layout::FIXED, Layout::INTRINSIC, Layout::RESPONSIVE], \true)) {
            if (!$width->isDefined() || $width->isAuto()) {
                return ['code' => self::INVALID_LAYOUT_AUTO_WIDTH, 'attribute' => Attribute::WIDTH];
            }
        }
        // INTRINSIC, RESPONSIVE must have same units for height and width.
        if (\in_array($layout, [Layout::INTRINSIC, Layout::RESPONSIVE], \true) && $width->getUnit() !== $height->getUnit()) {
        ...
        if (!$this->is_empty_attribute_value($heights_attr) && Layout::RESPONSIVE !== $layout) {
            return ['code' => self::INVALID_LAYOUT_HEIGHTS, 'attribute' => Attribute::LAYOUT, 'required_attr_value' => Layout::RESPONSIVE];
        }
        return \true;
    }
    /**
    ...
    {
        if ((!\array_key_exists($input_layout, Layout::TO_SPEC) || Layout::FIXED === $input_layout) && !$input_width->isDefined() && isset($amp_layout_spec['defines_default_width'])) {
    ...
    {
        if ((!\array_key_exists($input_layout, Layout::TO_SPEC) || \in_array($input_layout, [Layout::FIXED, Layout::FIXED_HEIGHT], \true)) && !$input_height->isDefined() && isset($amp_layout_spec['defines_default_height'])) {
            $css_length = new CssLength('1px');
            $css_length->validate(\false, \false);
            return $css_length;
        }
    ...
    private function calculate_layout($layout_attr, CssLength $width, CssLength $height, $sizes_attr, $heights_attr)
    {
        ...
        } elseif ($height->isDefined() && $height->isFluid() || $width->isDefined() && $width->isFluid()) {
            return Layout::FLUID;
        } elseif ($height->isDefined() && (!$width->isDefined() || $width->isAuto())) {
            return Layout::FIXED_HEIGHT;
        } elseif ($height->isDefined() && $width->isDefined() && (!$this->is_empty_attribute_value($sizes_attr) || !$this->is_empty_attribute_value($heights_attr))) {
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY]) && $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY]) {
            if ($node->hasAttribute($attr_name)) {
                return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
            }
            // Check if an alternative name list is specified.
            if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES])) {
                foreach ($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES] as $alt_name) {
                    if ($node->hasAttribute($alt_name)) {
                        return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
                    }
                }
            }
            return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL;
        }
        return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::NOT_APPLICABLE;
    }
    /**
    ...
    private function check_attr_spec_rule_value(\DOMElement $node, $attr_name, $attr_spec_rule)
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE])) {
            if ($node->hasAttribute($attr_name)) {
                if ($this->check_matching_attribute_value($attr_name, $node->getAttribute($attr_name), $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE])) {
                    return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
                }
                return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL;
            }
            if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES])) {
                foreach ($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES] as $alternative_name) {
                    if ($node->hasAttribute($alternative_name)) {
                        if ($this->check_matching_attribute_value($attr_name, $node->getAttribute($alternative_name), $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE])) {
                            return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
                        }
                        return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL;
                    }
    ...
    private function check_matching_attribute_value($attr_name, $attr_value, $spec_values)
    {
        ...
        foreach ($spec_values as $spec_value) {
            ...
            $is_bool_match = '' === $spec_value && \in_array($attr_name, \Google\Web_Stories_Dependencies\AMP_Rule_Spec::$boolean_attributes, \true) && \strtolower($attr_value) === \strtolower($attr_name);
            if ($is_bool_match) {
                return \true;
            }
        }
    ...
    private function check_attr_spec_rule_value_casei(\DOMElement $node, $attr_name, $attr_spec_rule)
    {
        ...
        $result = \Google\Web_Stories_Dependencies\AMP_Rule_Spec::NOT_APPLICABLE;
        $rule_values = (array) $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_CASEI];
        foreach ($rule_values as $rule_value) {
            ...
            if ($node->hasAttribute($attr_name)) {
                ...
                $result = \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL;
            } elseif (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES])) {
                foreach ($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALTERNATIVE_NAMES] as $alternative_name) {
                    if ($node->hasAttribute($alternative_name)) {
    ...
    private function check_attr_spec_rule_value_regex(\DOMElement $node, $attr_name, $attr_spec_rule)
    {
        ...
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX]) && $node->hasAttribute($attr_name)) {
            $rule_value = $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX];
            /*
    ...
    private function check_attr_spec_rule_value_regex_casei(\DOMElement $node, $attr_name, $attr_spec_rule)
    {
        ...
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX_CASEI]) && $node->hasAttribute($attr_name)) {
            $rule_value = $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX_CASEI];
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL]) && $node->hasAttribute($attr_name)) {
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOWED_PROTOCOL])) {
            if ($node->hasAttribute($attr_name)) {
                foreach ($this->extract_attribute_urls($node->getAttributeNode($attr_name)) as $url) {
                    ...
                    if ($this->args['allow_localhost_http_protocol'] && self::LOCALHOST === \wp_parse_url($url, \PHP_URL_HOST)) {
                        return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
                    } elseif (isset($url_scheme) && !\in_array(\strtolower($url_scheme), $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOWED_PROTOCOL], \true)) {
                        return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL;
                    }
                }
                return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
    ...
    private function extract_attribute_urls(\DOMAttr $attribute_node, $spec_attr_name = null)
    {
        ...
        if ('srcset' === $attribute_node->nodeName || 'srcset' === $spec_attr_name) {
            return \array_filter(\array_map(static function ($srcset_part) {
                // Remove descriptors for width and pixel density.
                return \preg_replace('/\\s.*$/', '', \trim($srcset_part));
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOW_RELATIVE]) && !$attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOW_RELATIVE]) {
            if ($node->hasAttribute($attr_name)) {
                foreach ($this->extract_attribute_urls($node->getAttributeNode($attr_name)) as $url) {
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOW_EMPTY])) {
            $allow_empty = $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOW_EMPTY];
        } else {
            $allow_empty = empty($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY]);
        }
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DISALLOWED_VALUE_REGEX])) {
            $pattern = '/' . $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::DISALLOWED_VALUE_REGEX] . '/u';
            if ($node->hasAttribute($attr_name)) {
                $attr_value = $node->getAttribute($attr_name);
    ...
    private function parse_properties_attribute($value)
    {
        ...
        foreach (\explode(',', $value) as $pair) {
            ...
            $properties[\strtolower(\trim($pair_parts[0]))] = $pair_parts[1];
        }
        return $properties;
    }
    /**
    ...
    {
        return \implode(',', \array_map(static function ($property_name) use($properties) {
    ...
    {
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_PROPERTIES]) && $node->hasAttribute($attr_name)) {
            $property_errors = [];
            $properties = $this->parse_properties_attribute($node->getAttribute($attr_name));
            $invalid_properties = \array_diff(\array_keys($properties), \array_keys($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_PROPERTIES]));
            // Fail if there are unrecognized properties.
            foreach ($invalid_properties as $invalid_property) {
                $property_errors[] = [self::DISALLOWED_PROPERTY_IN_ATTR_VALUE, ['name' => $invalid_property, 'value' => $properties[$invalid_property]]];
            }
            foreach ($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_PROPERTIES] as $prop_name => $property_spec) {
                ...
                if (!empty($property_spec['mandatory']) && !isset($properties[$prop_name])) {
                    ...
                    $property_errors[] = [self::MISSING_MANDATORY_PROPERTY, ['name' => $prop_name, 'required_value' => $required_value]];
                    continue;
                }
                ...
                if (isset($required_value) && $prop_value !== $required_value) {
                    $property_errors[] = [self::MISSING_REQUIRED_PROPERTY_VALUE, ['name' => $prop_name, 'value' => $prop_value, 'required_value' => $required_value]];
                }
            }
            if (empty($property_errors)) {
                return [\Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS, []];
            } else {
                return [\Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL, $property_errors];
            }
        }
        return [\Google\Web_Stories_Dependencies\AMP_Rule_Spec::NOT_APPLICABLE, []];
    }
    ...
    private function is_amp_allowed_attribute(\DOMAttr $attr_node, $attr_spec_list)
    {
        ...
        if (isset($attr_spec_list[$attr_name]) || 'data-' === \substr($attr_name, 0, 5) && Amp::BIND_DATA_ATTR_PREFIX !== \substr($attr_name, 0, 14) || 'html' === $attr_node->parentNode->nodeName && \in_array($attr_node->nodeName, ['amp', '⚡'], \true)) {
            return \true;
        }
         ...
         */
        $descendant_reference_points = ['amp-selector' => 'AMP-SELECTOR option', 'amp-story-grid-layer' => 'AMP-STORY-GRID-LAYER default'];
        foreach ($descendant_reference_points as $ancestor_name => $reference_point_spec_name) {
            if (empty($this->open_elements[$ancestor_name])) {
                continue;
            }
            $reference_point_spec = \Google\Web_Stories_Dependencies\AMP_Allowed_Tags_Generated::get_reference_point_spec($reference_point_spec_name);
            if (isset($reference_point_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST][$attr_name])) {
                return \true;
            }
        }
        return \false;
    ...
    private function parse_tag_and_attributes_from_spec_name($spec_name)
    {
        ...
        if (\preg_match('/^(?P<ancestors>.+? )?(?P<tag_name>[a-z0-9_-]+?)( (?P<raw_attrs>\\[.+?\\]))?$/i', $spec_name, $matches)) {
            ...
            if (isset($matches['raw_attrs'])) {
                $raw_attr_pairs = \explode('][', \trim($matches['raw_attrs'], '[]'));
                foreach ($raw_attr_pairs as $raw_attr_pair) {
                    $raw_attr_pair = \explode('=', $raw_attr_pair);
                    $attr_key = $raw_attr_pair[0];
                    $attr_val = isset($raw_attr_pair[1]) ? $raw_attr_pair[1] : \true;
                    $attributes[$attr_key] = $attr_val;
                }
            }
        } else {
            $tag_name = \strtok($spec_name, ' ');
            // Fallback case.
        }
    ...
    private function remove_disallowed_descendants(\DOMElement $node, $allowed_descendants, $spec_name)
    {
        ...
        foreach ($child_elements as $child_element) {
            if (!\in_array($child_element->nodeName, $allowed_descendants, \true)) {
                $this->remove_invalid_child($child_element, ['code' => self::DISALLOWED_DESCENDANT_TAG, 'allowed_descendants' => $allowed_descendants, 'disallowed_ancestor' => $node->parentNode->nodeName, 'spec_name' => $spec_name]);
            } else {
    ...
    private function remove_disallowed_siblings(\DOMElement $node, $spec_name)
    {
        ...
        while (null !== $prev_sibling) {
            ...
            if ($prev_sibling instanceof Element) {
                $this->remove_invalid_child($prev_sibling, ['code' => self::DISALLOWED_SIBLING_TAG, 'sibling' => $node->nodeName, 'spec_name' => $spec_name]);
            }
        ...
        while (null !== $next_sibling) {
            ...
            if ($next_sibling instanceof Element) {
                $this->remove_invalid_child($next_sibling, ['code' => self::DISALLOWED_SIBLING_TAG, 'sibling' => $node->nodeName, 'spec_name' => $spec_name]);
            }
    ...
    private function check_valid_children(\DOMElement $node, $child_tags)
    {
        ...
        if (isset($child_tags['first_child_tag_name_oneof']) && !empty($child_elements[0]) && !\in_array($child_elements[0]->nodeName, $child_tags['first_child_tag_name_oneof'], \true)) {
            return ['code' => self::DISALLOWED_FIRST_CHILD_TAG, 'first_child_tag' => $child_elements[0]->nodeName];
        }
        // Verify that all of the child are among the set of allowed elements.
        if (isset($child_tags['child_tag_name_oneof'])) {
            foreach ($child_elements as $child_element) {
                if (!\in_array($child_element->nodeName, $child_tags['child_tag_name_oneof'], \true)) {
                    return ['code' => self::DISALLOWED_CHILD_TAG, 'child_tag' => $child_element->nodeName];
                }
            }
        }
        ...
        if (isset($child_tags['mandatory_num_child_tags'])) {
            ...
            if ($child_element_count === $child_tags['mandatory_num_child_tags']) {
                ...
                return ['code' => self::INCORRECT_NUM_CHILD_TAGS, 'children_count' => $child_element_count, 'required_child_count' => $child_tags['mandatory_num_child_tags']];
            }
        }
        ...
        if (isset($child_tags['mandatory_min_num_child_tags'])) {
            ...
            if ($child_element_count >= $child_tags['mandatory_min_num_child_tags']) {
                ...
                return ['code' => self::INCORRECT_MIN_NUM_CHILD_TAGS, 'children_count' => $child_element_count, 'required_min_child_count' => $child_tags['mandatory_min_num_child_tags']];
            }
        }
        return \true;
    }
    ...
    private function get_ancestor_with_matching_spec_name(\DOMElement $node, $ancestor_spec_name)
    {
        ...
        while ($node->parentNode instanceof \DOMElement) {
            ...
            if ($node->nodeName === $parsed_spec_name['tag_name']) {
                // Ensure attributes match; if not move up to the next node.
                foreach ($parsed_spec_name['attributes'] as $attr_name => $attr_value) {
                    $match = \true === $attr_value ? $node->hasAttribute($attr_name) : \strtolower($node->getAttribute($attr_name)) === $attr_value;
                    if (!$match) {
                        continue 2;
                    }
                }
    ...
    private function supports_layout($tag_spec, $layout, $fallback = \false)
    {
        ...
        return \in_array(Layout::TO_SPEC[$layout], $tag_spec['amp_layout']['supported_layouts'], \true);
    }
}
